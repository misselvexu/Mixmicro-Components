 ...
 * space
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a> on 2016/12/1.
 */
  ...
  public class LoggerSelector {
    ...
    public Logger select(String name) {
      ...
      if (!isSpaceInitialized()) {
        ...
        if (!warned) {
          tempLogger.warn(
              ">>> Logger Space:{} has not be initialized! Use app logger temporary！", space);
          warned = true;
        }
        return tempLogger;
      }
      if (warned) {
        tempLogger.info("<<< Logger Space:{} was initialized! Use this space logger.", space);
        warned = false;
      }
