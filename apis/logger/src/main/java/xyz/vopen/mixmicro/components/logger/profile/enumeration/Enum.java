...
import xyz.vopen.mixmicro.components.logger.profile.enumeration.internal.NumberType;
import xyz.vopen.mixmicro.components.logger.utils.ClassLoaderUtil;
  ...
  private static Enum createEnum(String name, Number value, boolean withValue) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Could not instantiate enum instance of class " + enumClassName, var9);
    }
    ...
    if (!this.getClass().equals(otherEnum.getClass())) {
      throw new RuntimeException(
          MessageFormat.format(
              "Could not compare object of \"{0}\" with object of \"{1}\"",
              new Object[] {this.getClass().getName(), otherEnum.getClass().getName()}));
    } else {
  ...
  protected Object readResolve() throws ObjectStreamException {
    ...
    if (enumObject == null) {
      throw new InvalidClassException(
          "Enum name \"" + this.getName() + "\" not found in class " + enumClass.getName());
    } else if (!enumObject.value.equals(this.value)) {
      throw new InvalidClassException(
          "Enum value \"" + this.value + "\" does not match in class " + enumClass.getName());
    } else {
      return enumObject;
    }
  }
