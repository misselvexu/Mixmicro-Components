...
import xyz.vopen.mixmicro.components.logger.core.adapter.level.AdapterLevel;
import xyz.vopen.mixmicro.components.logger.core.env.LogEnvUtils;
import xyz.vopen.mixmicro.components.logger.core.proxy.TemporaryILoggerFactoryPool;
import xyz.vopen.mixmicro.components.logger.utils.ReportUtil;
import org.slf4j.ILoggerFactory;
import org.slf4j.Logger;
import xyz.vopen.mixmicro.components.logger.core.factory.*;
...
/**
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a> on 2016/12/7. Updated by
 *     guanchao.ygc@alibaba-inc.com on 14/04/28.
 */
public class MultiAppLoggerSpaceManager {

  private static final AbstractLoggerSpaceFactory NOP_LOGGER_FACTORY =
      new AbstractLoggerSpaceFactory("nop") {
        @Override
  ...
  private static final ConcurrentHashMap<SpaceId, SpaceInfo> SPACES_MAP =
      new ConcurrentHashMap<SpaceId, SpaceInfo>();

  /**
   * 非必要初始化操作。（如果需要为某些space定义特殊的变量，则必须先初始化该方法）。
   *
   * <p>"非必要"，是因为：getLoggerBySpace默认也会执行init操作；
   *
   * <p>使用 {@link MultiAppLoggerSpaceManager} API 建议一定要执行初始化操作即调用这个方法
   *
  ...
  public static void init(SpaceId spaceId, Map<String, String> props) {
    if (isSpaceInitialized(spaceId)) {
      throw new IllegalStateException(
          "Logger Space:" + spaceId.toString() + " is already initialized!");
    }
   ...
   */
  public static Logger setLoggerLevel(
      String loggerName, SpaceId spaceId, AdapterLevel adapterLevel) {
    AbstractLoggerSpaceFactory abstractLoggerSpaceFactory =
        getILoggerFactoryBySpaceName(spaceId, MultiAppLoggerSpaceManager.class.getClassLoader());
    try {
  ...
  public static Logger getLoggerBySpace(
      String name, String spaceName, ClassLoader spaceClassloader) {
  ...
  private static AbstractLoggerSpaceFactory getILoggerFactoryBySpaceName(
      SpaceId spaceId, ClassLoader spaceClassloader) {
  ...
  /**
   * @param spaceName
   * @return @NotThreadSafe 该场景中不要求线程安全；
   */
  ...
  private static AbstractLoggerSpaceFactory createILoggerFactory(
      SpaceId spaceId, ClassLoader spaceClassloader) {
    if (System.getProperty(Constants.MIXMICRO_LOG_DISABLE_PROP_KEY) != null
        && Boolean.TRUE
            .toString()
            .equalsIgnoreCase(System.getProperty(Constants.MIXMICRO_LOG_DISABLE_PROP_KEY))) {
      ReportUtil.reportWarn(
          "MixMicro-Log is disabled!  -D" + Constants.MIXMICRO_LOG_DISABLE_PROP_KEY + "=true");
    ...
    try {
      if (LogEnvUtils.isLogbackUsable(spaceClassloader)) {
        ...
        if (Boolean.TRUE.toString().equalsIgnoreCase(isLogbackDisable)) {
          ...
          ReportUtil.reportDebug(
              "Actual binding is of type [ " + spaceId.toString() + " Logback ]");
          LoggerSpaceFactoryBuilder loggerSpaceFactory4LogbackBuilder =
              new LoggerSpaceFactory4LogbackBuilder(spaceId, spaceInfo);
      ...
      if (LogEnvUtils.isLog4j2Usable(spaceClassloader)) {
        ...
        if (Boolean.TRUE.toString().equalsIgnoreCase(isLog4j2Disable)) {
          ...
          LoggerSpaceFactoryBuilder loggerSpaceFactory4Log4j2Builder =
              new LoggerSpaceFactory4Log4j2Builder(spaceId, spaceInfo);
      ...
      if (LogEnvUtils.isLog4jUsable(spaceClassloader)) {
        ...
        if (Boolean.TRUE.toString().equalsIgnoreCase(isLog4jDisable)) {
          ...
          LoggerSpaceFactoryBuilder loggerSpaceFactory4Log4jBuilder =
              new LoggerSpaceFactory4Log4jBuilder(spaceId, spaceInfo);
      ...
      if (LogEnvUtils.isCommonsLoggingUsable(spaceClassloader)) {
        ...
        if (Boolean.TRUE.toString().equalsIgnoreCase(isLog4jDisable)) {
          ...
          LoggerSpaceFactoryBuilder loggerSpaceFactory4Log4jBuilder =
              new LoggerSpaceFactory4CommonsLoggingBuilder(spaceId, spaceInfo);
