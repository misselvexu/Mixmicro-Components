...
public abstract class AbstractLoggerSpaceFactoryBuilder implements LoggerSpaceFactoryBuilder {
  private static final Logger logger =
      LoggerFactory.getLogger(AbstractLoggerSpaceFactoryBuilder.class);
  ...
  private URL getSpaceLogConfigFileURL(ClassLoader spaceClassloader, String spaceName) {
    ...
    String logConfigLocation =
        spaceName.replace('.', '/')
            ...
            + getLoggingToolName()
            + "/"
            + LOG_XML_CONFIG_FILE_NAME
            + suffix;

    String configProperyConfigLocation =
        spaceName.replace('.', '/')
            + "/"
            + LOG_DIRECTORY
            + "/"
      ...
      if (configFileUrl == null && suffix != null && !suffix.isEmpty()) {
        ...
        logConfigLocation =
            spaceName.replace('.', '/')
                ...
                + getLoggingToolName()
                + "/"
                + String.format(LOG_XML_CONFIG_FILE_ENV_PATTERN, suffix.substring(1));
  ...
  protected URL getResource(
      ClassLoader spaceClassloader, List<URL> logConfigFileUrls, List<URL> configPropertyFileUrls)
      throws IOException {
        ...
        if (configPropertyFileUrls != null) {
          for (URL configPropertyUrl : configPropertyFileUrls) {
            final String absoluteConfigPath =
                new File(configPropertyUrl.getFile()).getParentFile().getAbsolutePath();
            final String absoluteLogPath =
                new File(logConfigUrl.getFile()).getParentFile().getAbsolutePath();
    ...
    if (spaceInfo.getProperty(loggingLevelKey) == null) {
      ...
      for (int i = LOG_LEVEL.length(); i < loggingLevelKey.length(); ++i) {
        if (loggingLevelKey.charAt(i) == '.') {
          String level = spaceInfo.getProperty(loggingLevelKey.substring(0, i + 1) + LOG_START);
  ...
  protected abstract AbstractLoggerSpaceFactory doBuild(
      String spaceName, ClassLoader spaceClassloader, URL confFileUrl);
