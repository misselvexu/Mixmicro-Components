  ...
  static Enum.EnumType getEnumType(Class enumClass) {
    if (enumClass == null) {
      ...
      if (enumType == null) {
        ...
        if (enumType != null) {
          enumType.populateNames(enumClass);
          Enum.EnumType existing = (Enum.EnumType) entryMap.putIfAbsent(enumClass.getName(), enumType);
          if (existing != null) {
            enumType = existing;
          }
        }
      ...
      if (enumType == null) {
        throw new UnsupportedOperationException(
            MessageFormat.format(
                "Could not create EnumType for class \"{0}\"", new Object[] {enumClass.getName()}));
      } else {
        return enumType;
      }
    }
