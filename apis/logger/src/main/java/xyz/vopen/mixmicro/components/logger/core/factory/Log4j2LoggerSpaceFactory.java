...
import xyz.vopen.mixmicro.components.logger.core.spi.Log4j2FilterGenerator;
import xyz.vopen.mixmicro.components.logger.core.spi.Log4j2ReInitializer;
import xyz.vopen.mixmicro.components.logger.core.spi.ReInitializeChecker;
...
public class Log4j2LoggerSpaceFactory extends AbstractLoggerSpaceFactory {
  ...
  public Log4j2LoggerSpaceFactory(
      SpaceId spaceId, Properties properties, URL confFile, String source) throws Throwable {
    super(source);
    this.spaceId = spaceId;
    this.properties = properties;
    this.confFile = confFile;
    boolean willReinitialize = false;
    Iterator<ReInitializeChecker> checkers =
        ServiceLoader.load(ReInitializeChecker.class, this.getClass().getClassLoader()).iterator();
    while (checkers.hasNext()) {
      willReinitialize = !checkers.next().isReInitialize();
    }
    this.loggerContext = initialize(willReinitialize);
    Iterator<Log4j2FilterGenerator> matchers =
        ServiceLoader.load(Log4j2FilterGenerator.class, this.getClass().getClassLoader())
            .iterator();
  ...
  private LoggerContext initialize(boolean willReInitialize) throws Throwable {
    ...
    if (willReInitialize) {
      context =
          new LoggerContext(
              spaceId.getSpaceName(), Constants.MIXMICRO_LOG_FIRST_INITIALIZE, confFile.toURI());
    } else {
    ...
    try {
      ...
      Method getConfigurationMethod =
          configurationFactory.getClass().getMethod("getConfiguration", parameterTypes);
      config =
          (Configuration)
  ...
  public void reInitialize(Map<String, String> environment) {
    ...
    String spaceLoggingPath = environment.get(Constants.LOG_PATH_PREFIX + spaceId.getSpaceName());
    if (!StringUtil.isBlank(spaceLoggingPath)) {
      properties.setProperty(Constants.LOG_PATH_PREFIX + spaceId.getSpaceName(), spaceLoggingPath);
    } else if (Boolean.TRUE.toString().equals(properties.getProperty(IS_DEFAULT_LOG_PATH))) {
      properties.setProperty(
          Constants.LOG_PATH_PREFIX + spaceId.getSpaceName(), properties.getProperty(LOG_PATH));
    }
    ...
    if (Boolean.TRUE.toString().equals(properties.getProperty(Constants.IS_DEFAULT_LOG_LEVEL))
      ...
      for (int i = Constants.LOG_LEVEL.length(); i < loggingLevelKey.length(); ++i) {
        if (loggingLevelKey.charAt(i) == '.') {
          String level = environment.get(loggingLevelKey.substring(0, i + 1) + Constants.LOG_START);
    ...
    String spaceLoggingConfig =
        environment.get(String.format(Constants.LOGGING_CONFIG_PATH, spaceId.getSpaceName()));
    ...
    Iterator<Log4j2ReInitializer> matchers =
        ServiceLoader.load(Log4j2ReInitializer.class, this.getClass().getClassLoader()).iterator();
    if (matchers.hasNext()) {
  ...
  public Logger setLevel(String loggerName, AdapterLevel adapterLevel) {
    final String key =
        Logger.ROOT_LOGGER_NAME.equals(loggerName) ? LogManager.ROOT_LOGGER_NAME : loggerName;
