...
public class OneTimePad {
  ...
  /** Constructs a default <code>OneTimePad</code> instance. */
  public OneTimePad() {
    this(1024 * 1024, 256 * 1024);
  }

  /**
   * Constructs a <code>OneTimePad</code> instance with custom buffer sizes.
   *
   * @param ioBufferSize
  ...
  /**
   * Constructs a <code>OneTimePad</code> instance with custom random source and buffer sizes.
   *
   * @param random
   * @param ioBufferSize
   * @param workBufferSize
  ...
  /**
   * Reads bytes from <code>inFile</code>, pads them with corresponding offset bytes from <code>
   * padFile</code> and writes them to <code>outFile</code>.
   *
   * <p>Returns the new offset for the pad file.
   *
   * @param inFile
   * @param outFile
   * @param padFile
  ...
  /**
   * Sets whether padding bytes should be set to <code>0</code> when they have been used. This
   * potentially increases security because the message can only be padded and unpadded once.
   *
   * @param zeroFill
   */
  ...
  public static void main(String[] args) {
    ...
    options.addOption(
        Option.builder("p").longOpt("pad").hasArg().argName("file").desc("pad file").build());

    options.addOption(
        Option.builder("f").longOpt("offset").hasArg().argName("bytes").desc("offset").build());

    options.addOption(
        Option.builder("s").longOpt("size").hasArg().argName("bytes").desc("size").build());

    options.addOption(
        Option.builder("i").longOpt("in-file").hasArg().argName("file").desc("input file").build());
    ...
    options.addOption(
        Option.builder("z").longOpt("zerofill").desc("overwrite used padding with 0s").build());
