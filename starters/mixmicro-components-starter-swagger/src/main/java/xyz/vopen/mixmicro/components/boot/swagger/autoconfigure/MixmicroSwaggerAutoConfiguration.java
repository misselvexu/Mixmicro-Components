package xyz.vopen.mixmicro.components.boot.swagger.autoconfigure;
...
import xyz.vopen.mixmicro.components.boot.swagger.MixmicroSwaggerProperties;

import java.util.*;
import java.util.stream.Collectors;

import static xyz.vopen.mixmicro.components.boot.swagger.MixmicroSwaggerProperties.MIXMICRO_SWAGGER_CONFIG_PREFIX;

/**
 * {@link MixmicroSwaggerAutoConfiguration}
 *
...
@ConditionalOnProperty(
    prefix = MIXMICRO_SWAGGER_CONFIG_PREFIX,
    name = "enabled",
    matchIfMissing = true)
...
public class MixmicroSwaggerAutoConfiguration implements BeanFactoryAware {
  ...
  public MixmicroSwaggerProperties swaggerProperties() {
    return new MixmicroSwaggerProperties();
  }

  @Bean
  public UiConfiguration uiConfiguration(MixmicroSwaggerProperties swaggerProperties) {
  ...
  @ConditionalOnProperty(
      ...
      havingValue = "true",
      matchIfMissing = true)
  public List<Docket> createRestApi(MixmicroSwaggerProperties swaggerProperties) {
    ...
    if (swaggerProperties.getDocket().size() == 0) {
      ...
      Docket docketForBuilder =
          new Docket(DocumentationType.SWAGGER_2)
              ...
              .globalOperationParameters(
                  buildGlobalOperationParametersFromMixmicroSwaggerProperties(
                      swaggerProperties.getGlobalOperationParameters()));

      if (BASIC_AUTH_TYPE.equalsIgnoreCase(swaggerProperties.getAuthorization().getType())) {
      ...
      Class<?>[] ignoredParameterTypes =
          swaggerProperties.getIgnoredParameterTypes().toArray(array);
    ...
    for (String groupName : swaggerProperties.getDocket().keySet()) {
      MixmicroSwaggerProperties.DocketInfo docketInfo =
   ...
   * 配置基于 ApiKey 的鉴权对象
   *
   * @return instance of {@link ApiKey}
   */
   ...
   *
   * @return instancel of {@link BasicAuth}
   */
   ...
   *
   * @return instance of {@link SecurityContext}
   */
  ...
  private List<Parameter> buildGlobalOperationParametersFromMixmicroSwaggerProperties(
      List<MixmicroSwaggerProperties.GlobalOperationParameter> globalOperationParameters) {
    List<Parameter> parameters = Lists.newArrayList();

    if (Objects.isNull(globalOperationParameters)) {
      return parameters;
    }
    for (MixmicroSwaggerProperties.GlobalOperationParameter globalOperationParameter :
        globalOperationParameters) {
      parameters.add(
  ...
  private List<Parameter> assemblyGlobalOperationParameters(
      List<MixmicroSwaggerProperties.GlobalOperationParameter> globalOperationParameters,
      List<MixmicroSwaggerProperties.GlobalOperationParameter> docketOperationParameters) {

    if (Objects.isNull(docketOperationParameters) || docketOperationParameters.isEmpty()) {
      return buildGlobalOperationParametersFromMixmicroSwaggerProperties(globalOperationParameters);
    }

    Set<String> docketNames =
        docketOperationParameters.stream()
            .map(MixmicroSwaggerProperties.GlobalOperationParameter::getName)
            .collect(Collectors.toSet());

    List<MixmicroSwaggerProperties.GlobalOperationParameter> resultOperationParameters =
        Lists.newArrayList();

    if (Objects.nonNull(globalOperationParameters)) {
      for (MixmicroSwaggerProperties.GlobalOperationParameter parameter :
          globalOperationParameters) {
    ...
    return buildGlobalOperationParametersFromMixmicroSwaggerProperties(resultOperationParameters);
  }
  ...
  private void buildGlobalResponseMessage(
      MixmicroSwaggerProperties swaggerProperties, Docket docketForBuilder) {

    MixmicroSwaggerProperties.GlobalResponseMessage globalResponseMessages =
    ...
    docketForBuilder
        .useDefaultResponseMessages(swaggerProperties.getApplyDefaultResponseMessages())
  ...
  private List<ResponseMessage> getResponseMessageList(
      List<MixmicroSwaggerProperties.GlobalResponseMessageBody> globalResponseMessageBodyList) {
    List<ResponseMessage> responseMessages = new ArrayList<>();
    for (MixmicroSwaggerProperties.GlobalResponseMessageBody globalResponseMessageBody :
