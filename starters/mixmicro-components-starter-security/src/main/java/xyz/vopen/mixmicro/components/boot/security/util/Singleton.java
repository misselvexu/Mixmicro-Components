package xyz.vopen.mixmicro.components.boot.security.util;
...
/**
 * Singleton initializer class that uses an internal supplier to supply the singleton instance. The
 * supplier originally checks whether the instanceSupplier has been initialized or not, but after
...
public final class Singleton<R> implements Supplier<R> {
  ...
  public static <T, R> Singleton<R> fromLazy(
      final Function<T, R> original, Supplier<T> arg0Supplier) {
    return from(() -> original.apply(arg0Supplier.get()));
  }

  public static <T, U, R> Singleton<R> fromLazy(
      final BiFunction<T, U, R> original, Supplier<T> arg0Supplier, Supplier<U> arg1Supplier) {
