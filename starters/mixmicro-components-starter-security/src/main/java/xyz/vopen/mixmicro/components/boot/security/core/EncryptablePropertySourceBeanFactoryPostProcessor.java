...
import xyz.vopen.mixmicro.components.boot.security.annotation.EncryptablePropertySource;
import xyz.vopen.mixmicro.components.boot.security.annotation.EncryptablePropertySources;
...
import static xyz.vopen.mixmicro.components.boot.security.core.EncryptablePropertyResolverConfiguration.FILTER_BEAN_NAME;
import static xyz.vopen.mixmicro.components.boot.security.core.EncryptablePropertyResolverConfiguration.RESOLVER_BEAN_NAME;
...
public class EncryptablePropertySourceBeanFactoryPostProcessor
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
    ...
    Stream<AnnotationAttributes> encryptablePropertySourcesMetadata =
        getEncryptablePropertySourcesMetadata(beanFactory);
    EncryptablePropertyResolver propertyResolver =
        beanFactory.getBean(RESOLVER_BEAN_NAME, EncryptablePropertyResolver.class);
    EncryptablePropertyFilter propertyFilter =
        beanFactory.getBean(FILTER_BEAN_NAME, EncryptablePropertyFilter.class);
    List<PropertySourceLoader> loaders = initPropertyLoaders();
    encryptablePropertySourcesMetadata.forEach(
        eps ->
            loadEncryptablePropertySource(
                eps, env, ac, propertyResolver, propertyFilter, propertySources, loaders));
  }
  ...
  private void loadEncryptablePropertySource(
      AnnotationAttributes encryptablePropertySource,
    ...
    try {
      log.info(
          "Loading Encryptable Property Source '{}'", encryptablePropertySource.getString("name"));
      PropertySource ps =
          createPropertySource(
              encryptablePropertySource, env, resourceLoader, resolver, propertyFilter, loaders);
      propertySources.addLast(ps);
      log.info(
          "Created Encryptable Property Source '{}' from locations: {}",
          ps.getName(),
  ...
  private PropertySource createPropertySource(
      ...
      if (!resource.exists()) {
        if (!ignoreResourceNotFound) {
          throw new IllegalStateException(
              String.format(
                  "Encryptable Property Source '%s' from location: %s Not Found",
                  name, resolvedLocation));
        } else {
          log.info(
              "Ignoring NOT FOUND Encryptable Property Source '{}' from locations: {}",
              name,
              resolvedLocation);
        }
      } else {
    ...
    return new EncryptableEnumerablePropertySourceWrapper<>(
        compositePropertySource, resolver, propertyFilter);
  }

  private String generateName(String name) {
    return !StringUtils.isEmpty(name)
        ? name
        : "EncryptedPropertySource#" + System.currentTimeMillis();
  }

  private Stream<AnnotationAttributes> getEncryptablePropertySourcesMetadata(
      ConfigurableListableBeanFactory beanFactory) {
    return getBeanDefinitionsForAnnotation(
        beanFactory, EncryptablePropertySource.class, EncryptablePropertySources.class);
  }

  private Stream<AnnotationAttributes> getBeanDefinitionsForAnnotation(
      ConfigurableListableBeanFactory bf,
      Class<? extends Annotation> annotation,
      Class<? extends Annotation> repeatable) {
    return Stream.concat(
            Arrays.stream(bf.getBeanNamesForAnnotation(annotation)),
        ...
        .filter(
            bd ->
                bd.getAttribute(CONFIGURATION_CLASS_ATTRIBUTE) != null
                    && bd instanceof AbstractBeanDefinition)
        .map(AnnotatedBeanDefinition::getMetadata)
        .filter(
            am -> am.hasAnnotation(annotation.getName()) || am.hasAnnotation(repeatable.getName()))
        .flatMap(
            am ->
                Optional.ofNullable(
                        (AnnotationAttributes) am.getAnnotationAttributes(annotation.getName()))
                    .map(Stream::of)
                    .orElseGet(
                        () ->
                            Optional.ofNullable(
  ...
  private Optional<List<PropertySource<?>>> loadPropertySource(
      List<PropertySourceLoader> loaders, Resource resource, String sourceName) throws IOException {
  ...
  private List<PropertySource<?>> load(
      PropertySourceLoader loader, String sourceName, Resource resource) {
