    ...
    private Object[] provideParamsForHandle() {
        return p(
            ...
            p(
                ...
                singletonMap(ServletWebErrorHandler.NOT_ACCEPTABLE, singletonList(Argument.arg("types", new HashSet<>(asList(APPLICATION_JSON_VALUE, APPLICATION_PDF_VALUE)))))
            ),
            p(
                new HttpMediaTypeNotSupportedException(APPLICATION_JSON, emptyList()),
                NOT_SUPPORTED,
                UNSUPPORTED_MEDIA_TYPE,
                singletonMap(NOT_SUPPORTED, singletonList(Argument.arg("type", APPLICATION_JSON_VALUE)))
            ),
            p(
                new HttpRequestMethodNotSupportedException("POST"),
                ServletWebErrorHandler.METHOD_NOT_ALLOWED,
                HttpStatus.METHOD_NOT_ALLOWED,
                singletonMap(ServletWebErrorHandler.METHOD_NOT_ALLOWED, singletonList(Argument.arg("method", "POST")))
            ),
            p(
                new MissingServletRequestParameterException("name", "String"),
                MISSING_PARAMETER,
                BAD_REQUEST,
                singletonMap(MISSING_PARAMETER, asList(Argument.arg("name", "name"), Argument.arg("expected", "String")))
            ),
            p(
                new MissingServletRequestPartException("file"),
                MISSING_PART,
                BAD_REQUEST,
                singletonMap(MISSING_PART, singletonList(Argument.arg("name", "file")))
            ),
            p(
                new NoHandlerFoundException("POST", "/test", null),
                NO_HANDLER,
                NOT_FOUND,
                singletonMap(NO_HANDLER, singletonList(Argument.arg("path", "/test")))
            ),
