package xyz.vopen.mixmicro.components.boot.errors.reactive;

import com.fasterxml.jackson.databind.ObjectMapper;
import junitparams.JUnitParamsRunner;
import junitparams.Parameters;
import xyz.vopen.mixmicro.components.boot.errors.ExceptionLogger;
import xyz.vopen.mixmicro.components.boot.errors.handlers.ServletWebErrorHandler;
...
public class ReactiveIT {
    ...
    private Object[] provideInvalidBody() {
        return p(
            ...
            p(ReactiveController.Dto.dto(null, 10, "code"), null, cm("text.required", "The text is required")),
            p(ReactiveController.Dto.dto("", 10, "code"), null, cm("text.required", "The text is required")),
            p(ReactiveController.Dto.dto("text", -1, "code"), null, cm("number.min", "The min is 0")),
            p(ReactiveController.Dto.dto("text", 0), null, cm("range.limit", "Between 1 and 2")),
            p(
                ReactiveController.Dto.dto("", -1),
                null,
    ...
    private Object[] provideInvalidBodyForDefaultCodes() {
        return p(
            p(new ReactiveController.DefaultDto().setActive(false), cm("isActive.shouldBeTrue", null)),
            p(new ReactiveController.DefaultDto().setFailed(true), cm("isFailed.shouldBeFalse", null)),
            p(new ReactiveController.DefaultDto().setDecimalMax(BigDecimal.valueOf(14)), cm("decimalMax.exceedsMax", null)),
            p(new ReactiveController.DefaultDto().setDecimalMin(BigDecimal.ONE), cm("decimalMin.lessThanMin", null)),
            p(new ReactiveController.DefaultDto().setUserEmail("email"), cm("userEmail.invalidEmail", null)),
            p(new ReactiveController.DefaultDto().setWrapper(new ReactiveController.Wrapper().setName("")), cm("wrapper.name.shouldNotBeBlank", null))
        );
    }
