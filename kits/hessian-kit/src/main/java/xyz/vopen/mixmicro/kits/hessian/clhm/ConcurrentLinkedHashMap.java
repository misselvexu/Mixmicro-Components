...
import static xyz.vopen.mixmicro.kits.hessian.clhm.ConcurrentLinkedHashMap.DrainStatus.*;
 ...
 * weighted capacity</tt> threshold. A {@link Weigher} instance determines how many units of
 * capacity that a value consumes. The default weigher assigns each value a weight of <tt>1</tt> to
 ...
 * such as by submitting a task to an {@link ExecutorService}.
 *
 ...
 * allow <tt>null</tt> to be used as a key or value. Unlike {@link LinkedHashMap}, this class does
 * <em>not</em> provide predictable iteration order. A snapshot of the keys and entries may be
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V>
  ...
  public static final class Builder<K, V> {
     ...
     *
     * <p>If unspecified or the executor is shutdown, the catching up will be amortized on user
     * threads during write operations (or during read operations, in the absence of writes).
     *
     * <p>A single-threaded {@link ScheduledExecutorService} should be sufficient for catching up
     * the page replacement policy in many maps.
     *
  ...
  final class Node extends AtomicReference<WeightedValue<V>> implements Linked<Node> {
    ...
    /**
     * Atomically transitions the node from the <tt>alive</tt> state to the <tt>retired</tt> state,
     * if a valid transition.
     */
    void makeRetired() {
      for (; ; ) {
