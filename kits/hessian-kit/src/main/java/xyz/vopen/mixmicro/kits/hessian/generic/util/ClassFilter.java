...
public class ClassFilter {
  ...
  private static boolean doFilter(String type, boolean excludeClass) {
    ...
    while ((index = type.indexOf('.', index + 1)) != -1) {
      ...
      if (PKG_FILTER.contains(pkg)) return true;
    }
    return false;
  }

  /**
   * 在反序列化数组时,会调用此方法检测是否是jdk类型, 包含一个维度, 从指定的特殊类型中查找 不从包名检测的原因是, 数组类型是jdk类型, 但是存放的内容可能是子类,
   * 子类反序列化为GenericObject无法放入数组 NAME_FILTER没有这个问题的原因是, 除了object和date外, 这几个类为final类,无法继承
