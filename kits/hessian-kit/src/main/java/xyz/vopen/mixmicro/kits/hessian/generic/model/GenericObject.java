 ...
 */
package xyz.vopen.mixmicro.kits.hessian.generic.model;

import java.io.Serializable;
import java.util.*;

/**
 * 此类用于表示泛化类型. 除了表示一般的JavaBean, 此类还能表示Enum与AliEnum. 之所以没有为 Enum 与 AliEnum 单独提供泛化类型,
 * 是因为在反序列化时无法通过类名判断具体类型, 所以不管是普通JavaBean 还是 Enum 类型都会反序列化成此类型. 对Enum类型或者AliEnum类型,
 * fields类型只应该包含一个name属性.
 *
...
public final class GenericObject implements Serializable, Comparable<GenericObject> {
  ...
  public String toString(Set<Object> set) {
    if (set.contains(this)) return "<REF>";
    set.add(this);
    ...
    while (it.hasNext()) {
      ...
      if (value instanceof GenericObject) sb.append(((GenericObject) value).toString(set));
