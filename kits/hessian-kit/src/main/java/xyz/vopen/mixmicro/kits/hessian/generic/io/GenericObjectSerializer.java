...
public class GenericObjectSerializer extends AbstractSerializer {
  ...
  private static boolean WRITE_DEFINITION_EVERYTIME =
      Boolean.parseBoolean(
          System.getProperty("generic_hessian_write_definition_everytime", "false"));
  ...
  private static FieldSerializer getFieldSerializer(Class type) {
    if (type == null) return FieldSerializer.SER;
  ...
  public void doWriteObject(GenericObject obj, AbstractHessianOutput out) throws IOException {
    ...
    try {
      ...
      if (!WRITE_DEFINITION_EVERYTIME) {
        ...
        if (SIZE < _offset + 32) output.flushBuffer();
  ...
  private void writeInstance(
      GenericObject obj, ObjectDefinition definition, AbstractHessianOutput out)
      throws IOException {
    String[] _fieldNames = definition.getFieldNames();
