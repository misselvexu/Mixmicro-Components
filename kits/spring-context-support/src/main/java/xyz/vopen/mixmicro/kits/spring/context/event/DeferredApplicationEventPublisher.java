 ...
 */
package xyz.vopen.mixmicro.kits.spring.context.event;
...
/**
 * Before Spring Framework 4.2, {@link AbstractApplicationContext} is an implementation of {@link
 * ApplicationEventPublisher} can't handle the early {@link ApplicationEvent event} that is {@link
 * #publishEvent(ApplicationEvent) published} before {@link ApplicationEventMulticaster}'s
 * initialization, in this scenario, {@link DeferredApplicationEventPublisher} is introduced and
 * used to resolve {@link #publishEvent(ApplicationEvent)} too early to publish {@link
 * ApplicationEvent} when {@link AbstractApplicationContext#initApplicationEventMulticaster() Spring
 * ApplicationContexts' ApplicationEventMulticaster} is not ready. First, {@link
 * DeferredApplicationEventPublisher} stores these early events temporarily, and then {@link
 * #replayDeferredEvents() re-publish} them on {@link ContextRefreshedEvent Application context is
 * ready}.
 *
 * <p>In contrast, If current runtime is based on Spring Framework that {@link
 * #supportsEarlyApplicationEvents() supports early application events}, {@link
 * DeferredApplicationEventPublisher} only delegates the {@link ConfigurableApplicationContext
 * Application Context} that was injected by {@link
 * #DeferredApplicationEventPublisher(ApplicationEventPublisher) constructor}.
 *
...
public class DeferredApplicationEventPublisher
  ...
  private final ConcurrentLinkedQueue<ApplicationEvent> deferredEvents =
      new ConcurrentLinkedQueue<ApplicationEvent>();
  ...
  public DeferredApplicationEventPublisher(ApplicationEventPublisher delegate) {
    ...
    this.shouldDefer =
        !supportsEarlyApplicationEvents() || !isInitializedApplicationEventMulticaster();
  }

  private static Method detectPublishEventMethod() {
    return findMethod(
        ApplicationEventPublisher.class, PUBLISH_EVENT_OBJECT_METHOD_NAME, Object.class);
  }
  ...
  private boolean supportsEarlyApplicationEvents() {
    return context != null
        && findField(context.getClass(), EARLY_APPLICATION_EVENTS_FIELD_NAME) != null;
  }

  private boolean isInitializedApplicationEventMulticaster() {
    return context != null
        && findField(context.getClass(), APPLICATION_EVENT_MULTICASTER_FIELD_NAME) != null;
  }
}
