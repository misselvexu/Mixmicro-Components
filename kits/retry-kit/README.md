...
Retry Kit is a simple Java library to assist with retrying transient failure situations or unreliable code. Retry Kit aims to be readable, well documented and streamlined.
...
Retry Kit does not require any external dependencies. It does require that you are using Java 8 or newer.
...
Retry Kit has built in support for several backoff strategies. They can be specified like so:
...
...where `SomeCustomBackoffStrategy` is an object that implements the `BackoffStrategy` interface. The only mandatory method to implement is `getDurationToWait()` which determines how long to wait between each try. Optionally, the `validateConfig()` method can also be implemented if your backoff strategy needs to verify that the configuration being used is valid.

For examples creating backoff strategies.

### Simple Configs

Retry Kit offers some predefined configurations if you just want to get rolling and worry about tweaking later. These configs can be utilized like so:
...
Retry Kit has the potential throw several unique exceptions when building a config, when executing retries or upon completing execution (if unsuccessful). All Retry Kit exceptions are unchecked. You do not have to explicitly catch them if you wish to let them bubble up cleanly to some other exception handling mechanism. The types of **Retry4jException**'s are:
...
Retry Kit has some built in support for executing and retrying on one or more threads in an asynchronous fashion. The 
...
Retry Kit contains detailed internal logging using [SLF4J](https://www.slf4j.org/manual.html). If you do not specify a SLF4J implementation, these logs will be discarded. If you do provide an implementation (eg: Logback, Log4J, etc) you can specify the log level on the `xyz.vopen.mixmicro.kits.retry-kit` package to set Retry Kit logging to a specific level.

## Other Notes

Retry Kit follows semantic versioning: http://semver.org/. As it is still version 0.x.x and prior to 1.0.0, the API is subject to rapid change and breakage.

There are a number of other retry libraries for Java and the JVM that might better suit your needs. Please feel free to check out the following libraries as well if Retry Kit doesn't fit:
