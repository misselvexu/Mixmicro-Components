...
final class ThresholdOutputStream extends OutputStream {
  ...
  ThresholdOutputStream(
    ...
    assert out1 != null
        && compressingStreamFactory != null
        && context != null
        && thresholdReachedCallback != null;
    buffering = true;
    this.out1 = out1;
  ...
  /**
   * Implementations of this interface are used to receive notification that this stream has either
   * committed bytes to the "raw" stream (without compression), or has committed bytes to a
   * compressing stream.
   */
