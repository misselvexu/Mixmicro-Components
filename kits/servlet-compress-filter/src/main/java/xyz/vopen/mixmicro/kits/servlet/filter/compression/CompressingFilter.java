/*
 * Copyright 2004 and onwards Elve.Xu
 *
...
import xyz.vopen.mixmicro.kits.servlet.filter.compression.statistics.CompressingFilterStats;
...
/**
 * This filter can, based on HTTP headers in a {@link HttpServletRequest}, compress data written to
 * the {@link HttpServletResponse}, or decompress data read from the request. When supported by the
 * client browser, this can potentially greatly reduce the number of bytes written across the
 * network from and to the client. As a {@link Filter}, this class can also be easily added to any
 * J2EE 1.3+ web application.
 *
 * <h3>Features</h3>
 *
 * <p>Why might you want to use this solution compared to others?
 *
 * <ol>
 *   <li>Little in-memory buffering
 *   <li>Handles compressed requests too
 *   <li>Selective compression based on content type, size, or user agent
 *   <li>Exposes compression statistics
 * </ol>
 *
 * <h3>Installation</h3>
 *
 * <ol>
 *   <li>Add the {@code pjl-comp-filter-XX.jar} file containing CompressingFilter to your web
 *       application's {@code WEB-INF/lib} directory.
 *   <li>Add the following entries to your {@code web.xml} deployment descriptor:
 *       <pre>{@code
 * <filter>
 *  <filter-name>CompressingFilter</filter-name>
 *  <filter-class>CompressingFilter</filter-class>
 * </filter>
 * ...
 * <filter-mapping>
 *  <filter-name>CompressingFilter</filter-name>
 ...
 * <h3>Configuration</h3>
 *
 * <p>{@link CompressingFilter} supports the following parameters:
 *
 * <ul>
 ...
 *       client unmodified. If 0, compression always begins immediately. Defaults to 1024.
 *   <li><strong>compressionLevel</strong> (optional): sets the compression level used for response
 *       gzip/deflate compression, from 1 (fastest compression, less CPU) to 9 (best compression,
 *       more CPU), or -1 (platform default, currently equivalent to 6). Defaults to -1.
 *   <li><strong>statsEnabled</strong> (optional): enables collection of statistics. See {@link
 *       CompressingFilterStats}. Defaults to false.
 *   <li><strong>includeContentTypes</strong> (optional): if specified, this is treated as a
 *       comma-separated list of content types (e.g. {@code text/html,text/xml}). The filter will
 *       attempt to only compress responses which specify one of these values as its content type,
 *       for example via {@link HttpServletResponse#setContentType(String)}. Note that the filter
 *       does not know the response content type at the time it is applied, and so must apply itself
 *       and later attempt to disable compression when content type has been set. This will fail if
 *       the response has already been committed. Also note that this parameter cannot be specified
 *       if {@code excludeContentTypes} is too.
 *   <li><strong>excludeContentTypes</strong> (optional): same as above, but specifies a list of
 *       content types to <strong>not</strong> compress. Everything else will be compressed. However
 *       note that any content type that indicates a compressed format (e.g. application/gzip,
 ...
 *       match exactly those paths which should be compressed by this filter. Anything else will not
 *       be compressed. One can also merely apply the filter to a subset of all URIs served by the
 *       web application using standard {@code filter-mapping} elements in {@code web.xml}; this
 *       element provides more fine-grained control for when that mechanism is insufficient. "Paths"
 *       here means values returned by {@link HttpServletRequest#getRequestURI()}. Note that the
 *       regex must match the filename exactly; pattern "static" does <strong>not</strong> match
 *       everything containing the string "static. Use ".*static.*" for that, for example. This
 *       cannot be specified if {@code excludeFileTypes} is too.
 *   <li><strong>excludePathPatterns</strong> (optional): same as above, but specifies a list of
 *       patterns which match paths that should <strong>not</strong> be compressed. Everything else
 *       will be compressed.
 *   <li><strong>includeUserAgentPatterns</strong> (optional): Like {@code includePathPatterns}.
 *       Only requests with {@code User-Agent} headers whose value matches one of these regular
 *       expressions will be compressed. Can't be specified if {@code excludeUserAgentPatterns} is
 *       too.
 *   <li><strong>excludeUserAgentPatterns</strong> (optional): as above, requests whose {@code
 *       User-Agent} header matches one of these patterns will not be compressed.
 *   <li><strong>noVaryHeaderPatterns</strong> (optional): Like {@code includeUserAgentPatterns}.
 *       Requests with {@code User-Agent} headers whose value matches one of these regular
 ...
 * repsonse, as specified in the "Accept-Encoding" HTTP request header:
 *
 * <ul>
 *   <li>gzip
 *   <li>x-gzip
 ...
 *   <li>x-compress
 *   <li>deflate
 *   <li>identity (that is, no compression)
 * </ul>
 *
 * <h4>Request</h4>
 *
 * <p>This filter supports the following compression algorithms when decompressing data from the
 * request body, as specified in the "Content-Encoding" HTTP request header:
 *
 * <ul>
 *   <li>gzip
 *   <li>x-gzip
 ...
 * be set upstream from the filter, not downstream.
 *
 * <h3>Caveats and Notes</h3>
 *
 * <p>The filter requires Java 5 and J2EE 1.4 or better.
 *
 ...
 * want this content compressed too anyway.
 *
 * <p>If a {@link java.io.OutputStream#flush()} occurs before the filter has decided whether to
 * compress or not, it will be forced into compression mode.
 *
 ...
 * associated this ETag with the compressed content and simply return the response again.
 *
 * <p>The filter always sets the {@code Vary} response header to indicate that a different response
 * may be returned based on the {@code Accept-Encoding} header of the request.
 *
 * @author Elve.Xu
 * @since 1.0
 */
public final class CompressingFilter implements Filter {
   ...
   * @since 1.2
   */
  public static final String FORCE_ENCODING_KEY =
      "xyz.vopen.mixmicro.kits.servlet.filter.compression.ForceEncoding";
  /**
   ...
   * response has been compressed, then it will be closed by the time this filter finishes as well.
   *
   * @since 1.2
   */
  ...
  static final String VERSION = "1.0.0.RC1";
  ...
  private static final String ALREADY_APPLIED_KEY =
      "xyz.vopen.mixmicro.kits.servlet.filter.compression.AlreadyApplied";
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (attemptingToCompressResponse) {
      ...
      CompressingHttpServletResponse compressingResponse =
          (CompressingHttpServletResponse) chainResponse;
      ...
      // has been written to the stream, it's almost certainly not valid or even possible to write
      // more
  ...
  private ServletRequest getRequest(ServletRequest request) {
    ...
    String contentEncoding =
        httpRequest.getHeader(CompressingHttpServletResponse.CONTENT_ENCODING_HEADER);
    ...
    return new CompressedHttpServletRequest(
        httpRequest,
  ...
  private ServletResponse getResponse(ServletRequest request, ServletResponse response) {
    ...
    // The "Vary" header must be applied if the response *could be compressed*, not *if it is
    // compressed*.
    // So we have to set it here as we now know the request is allowed by the filter-mapping
    // (web.xml)
    // and by the path and user-agent patterns (above). It may or may not be compressed (depending
    // on the request "Accept-Encoding" header, below) - this is why we have to add the Vary Header
    // now:
    ...
    return new CompressingHttpServletResponse(
        httpResponse, compressingStreamFactory, contentEncoding, context);
  }

  /**
   * Adds the "Vary" header; must be applied if the response <b>could be compressed</b>, not <b>if
   * it is compressed</b>.
   *
  ...
  void setVaryHeader(HttpServletResponse httpResponse, String userAgent) {
    ...
    //     IE6: will ignore the Vary header entirely if the response was delivered with HTTP
    // Compression.
    //     IE7: WinINET will remove the Vary: Accept-Encoding header if it decompressed the
    // response.
    ...
    if (sendVaryHeader(userAgent)) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug(
            "Setting Vary Header because the response *could be compressed*. "
                + VARY_HEADER
                + " : "
   ...
   * set and the given path matches a regular expression in that parameter's list, or if the latter
   * is set and the path does not match, then this method returns {@code true}.
   *
