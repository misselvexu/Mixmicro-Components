...
final class CompressingHttpServletResponse extends HttpServletResponseWrapper {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CompressingHttpServletResponse.class);
  ...
  public PrintWriter getWriter() throws IOException {
    ...
    if (printWriter == null) {
      printWriter =
          new PrintWriter(
              new OutputStreamWriter(getCompressingServletOutputStream(), getCharacterEncoding()),
              true);
    }
    return printWriter;
  }
  ...
  public void addHeader(String name, String value) {
    if (CACHE_CONTROL_HEADER.equalsIgnoreCase(name)) {
      ...
      // appending "-gzip" for instance) to reflect that the body is not the same as the
      // uncompressed
  ...
  private void setCompressionResponseHeaders() {
    ...
    String fullContentEncodingHeader =
  ...
  /**
   * Returns true if and only if the named HTTP header may be set directly by the application, as
   * some headers must be handled specially. null is allowed, though it setting a header named null
   * will probably generate an exception from the underlying {@link HttpServletResponse}. {@link
   * #CONTENT_LENGTH_HEADER}, {@link #CONTENT_ENCODING_HEADER} and {@link #X_COMPRESSED_BY_HEADER}
   * are not allowed.
   *
  ...
  /**
   * Checks to see if the given content type should be compressed. If the content type indicates it
   * is already a compressed format (e.g. contains "gzip") then this wil return {@code false}.
   *
   * <p>Otherwise this checks against the {@code includeContentTypes} and {@code
   * excludeContentTypes} filter init parameters; if the former is set and the given content type is
   * in that parameter's list, or if the latter is set and the content type is not in that list,
  ...
  private CompressingServletOutputStream getCompressingServletOutputStream() throws IOException {
    if (compressingSOS == null) {
      compressingSOS =
          new CompressingServletOutputStream(
              httpResponse.getOutputStream(), compressingStreamFactory, this, context);
    }
  ...
  private boolean mustNotCompress() {
    ...
    if (savedContentLengthSet && savedContentLength < (long) context.getCompressionThreshold()) {
      LOGGER.debug(
          "Will not compress since page has set a content length which is less than "
