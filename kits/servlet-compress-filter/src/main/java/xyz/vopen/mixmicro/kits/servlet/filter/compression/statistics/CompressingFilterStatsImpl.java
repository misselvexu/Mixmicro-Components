...
import xyz.vopen.mixmicro.kits.servlet.filter.compression.CompressingFilter;
import java.io.Serializable;
import java.util.concurrent.atomic.AtomicInteger;
import java.util.concurrent.atomic.AtomicLong;

/**
 * This class provides runtime statistics on the performance of {@link CompressingFilter}. If stats
 * are enabled, then an instance of this object will be available in the servlet context under the
 * key {@link #STATS_KEY}. It can be retrieved and used like so:
 *
 * <pre>
 * ServletContext ctx = ...;
...
public class CompressingFilterStatsImpl implements Serializable, CompressingFilterStats {
   ...
   * context.
   */
  private static final String STATS_KEY = "CompressingFilterStatsImpl";
  /** @serial */
  ...
  /**
   * @return the number of responses which {@link CompressingFilter} has processed but <em>not</em>
   *     compressed for some reason (compression not supported by the browser, for example).
   */
   ...
   *     nothing has yet been compressed. Note that this is (typically) greater than 1, not less
   *     than 1.
   */
  public double getResponseAverageCompressionRatio() {
    return getResponseCompressedBytes() == 0L
        ? 0.0
  ...
  /**
   * @return the number of requests which {@link CompressingFilter} has processed but <em>not</em>
   *     compressed for some reason (no compression requested, for example).
   * @since 1.6
   */
  ...
  /**
   * @return average compression ratio (input bytes / compressed bytes) in requests, or 0 if nothing
   *     has yet been compressed. Note that this is (typically) greater than 1, not less than 1.
   * @since 1.6
   */
  public double getRequestAverageCompressionRatio() {
    return requestCompressedBytes.get() == 0L
        ? 0.0
  ...
  public String toString() {
    return "CompressingFilterStatsImpl[responses compressed: "
        + numResponsesCompressed
        + ", avg. response compression ratio: "
        + getResponseAverageCompressionRatio()
        + ", requests compressed: "
        + numRequestsCompressed
        + ", avg. request compression ratio: "
