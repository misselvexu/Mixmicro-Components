...
import xyz.vopen.mixmicro.kits.servlet.filter.compression.statistics.CompressingFilterEmptyStats;
import xyz.vopen.mixmicro.kits.servlet.filter.compression.statistics.CompressingFilterStats;
import xyz.vopen.mixmicro.kits.servlet.filter.compression.statistics.CompressingFilterStatsImpl;
...
final class CompressingFilterContext {
  ...
  CompressingFilterContext(FilterConfig filterConfig) throws ServletException {
    ...
    if (!contentTypes.isEmpty()) {
      LOGGER.debug(
          "Filter will "
              + (includeContentTypes ? "include" : "exclude")
    ...
    if (!pathPatterns.isEmpty() && LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          "Filter will "
              + (includePathPatterns ? "include" : "exclude")
              + " only these file patterns: "
              + pathPatterns);
    }

    String includeUserAgentPatternsString =
        filterConfig.getInitParameter("includeUserAgentPatterns");
    String excludeUserAgentPatternsString =
        filterConfig.getInitParameter("excludeUserAgentPatterns");
    ...
    if (!userAgentPatterns.isEmpty() && LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          "Filter will "
              + (includeUserAgentPatterns ? "include" : "exclude")
              + " only these User-Agent patterns: "
              + userAgentPatterns);
    }
  }
    ...
    if (compressionThresholdString != null) {
      try {
        ...
        throw new ServletException(
            "Invalid compression threshold: " + compressionThresholdString, nfe);
      }
      if (value < 0) {
