...
public interface FanOutQueue extends Closeable {
  ...
  /**
   * Get timestamp of data item at specific index of the queue, this is the timestamp when
   * corresponding item was appended into the queue.
   *
   * @param index data item index
   ...
   *
   * <p>normally, you don't need to flush explicitly since: 1.) FanOutQueue will automatically flush
   * a cached page when it is replaced out, 2.) FanOutQueue uses memory mapped file technology
  ...
  /**
   * Remove all data before specific timestamp, truncate back files and advance the queue front if
   * necessary.
   *
  ...
  /**
   * Limit the back file size of this queue, truncate back files and advance the queue front if
   * necessary.
   *
   * <p>Note, this is a best effort call, exact size limit can't be guaranteed
   *
  ...
  /**
   * Find an index closest to the specific timestamp when the corresponding item was enqueued. to
   * find latest index, use {@link #LATEST} as timestamp. to find earliest index, use {@link
   * #EARLIEST} as timestamp.
   *
