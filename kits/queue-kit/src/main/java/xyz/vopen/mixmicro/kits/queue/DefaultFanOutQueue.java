...
import xyz.vopen.mixmicro.kits.queue.utils.FolderNameValidator;
 ...
 *
 * <p>Main features: 1. FAST : close to the speed of direct memory access, both enqueue and dequeue
 * are close to O(1) memory access. 2. MEMORY-EFFICIENT : automatic paging & swapping algorithm,
 * only most-recently accessed data is kept in memory. 3. THREAD-SAFE : multiple threads can
 * concurrently enqueue and dequeue without data corruption. 4. PERSISTENT - all data in queue is
 * persisted on disk, and is crash resistant. 5. BIG(HUGE) - the total size of the queued data is
 * only limited by the available disk space. 6. FANOUT - support fan out semantics, multiple
 * consumers can independently consume a single queue without intervention, everyone has its own
 * queue front index. 7. CLIENT MANAGED INDEX - support access by index and the queue index is
 * managed at client side.
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
public class DefaultFanOutQueue implements FanOutQueue {

  final DefaultBigArray innerArray;

  // 2 ^ 3 = 8
  static final int QUEUE_FRONT_INDEX_ITEM_LENGTH_BITS = 3;
  // size in bytes of queue front index page
  static final int QUEUE_FRONT_INDEX_PAGE_SIZE = 1 << QUEUE_FRONT_INDEX_ITEM_LENGTH_BITS;
  ...
  final ConcurrentMap<String, QueueFront> queueFrontMap =
      new ConcurrentHashMap<String, QueueFront>();
   ...
   *     DefaultBigArray#MINIMUM_DATA_PAGE_SIZE}
   * @throws IOException exception throws if there is any IO error during queue initialization
   */
  public DefaultFanOutQueue(String queueDir, String queueName, int pageSize) throws IOException {
    innerArray = new DefaultBigArray(queueDir, queueName, pageSize);
  }

  /**
   * A big, fast and persistent queue implementation with fanout support, use default back data page
   * size, see {@link DefaultBigArray#DEFAULT_DATA_PAGE_SIZE}
   *
   * @param queueDir the directory to store queue data
   * @param queueName the name of the queue, will be appended as last part of the queue directory
   * @throws IOException exception throws if there is any IO error during queue initialization
   */
  public DefaultFanOutQueue(String queueDir, String queueName) throws IOException {
    this(queueDir, queueName, DefaultBigArray.DEFAULT_DATA_PAGE_SIZE);
  }
  ...
  class QueueFront {
    ...
    void validateAndAdjustIndex() throws IOException {
      if (index.get()
          != innerArray.arrayHeadIndex.get()) { // ok that index is equal to array head index
        try {
          innerArray.validateIndex(index.get());
        } catch (
            IndexOutOfBoundsException ex) { // maybe the back array has been truncated to limit size
          resetIndex();
        }
      }
    }
