...
import xyz.vopen.mixmicro.kits.queue.cache.DefaultLRUCache;
import xyz.vopen.mixmicro.kits.queue.cache.LRUCache;
...
/**
 * Mapped mapped page resource manager, responsible for the creation, cache, recycle of the mapped
 * pages.
 *
 * <p>automatic paging and swapping algorithm is leveraged to ensure fast page fetch while keep
 * memory usage efficient at the same time.
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
public class DefaultMappedPageFactory implements MappedPageFactory {

  private static final Logger logger = LoggerFactory.getLogger(DefaultMappedPageFactory.class);
  ...
  private LRUCache<Long, DefaultMappedPage> cache;

  public DefaultMappedPageFactory(int pageSize, String pageDir, long cacheTTL) {
  ...
  public MappedPage acquirePage(long index) throws IOException {
    DefaultMappedPage mpi = cache.get(index);
        ...
        synchronized (lock) { // only lock the creation of page index
          ...
          if (mpi == null) {
            ...
            try {
              ...
              mpi = new DefaultMappedPage(mbb, fileName, index);
  ...
  @Override
  public void flush() {
    Collection<DefaultMappedPage> cachedPages = cache.getValues();
