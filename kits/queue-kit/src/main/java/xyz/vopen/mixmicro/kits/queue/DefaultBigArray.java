package xyz.vopen.mixmicro.kits.queue;

import xyz.vopen.mixmicro.kits.queue.page.DefaultMappedPageFactory;
import xyz.vopen.mixmicro.kits.queue.page.MappedPage;
import xyz.vopen.mixmicro.kits.queue.page.MappedPageFactory;
import xyz.vopen.mixmicro.kits.queue.utils.Calculator;
 ...
 *
 * <p>Main features: 1. FAST : close to the speed of direct memory access, extremely fast in append
 * only and sequential read modes, sequential append and read are close to O(1) memory access,
 * random read is close to O(1) memory access if data is in cache and is close to O(1) disk access
 * if data is not in cache. 2. MEMORY-EFFICIENT : automatic paging & swapping algorithm, only
 * most-recently accessed data is kept in memory. 3. THREAD-SAFE : multiple threads can concurrently
 * read/append the array without data corruption. 4. PERSISTENT - all array data is persisted on
 * disk, and is crash resistant. 5. BIG(HUGE) - the total size of the array data is only limited by
 * the available disk space.
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
public class DefaultBigArray implements BigArray {
  ...
  // 2 ^ 17 = 1024 * 128
  static final int INDEX_ITEMS_PER_PAGE_BITS = 17; // 1024 * 128
  // number of items per page
  static final int INDEX_ITEMS_PER_PAGE = 1 << INDEX_ITEMS_PER_PAGE_BITS;
  // 2 ^ 5 = 32
  static final int INDEX_ITEM_LENGTH_BITS = 5;
  // length in bytes of an index item
  static final int INDEX_ITEM_LENGTH = 1 << INDEX_ITEM_LENGTH_BITS;
  // size in bytes of an index page
  static final int INDEX_PAGE_SIZE = INDEX_ITEM_LENGTH * INDEX_ITEMS_PER_PAGE;
  ...
  static final int META_DATA_PAGE_SIZE = 1 << META_DATA_ITEM_LENGTH_BITS;
  ...
  /**
   * A big array implementation supporting sequential write and random read, use default back data
   * file size per page, see {@link #DEFAULT_DATA_PAGE_SIZE}.
   *
   ...
   */
  public DefaultBigArray(String arrayDir, String arrayName) throws IOException {
   ...
   * @param arrayDir directory for array data store
   * @param arrayName the name of the array, will be appended as last part of the array directory
   * @param pageSize the back data file size per page in bytes, see minimum allowed {@link
   *     #MINIMUM_DATA_PAGE_SIZE}.
   * @throws IOException exception throws during array initialization
   */
  public DefaultBigArray(String arrayDir, String arrayName, int pageSize) throws IOException {
  ...
  void commonInit() throws IOException {
    ...
    this.indexPageFactory =
        new DefaultMappedPageFactory(
            INDEX_PAGE_SIZE, this.arrayDirectory + INDEX_PAGE_FOLDER, INDEX_PAGE_CACHE_TTL);
    this.dataPageFactory =
        new DefaultMappedPageFactory(
            DATA_PAGE_SIZE, this.arrayDirectory + DATA_PAGE_FOLDER, DATA_PAGE_CACHE_TTL);
  ...
  void initDataPageIndex() throws IOException {
    ...
    if (this.isEmpty()) {
      ...
      try {
        ...
        previousIndexPageIndex =
            Calculator.div(previousIndex, INDEX_ITEMS_PER_PAGE_BITS); // shift optimization
        previousIndexPage = this.indexPageFactory.acquirePage(previousIndexPageIndex);
        int previousIndexPageOffset =
            (int)
                (Calculator.mul(
                    Calculator.mod(previousIndex, INDEX_ITEMS_PER_PAGE_BITS),
  ...
  public long append(byte[] data) throws IOException {
    try {
      ...
      try {
        ...
        toAppendIndexPageIndex =
            Calculator.div(toAppendArrayIndex, INDEX_ITEMS_PER_PAGE_BITS); // shift optimization
        toAppendIndexPage = this.indexPageFactory.acquirePage(toAppendIndexPageIndex);
        int toAppendIndexItemOffset =
            (int)
                (Calculator.mul(
                    Calculator.mod(toAppendArrayIndex, INDEX_ITEMS_PER_PAGE_BITS),
                    INDEX_ITEM_LENGTH_BITS));
  ...
  ByteBuffer getIndexItemBuffer(long index) throws IOException {
    ...
    try {
      ...
      int indexItemOffset =
          (int)
              (Calculator.mul(
                  Calculator.mod(index, INDEX_ITEMS_PER_PAGE_BITS), INDEX_ITEM_LENGTH_BITS));
  ...
  public long findClosestIndex(long timestamp) throws IOException {
    try {
      ...
      if (tailIndex < lastIndex) {
        ...
        closestIndex =
            Math.abs(timestamp - lowPartTimestamp) < Math.abs(timestamp - highPartTimestamp)
                ? lowPartClosestIndex
                : highPartClosetIndex;
      }
  ...
  public void limitBackFileSize(long sizeLimit) throws IOException {
    ...
    try {
      ...
      while (true) {
        ...
        if (Calculator.mod(tailIndex, INDEX_ITEMS_PER_PAGE_BITS)
            == 0) { // take index page into account
