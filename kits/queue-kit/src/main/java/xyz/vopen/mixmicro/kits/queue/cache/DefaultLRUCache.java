package xyz.vopen.mixmicro.kits.queue.cache;
...
/**
 * Simple and thread-safe LRU cache implementation, supporting time to live and reference counting
 * for entry.
 *
 * <p>in current implementation, entry expiration and purge(mark&sweep) is triggered by put
 * operation, and resource closing after mark&sweep is done in async way.
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 * @param <K> key
 * @param <V> value
 */
public class DefaultLRUCache<K, V extends Closeable> implements LRUCache<K, V> {

  private static final Logger logger = LoggerFactory.getLogger(DefaultLRUCache.class);

  // milliseconds
  public static final long DEFAULT_TTL = 10 * 1000;
   ...
   *
   * <p>Call this only after you have closed your bigqueue instance.
   */
  ...
  public void put(K key, V value, long ttlInMilliSeconds) {
    ...
    if (valuesToClose != null && valuesToClose.size() > 0) {
      if (logger.isDebugEnabled()) {
        ...
        logger.info(
            "Mark&Sweep found " + size + (size > 1 ? " resources" : " resource") + " to close.");
      }
   ...
   *
   * <p>a separate thread can also do this.
   */
  private Collection<V> markAndSweep() {
    ...
    for (K key : keys) {
      ...
      if (ttl.refCount.get() <= 0
          && (currentTS - ttl.lastAccessedTimestamp.get())
              > ttl.ttl) { // remove object with no reference and expired
