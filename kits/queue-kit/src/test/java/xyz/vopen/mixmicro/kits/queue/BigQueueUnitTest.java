...
public class BigQueueUnitTest {
  ...
  public void simpleTest() throws IOException {
    for (int i = 1; i <= 2; i++) {
      ...
      bigQueue = new DefaultBigQueue(testDir, "simple_test");
  ...
  public void bigLoopTest() throws IOException {
    bigQueue = new DefaultBigQueue(testDir, "big_loop_test");
    assertNotNull(bigQueue);
  ...
  public void loopTimingTest() throws IOException {
    bigQueue = new DefaultBigQueue(testDir, "loop_timing_test");
    assertNotNull(bigQueue);
  ...
  public void testInvalidDataPageSize() throws IOException {
    try {
      bigQueue =
          new DefaultBigQueue(
              testDir, "testInvalidDataPageSize", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE - 1);
    ...
    // ok
    bigQueue =
        new DefaultBigQueue(testDir, "testInvalidDataPageSize", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  }
  ...
  public void testApplyForEachDoNotChangeTheQueue() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "testApplyForEachDoNotChangeTheQueue", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  public void concurrentApplyForEachTest() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "concurrentApplyForEachTest", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  public void testIfFutureIsCompletedAtEnqueueAndListenersAreCalled() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir,
            "testIfFutureIsCompletedAtEnqueueAndListenersAreCalled",
  ...
  public void testIfFutureIsCompletedIfListenerIsRegisteredLater() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir,
            "testIfFutureIsCompletedIfListenerIsRegisteredLater",
            DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  public void testIfFutureIsRecreatedAfterDequeue() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "testIfFutureIsRecreatedAfterDequeue", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
    Executor executor = mock(Executor.class);
    bigQueue.enqueue("test".getBytes());
    ListenableFuture<byte[]> future = bigQueue.dequeueAsync();
    assertTrue(future.isDone());
  ...
  public void testIfFutureIsCanceledAfterClosing() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "testIfFutureIsCanceledAfterClosing", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
    Executor executor = mock(Executor.class);
    ListenableFuture<byte[]> future = bigQueue.dequeueAsync();
    future.addListener(mock(Runnable.class), executor);
    bigQueue.close();
  ...
  public void testIfFutureWorksAfterQueueRecreation() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "testIfFutureWorksAfterQueueRecreation", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
    bigQueue.enqueue("test".getBytes());
    bigQueue.close();
  ...
  public void testIfFutureIsInvalidatedIfAllItemsWhereRemoved() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir,
            "testIfFutureIsInvalidatedIfAllItemsWhereRemoved",
            DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
    bigQueue.enqueue("test".getBytes());
  ...
  public void testParallelAsyncDequeueAndPeekOperations() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir,
            "testParallelAsyncDequeueAndPeekOperations",
            DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  public void testMultiplePeekAsyncOperations() throws Exception {
    bigQueue =
        new DefaultBigQueue(
            testDir, "testMultiplePeekAsyncOperations", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  public void testFutureIfConsumerDequeuesAllWhenAsynchronousWriting() throws Exception {
      bigQueue = new DefaultBigQueue(testDir, "testFutureIfConsumerDequeuesAllWhenAsynchronousWriting", DefaultBigArray.MINIMUM_DATA_PAGE_SIZE);
  ...
  */
  ...
  private class DefaultItemIterator implements BigQueue.ItemIterator {
