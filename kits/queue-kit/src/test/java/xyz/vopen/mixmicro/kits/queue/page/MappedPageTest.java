...
public class MappedPageTest {
	...
	public void testSingleThread() throws IOException {
		...
		mappedPageFactory = new DefaultMappedPageFactory(pageSize, testDir + "/test_single_thread", 2 * 1000);
		
		MappedPage mappedPage = this.mappedPageFactory.acquirePage(0);
	...
	public void testMultiThreads() {
		int pageSize = 1024 * 1024 * 32;
		mappedPageFactory = new DefaultMappedPageFactory(pageSize, testDir + "/test_multi_threads", 2 * 1000);
		
		int threadNum = 100;
		int pageNumLimit = 50;
		
		Set<MappedPage> pageSet = Collections.newSetFromMap(new ConcurrentHashMap<MappedPage, Boolean>());
	...
	private static class Worker extends Thread {
		...
		private Set<MappedPage> sharedPageSet;
		private List<ByteBuffer> localBufferList;
		
		public Worker(int id, MappedPageFactory pageFactory, int pageNumLimit,
                  Set<MappedPage> sharedPageSet, List<ByteBuffer> localBufferList) {
			this.id = id;
		...
		public void run() {
			for(int i = 0; i < pageNumLimit; i++) {
				try {
					MappedPage page = this.pageFactory.acquirePage(i);
