...
import xyz.vopen.mixmicro.components.remoting.rpc.exception.RpcServerException;
...
public class RpcCommandFactory implements CommandFactory {
  ...
  @Override
  public RpcResponseCommand createResponse(
      final Object responseObject, final RemotingCommand requestCmd) {
  ...
  @Override
  public RemotingCommand createSendFailedResponse(
      final InetSocketAddress address, Throwable throwable) {
    ResponseCommand responseCommand = new ResponseCommand();
  ...
  /**
   * create server exception using error msg and fill the stack trace using the stack trace of
   * throwable.
   *
  ...
  private RpcServerException createServerException(Throwable t, String errMsg) {
    String formattedErrMsg =
        String.format(
            "[Server]OriginErrorMsg: %s: %s. AdditionalErrorMsg: %s",
            t.getClass().getName(), t.getMessage(), errMsg);
