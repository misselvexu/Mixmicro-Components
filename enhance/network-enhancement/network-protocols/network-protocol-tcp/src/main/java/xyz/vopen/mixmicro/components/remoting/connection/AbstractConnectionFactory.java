 ...
 */
package xyz.vopen.mixmicro.components.remoting.connection;
...
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcProtocol;
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcProtocolV2;
 ...
 *
 * @author Elve.Xu (iskp.me@gmail.com) 2018-06-20 14:29
 */
public abstract class AbstractConnectionFactory implements ConnectionFactory {

  private static final Logger logger =
      RemotingLoggerFactory.getLogger(AbstractConnectionFactory.class);
  ...
  public void init(final ConnectionEventHandler connectionEventHandler) {
    ...
    final boolean flushConsolidationSwitch =
        this.confInstance.switches().isOn(GlobalSwitch.CODEC_FLUSH_CONSOLIDATION);
    bootstrap.handler(
  ...
  public Connection createConnection(Url url) throws Exception {
    ...
    Connection conn =
        new Connection(channel, ProtocolCode.fromBytes(url.getProtocol()), url.getVersion(), url);
    if (channel.isActive()) {
  ...
  public Connection createConnection(String targetIP, int targetPort, int connectTimeout)
    ...
    Connection conn =
        new Connection(
            ...
            RpcProtocolV2.PROTOCOL_VERSION_1,
  ...
  public Connection createConnection(
      String targetIP, int targetPort, byte version, int connectTimeout) throws Exception {
    Channel channel = doCreateConnection(targetIP, targetPort, connectTimeout);
    Connection conn =
        new Connection(
            channel,
  ...
  private SslContext initSSLContext() {
    ...
    try {
      ...
      TrustManagerFactory tmf =
          TrustManagerFactory.getInstance(RpcConfigManager.client_ssl_tmf_algorithm());
      tmf.init(ks);
