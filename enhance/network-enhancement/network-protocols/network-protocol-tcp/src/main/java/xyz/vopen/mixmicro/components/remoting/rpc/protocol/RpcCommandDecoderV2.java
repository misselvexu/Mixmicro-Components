...
import xyz.vopen.mixmicro.components.remoting.CommandDecoder;
...
import xyz.vopen.mixmicro.components.remoting.rpc.*;
import xyz.vopen.mixmicro.components.remoting.util.CrcUtil;
...
public class RpcCommandDecoderV2 implements CommandDecoder {
  ...
  {
    lessLen =
        RpcProtocolV2.getResponseHeaderLength() < RpcProtocolV2.getRequestHeaderLength()
        ...
        if (in.readableBytes() > 2 + 1) {
          ...
          if (type == RpcCommandType.REQUEST || type == RpcCommandType.REQUEST_ONEWAY) {
            ...
            if (in.readableBytes() >= RpcProtocolV2.getRequestHeaderLength() - 3) {
              ...
              boolean crcSwitchOn =
                  ProtocolSwitch.isOn(ProtocolSwitch.CRC_SWITCH_INDEX, protocolSwitchValue);
