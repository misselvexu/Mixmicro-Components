...
import xyz.vopen.mixmicro.components.remoting.CommandCode;
import xyz.vopen.mixmicro.components.remoting.CommandFactory;
import xyz.vopen.mixmicro.components.remoting.CommandHandler;
import xyz.vopen.mixmicro.components.remoting.CommonCommandCode;
import xyz.vopen.mixmicro.components.remoting.ProcessorManager;
...
import xyz.vopen.mixmicro.components.remoting.RemotingProcessor;
import xyz.vopen.mixmicro.components.remoting.ResponseStatus;
...
import xyz.vopen.mixmicro.components.remoting.rpc.RpcCommand;
import xyz.vopen.mixmicro.components.remoting.rpc.RpcCommandType;
import xyz.vopen.mixmicro.components.remoting.rpc.RpcConfigManager;
...
public class RpcCommandHandler implements CommandHandler {
  ...
  public RpcCommandHandler(CommandFactory commandFactory) {
    ...
    this.processorManager.registerProcessor(
        RpcCommandCode.RPC_REQUEST, new RpcRequestProcessor(this.commandFactory));
    // process response
    this.processorManager.registerProcessor(
        RpcCommandCode.RPC_RESPONSE, new RpcResponseProcessor());

    this.processorManager.registerProcessor(
        CommonCommandCode.HEARTBEAT, new RpcHeartBeatProcessor());

    this.processorManager.registerDefaultProcessor(
        new AbstractRemotingProcessor<RemotingCommand>() {
          @Override
  ...
  private void processExceptionForSingleCommand(RemotingContext ctx, Object msg, Throwable t) {
    ...
    final String emsg =
        "Exception caught when processing "
      ...
      if (cmd.getType() != RpcCommandType.REQUEST_ONEWAY) {
        if (t instanceof RejectedExecutionException) {
          final ResponseCommand response =
              this.commandFactory.createExceptionResponse(
                  id, ResponseStatus.SERVER_THREADPOOL_BUSY);
          // RejectedExecutionException here assures no response has been sent back
          // Other exceptions should be processed where exception was caught, because here we don't
          // known whether ack had been sent back.
          ctx.getChannelContext()
  ...
  public void registerProcessor(
      CommandCode cmd, @SuppressWarnings("rawtypes") RemotingProcessor processor) {
