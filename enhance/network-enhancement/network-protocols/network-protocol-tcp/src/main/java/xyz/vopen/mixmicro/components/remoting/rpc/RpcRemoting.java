...
import xyz.vopen.mixmicro.components.remoting.BaseRemoting;
import xyz.vopen.mixmicro.components.remoting.config.switches.ProtocolSwitch;
...
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcProtocolManager;
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcRequestCommand;
...
public abstract class RpcRemoting extends BaseRemoting {
  ...
  public abstract void oneway(
      final Url url, final Object request, final InvokeContext invokeContext)
      throws RemotingException, InterruptedException;
  ...
  public void oneway(final Connection conn, final Object request, final InvokeContext invokeContext)
    ...
    RequestCommand requestCommand =
        (RequestCommand) toRemotingCommand(request, conn, invokeContext, -1);
   ...
   * @throws InterruptedException
   */
  public abstract Object invokeSync(
      final Url url,
      final Object request,
      final InvokeContext invokeContext,
      final int timeoutMillis)
  ...
  public Object invokeSync(
    ...
    ResponseCommand responseCommand =
        (ResponseCommand) super.invokeSync(conn, requestCommand, timeoutMillis);
    responseCommand.setInvokeContext(invokeContext);

    Object responseObject =
        RpcResponseResolver.resolveResponseObject(
            responseCommand, RemotingUtil.parseRemoteAddress(conn.getChannel()));
  ...
  public RpcResponseFuture invokeWithFuture(
      final String addr, final Object request, final InvokeContext invokeContext, int timeoutMillis)
      throws RemotingException, InterruptedException {
    Url url = this.addressParser.parse(addr);
   ...
   * @throws InterruptedException
   */
  public abstract RpcResponseFuture invokeWithFuture(
      final Url url,
      final Object request,
      final InvokeContext invokeContext,
      final int timeoutMillis)
   ...
   * @throws InterruptedException
   */
  public abstract void invokeWithCallback(
      final Url url,
      final Object request,
      final InvokeContext invokeContext,
   ...
   */
  protected RemotingCommand toRemotingCommand(
      Object request, Connection conn, InvokeContext invokeContext, int timeoutMillis)
    ...
    if (null != invokeContext) {
      ...
      // enable crc by default, user can disable by set invoke context `false` for key
      // `InvokeContext.BOLT_CRC_SWITCH`
      Boolean crcSwitch =
          invokeContext.get(InvokeContext.BOLT_CRC_SWITCH, ProtocolSwitch.CRC_SWITCH_DEFAULT_VALUE);
  ...
  protected abstract void preProcessInvokeContext(
  ...
  /**
   * @see BaseRemoting#createInvokeFuture(Connection, RemotingCommand, InvokeContext,
   *     InvokeCallback)
   */
  @Override
  protected InvokeFuture createInvokeFuture(
      Connection conn,
