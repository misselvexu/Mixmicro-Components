 ...
 * under the License.
 */
package xyz.vopen.mixmicro.components.remoting.codec;
...
/**
 * This class mainly hack the {@link io.netty.handler.codec.ByteToMessageDecoder} to provide batch
 * submission capability. This can be used the same way as ByteToMessageDecoder except the case your
 * following inbound handler may get a decoded msg, which actually is an array list, then you can
 * submit the list of msgs to an executor to process. For example
 *
 * <pre>
...
public abstract class AbstractBatchDecoder extends ChannelInboundHandlerAdapter {
      ...
      new Cumulator() {
        ...
        public ByteBuf cumulate(ByteBufAllocator alloc, ByteBuf cumulation, ByteBuf in) {
          ...
          if (cumulation.refCnt() > 1) {
            // Expand cumulation (by replace it) when the refCnt is greater then 1 which may happen
            // when the user
   ...
   * nothing is mixed up.
   *
   * <p>Default is {@code false} as this has performance impacts.
   */
  ...
  /**
   * If {@code true} then only one message is decoded on each {@link
   ...
   * local variable {@code RecyclableArrayList out}. If has decoded more than one msg, then
   * construct an array list to submit all decoded msgs to the pipeline.
   *
   ...
   *
   * <p>By default this will just call {@link #decode(ChannelHandlerContext, ByteBuf, List)} but
