...
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcDeserializeLevel;
...
public abstract class RpcCommand implements RemotingCommand {
  ...
  private byte type;
  /**
   * Serializer, see the Configs.SERIALIZER_DEFAULT for the default serializer. Notice: this can not
   * be changed after initialized at runtime.
   */
   ...
   *
   * <ol>
   *   <li>If mask <= {@link RpcDeserializeLevel#DESERIALIZE_CLAZZ}, only deserialize clazz - only
   *       one part.
   *   <li>If mask <= {@link RpcDeserializeLevel#DESERIALIZE_HEADER}, deserialize clazz and header -
   *       two parts.
   *   <li>If mask <= {@link RpcDeserializeLevel#DESERIALIZE_ALL}, deserialize clazz, header and
   *       content - all three parts.
   * </ol>
   *
   * @param mask
