...
public class RpcClient extends AbstractRemotingClient {
  ...
  public void startup() throws LifeCycleException {
    ...
    this.rpcRemoting =
        new RpcClientRemoting(new RpcCommandFactory(), this.addressParser, this.connectionManager);
    ...
    if (switches().isOn(GlobalSwitch.CONN_MONITOR_SWITCH)) {
      if (monitorStrategy == null) {
        connectionMonitor =
            new DefaultConnectionMonitor(new ScheduledDisconnectStrategy(), this.connectionManager);
      } else {
  ...
  @Override
  public void oneway(final String address, final Object request)
  ...
  @Override
  public RpcResponseFuture invokeWithFuture(
      final Connection conn, final Object request, int timeoutMillis) throws RemotingException {
    ensureStarted();
  ...
  public void invokeWithCallback(
    ...
    this.rpcRemoting.invokeWithCallback(
        address, request, invokeContext, invokeCallback, timeoutMillis);
  }
  ...
  @Override
  public Connection getConnection(String address, int connectTimeout)
      throws RemotingException, InterruptedException {
    ensureStarted();
