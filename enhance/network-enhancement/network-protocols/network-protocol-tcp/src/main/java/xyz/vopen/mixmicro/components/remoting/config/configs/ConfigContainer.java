 ...
 */
package xyz.vopen.mixmicro.components.remoting.config.configs;

/**
 * the interface of a config container
 *
 * <p>Mainly used to manage config by user api, this is instance related, not globally. That is to
 * say, different remoting instance client or server hold different ConfigContainer.
 *
...
public interface ConfigContainer {
   ...
   *
   * @param configType config types in the config container, different config type can hold the same
   *     config item key
  ...
  /**
   * init a config item with certain config type, and the value can be any type. Notice: the value
   * can be overwrite if you set more than once.
   *
   * @param configType config types in the config container, different config type can hold the same
   *     config item key
   * @param configItem config items in the config container
