...
import xyz.vopen.mixmicro.components.remoting.rpc.RpcConfigs;
import xyz.vopen.mixmicro.components.remoting.rpc.RpcResponseFuture;
 ...
 *
 * @author Elve.Xu (iskp.me@gmail.com) 2018-11-07 11:56
 */
public interface RemotingClient extends Configurable, LifeCycle {
   ...
   *   <li>When do invocation, use the string address to find a available connection, if none then
   *       create one.
   *       <ul>
   *         <li>You can use {@link RpcConfigs#CONNECT_TIMEOUT_KEY} to specify connection timeout,
   *             time unit is milliseconds, e.g [127.0.0.1:12200?_CONNECTTIMEOUT=3000]
   *         <li>You can use {@link RpcConfigs#CONNECTION_NUM_KEY} to specify connection number for
   *             each ip and port, e.g [127.0.0.1:12200?_CONNECTIONNUM=30]
   *         <li>You can use {@link RpcConfigs#CONNECTION_WARMUP_KEY} to specify whether need warmup
   *             all connections for the first time you call this method, e.g
   *             [127.0.0.1:12200?_CONNECTIONWARMUP=false]
   *       </ul>
   ...
   */
  void oneway(final String address, final Object request)
      throws RemotingException, InterruptedException;

  /**
   * Oneway invocation with a {@link InvokeContext}, common api notice please see {@link
   ...
   * <ol>
   *   <li><b>DO NOT modify the request object concurrently when this method is called.</b>
   *   <li>When do invocation, use the parsed {@link Url} to find a available connection, if none
   *       then create one.
   *       <ul>
   *         <li>You can use {@link Url#setConnectTimeout} to specify connection timeout, time unit
   *             is milliseconds.
   *         <li>You can use {@link Url#setConnNum} to specify connection number for each ip and
   *             port.
   *         <li>You can use {@link Url#setConnWarmup} to specify whether need warmup all
   *             connections for the first time you call this method.
   *       </ul>
   ...
   */
  RpcResponseFuture invokeWithFuture(
      final String address, final Object request, final int timeoutMillis)
      throws RemotingException, InterruptedException;
   ...
   * @return RpcResponseFuture
   */
  RpcResponseFuture invokeWithFuture(final Connection conn, final Object request, int timeoutMillis)
      throws RemotingException;
  ...
  /**
   * Get a connection using address, address format example -
   * 127.0.0.1:12200?key1=value1&key2=value2 <br>
   *
   * <p>Notice:<br>
   *
   ...
   *
   * @param address target address
   * @return true if and only if there is a connection, and the connection is active and
   *     writable;else return false
   */
  boolean checkConnection(String address);

  /**
   * check connection, the address format example - 127.0.0.1:12200?key1=value1&key2=value2 and if
   * there is no connection, try create new one
   *
   * @param address target address
  ...
  /**
   * check connection, the address format example - 127.0.0.1:12200?key1=value1&key2=value2 and if
   * there is no connection, try create new one in asynchronous
   *
   * @param address target address
   * @param createIfAbsent if need to create
   ...
   *
   * <p>Notice: this method takes no effect on a stand alone connection.
   *
   * @param address target address
   */
  ...
  /**
   * Disable heart beat for a certain connection. If this {@link Url} not connected, then do
   * nothing.
   *
   * <p>Notice: this method takes no effect on a stand alone connection.
   *
   ...
   *
   * <p>Notice: This api should be called before {@link RpcClient#init()}
   */
