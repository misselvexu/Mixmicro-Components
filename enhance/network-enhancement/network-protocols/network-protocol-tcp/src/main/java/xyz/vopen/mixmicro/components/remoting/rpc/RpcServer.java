...
import xyz.vopen.mixmicro.components.remoting.codec.Codec;
import xyz.vopen.mixmicro.components.remoting.config.ConfigManager;
import xyz.vopen.mixmicro.components.remoting.config.RemotingGenericOption;
import xyz.vopen.mixmicro.components.remoting.config.switches.GlobalSwitch;
import xyz.vopen.mixmicro.components.remoting.constant.Constants;
...
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.UserProcessorRegisterHelper;
import xyz.vopen.mixmicro.components.remoting.util.IoUtils;
import xyz.vopen.mixmicro.components.remoting.util.NettyEventLoopUtil;
 ...
 * Server for Rpc.
 *
 * <p>Usage: You can initialize RpcServer with one of the three constructors: {@link
 * #RpcServer(int)}, {@link #RpcServer(int, boolean)}, {@link #RpcServer(int, boolean, boolean)}
 * Then call start() to start a rpc server, and call stop() to stop a rpc server.
 *
 * <p>Notice: Once rpc server has been stopped, it can never be start again. You should init another
 * instance of RpcServer to use.
 *
...
public class RpcServer extends AbstractRemotingServer {
  ...
  private final EventLoopGroup bossGroup =
      NettyEventLoopUtil.newEventLoopGroup(
          1, new NamedThreadFactory("Rpc-netty-server-boss", false));
   ...
   * the random port will determined after server startup Note:<br>
   * You can only use invoke methods with params {@link Connection}, for example {@link
   * #invokeSync(Connection, Object, int)} <br>
   ...
   *
   * <ul>
   *   <li>You can enable connection management feature by specify @param manageConnection true.
   *       <ul>
   *         <li>When connection management feature enabled, you can use all invoke methods with
   *             params {@link String}, {@link Url}, {@link Connection} methods.
   *         <li>When connection management feature disabled, you can only use invoke methods with
   *             params {@link Connection}, otherwise {@link UnsupportedOperationException} will be
   *             thrown.
   *       </ul>
   * </ul>
   *
  ...
  /**
   * Construct a rpc server. <br>
   * You can construct a rpc server with synchronous or asynchronous stop strategy by {@param
   * syncStop}.
   *
   * @param port listened port
  ...
  protected void doInit() {
    ...
    if (this.switches().isOn(GlobalSwitch.SERVER_MANAGE_CONNECTION_SWITCH)) {
      // in server side, do not care the connection service state, so use null instead of global
      // switch
    ...
    Integer tcpSoSndBuf = option(RemotingGenericOption.TCP_SO_SNDBUF);
    Integer tcpSoRcvBuf = option(RemotingGenericOption.TCP_SO_RCVBUF);
    ...
    final boolean flushConsolidationSwitch =
        switches().isOn(GlobalSwitch.CODEC_FLUSH_CONSOLIDATION);
    ...
    this.bootstrap.childHandler(
        new ChannelInitializer<SocketChannel>() {
          ...
          protected void initChannel(SocketChannel channel) {
            ...
            if (idleSwitch) {
              pipeline.addLast(
                  "idleStateHandler", new IdleStateHandler(0, 0, idleTime, TimeUnit.MILLISECONDS));
           ...
           *
           * <ul>
           *   <li>If flag manageConnection be true, use {@link DefaultConnectionManager} to add a
           *       new connection, meanwhile bind it with the channel.
           *   <li>If flag manageConnection be false, just create a new connection and bind it with
           *       the channel.
           * </ul>
           */
  ...
  private SslContext initSSLContext() {
    ...
    try {
      ...
      KeyManagerFactory kmf =
          KeyManagerFactory.getInstance(RpcConfigManager.server_ssl_kmf_algorithm());
      kmf.init(ks, passChs);
  ...
  /**
   * Notice: only {@link GlobalSwitch#SERVER_MANAGE_CONNECTION_SWITCH} switch on, will close all
   * connections.
   *
  ...
  protected void initRpcRemoting() {
    this.rpcRemoting =
        new RpcServerRemoting(new RpcCommandFactory(), this.addressParser, this.connectionManager);
  }

  /** @see RemotingServer#registerProcessor(byte, CommandCode, RemotingProcessor) */
  @Override
  public void registerProcessor(
      byte protocolCode, CommandCode cmd, RemotingProcessor<?> processor) {
   ...
   */
  public void addConnectionEventProcessor(
      ConnectionEventType type, ConnectionEventProcessor processor) {
    this.connectionEventListener.addConnectionEventProcessor(type, processor);
  }

  /**
   * Use UserProcessorRegisterHelper{@link UserProcessorRegisterHelper} to help register user
   * processor for server side.
   *
   ...
   * 127.0.0.1:12200?key1=value1&key2=value2 <br>
   *
   * <p>Notice:<br>
   *
   * <ol>
   *   <li><b>DO NOT modify the request object concurrently when this method is called.</b>
   *   <li>When do invocation, use the string address to find a available client connection, if none
   *       then throw exception
   *   <li>Unlike rpc client, address arguments takes no effect here, for rpc server will not create
   *       connection.
   * </ol>
   *
  ...
  /**
   * One way invocation with a {@link InvokeContext}, common api notice please see {@link
   ...
   * <ol>
   *   <li><b>DO NOT modify the request object concurrently when this method is called.</b>
   *   <li>When do invocation, use the parsed {@link Url} to find a available client connection, if
   *       none then throw exception
   * </ol>
   *
   * @param url url
   * @param request request request
  ...
  /**
   * Synchronous invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeSync(String, Object, int)}
   *
   * @param addr address addr
   * @param request request request
   * @param invokeContext invokeContext invokeContext
   ...
   * @throws InterruptedException interrupted exception
   */
  public Object invokeSync(Url url, Object request, int timeoutMillis)
      throws RemotingException, InterruptedException {
    ensureStarted();
    check();
  ...
  /**
   * Synchronous invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeSync(Connection, Object, int)}
   *
   * @param conn connection connection
   * @param request request request
   * @param invokeContext invokeContext invokeContext
  ...
  public RpcResponseFuture invokeWithFuture(
      final String addr, final Object request, final int timeoutMillis)
      throws RemotingException, InterruptedException {
    ensureStarted();
    check();
  ...
  /**
   * Future invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeWithFuture(String, Object, int)}
   *
   * @param addr address address
   * @param request request request
   * @param invokeContext invokeContext invokeContext
   ...
   */
  public RpcResponseFuture invokeWithFuture(
      final Url url, final Object request, final int timeoutMillis)
      throws RemotingException, InterruptedException {
    ensureStarted();
    check();
  ...
  /**
   * Future invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeWithFuture(Url, Object, int)}
   *
   * @param url url
   * @param request request request
   * @param invokeContext invokeContext
   ...
   */
  public RpcResponseFuture invokeWithFuture(
      final Connection conn, final Object request, final int timeoutMillis)
      throws RemotingException {
  ...
  /**
   * Future invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeWithFuture(Connection, Object, int)}
   *
   * @param conn connection connection
   * @param request request request
   * @param invokeContext invokeContext invokeContext
   ...
   * #invokeWithCallback(String, Object, InvokeCallback, int)}
   *
   * @param addr address address
   * @param request request request
   * @param invokeContext invokeContext
  ...
  public void invokeWithCallback(
    ...
    ensureStarted();
    check();
    this.rpcRemoting.invokeWithCallback(
  ...
  /**
   * Callback invocation with a {@link InvokeContext}, common api notice please see {@link
   * #invokeWithCallback(Url, Object, InvokeCallback, int)}
   *
   * @param url url
   * @param request request request
   * @param invokeContext invokeContext invokeContext
