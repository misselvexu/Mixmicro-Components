...
public class ConnectionEventHandler extends ChannelDuplexHandler {
  private static final Logger logger = RemotingLoggerFactory.getLogger("ConnectionEvent");
  ...
  public void connect(
    ...
    if (logger.isInfoEnabled()) {
      final String local =
          localAddress == null ? null : RemotingUtil.parseSocketAddressToString(localAddress);
      final String remote =
          remoteAddress == null
              ? "UNKNOWN"
  ...
  public void userEventTriggered(ChannelHandlerContext ctx, Object event) throws Exception {
    if (event instanceof ConnectionEventType) {
      ...
      logger.info(
          "trigger user event, local[{}], remote[{}], event: {}",
          localAddress,
          remoteAddress,
          eventType.name());
  ...
  private void onEvent(
      final Connection conn, final String remoteAddress, final ConnectionEventType type) {
    if (this.eventListener != null) {
      this.eventExecutor.onEvent(
          new Runnable() {
            @Override
            public void run() {
  ...
  public class ConnectionEventExecutor {
    Logger logger = RemotingLoggerFactory.getLogger("CommonDefault");
