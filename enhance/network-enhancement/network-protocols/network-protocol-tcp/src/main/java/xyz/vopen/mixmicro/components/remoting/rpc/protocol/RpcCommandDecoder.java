...
public class RpcCommandDecoder implements CommandDecoder {
  ...
  {
    lessLen =
        RpcProtocol.getResponseHeaderLength() < RpcProtocol.getRequestHeaderLength()
            ? RpcProtocol.getResponseHeaderLength()
            : RpcProtocol.getRequestHeaderLength();
  }

  /**
   * @see CommandDecoder#decode(io.netty.channel.ChannelHandlerContext, io.netty.buffer.ByteBuf,
   *     java.util.List)
   */
  @Override
