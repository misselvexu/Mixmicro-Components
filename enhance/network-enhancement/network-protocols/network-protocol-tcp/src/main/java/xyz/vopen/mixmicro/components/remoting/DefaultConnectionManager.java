...
import xyz.vopen.mixmicro.components.remoting.connection.ConnectionFactory;
 ...
 */
public class DefaultConnectionManager extends AbstractLifeCycle
    implements ConnectionManager, ConnectionHeartbeatManager, Scannable, LifeCycle {
   ...
   */
  public DefaultConnectionManager(
      ConnectionSelectStrategy connectionSelectStrategy, ConnectionFactory connectionFactory) {
  ...
  public void startup() throws LifeCycleException {
    ...
    this.asyncCreateConnectionExecutor =
  ...
  public void add(Connection connection, String poolKey) {
    ...
    try {
      ...
      logger.error(
          "[NOTIFYME] Exception occurred when getOrCreateIfAbsent an empty ConnectionPool!", e);
    }
    if (pool != null) {
      pool.add(connection);
    } else {
  ...
  public void remove(Connection connection, String poolKey) {
    ...
    if (null == pool) {
      ...
      if (pool.isEmpty()) {
        this.removeTask(poolKey);
        logger.warn(
            "Remove and close the last connection in ConnectionPool with poolKey {}", poolKey);
      } else {
        logger.warn(
  ...
  /**
   * Warning! This is weakly consistent implementation, to prevent lock the whole {@link
   * ConcurrentHashMap}.
   */
  @Deprecated
  @Override
  ...
  public void check(Connection connection) throws RemotingException {
    ...
    if (!connection.getChannel().isWritable()) {
      ...
      throw new RemotingException(
          "Check connection failed for address: "
              + connection.getUrl()
  ...
  public void scan() {
    if (null != this.connTasks && !this.connTasks.isEmpty()) {
      ...
      while (iter.hasNext()) {
        ...
        if (task == null) {
          logger.info("task(poolKey={}) is null, do not scan the connection pool", poolKey);
          continue;
        }

        if (!task.isDone()) {
          logger.info("task(poolKey={}) is not done, do not scan the connection pool", poolKey);
          continue;
        }
  ...
  public Connection getAndCreateIfAbsent(Url url) throws InterruptedException, RemotingException {
    ...
    ConnectionPool pool =
        this.getConnectionPoolAndCreateIfAbsent(url.getUniqueKey(), new ConnectionPoolCall(url));
    if (null != pool) {
      return pool.get();
    } else {
  ...
  /**
   * If no task cached, create one and initialize the connections. If task cached, check whether the
   * number of connections adequate, if not then heal it.
   *
  ...
  /**
   * Get the mapping instance of {@link ConnectionPool} with the specified poolKey, or create one if
   * there is none mapping in connTasks.
   *
   ...
   * @throws InterruptedException
   */
  private ConnectionPool getConnectionPoolAndCreateIfAbsent(
      String poolKey, Callable<ConnectionPool> callable)
    ...
    for (int i = 0; (i < retry) && (pool == null); ++i) {
      ...
      try {
        ...
        if (null == pool) {
          ...
          String errMsg =
              "Get future task result null for poolKey ["
                  + poolKey
                  + "] after ["
                  + (timesOfResultNull + 1)
                  + "] times try.";
    ...
    if (pool.isAsyncCreationDone() && pool.size() < url.getConnNum()) {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "[NOTIFYME] - conn num after heal {}, expected {}, warmup {}",
              numAfterHeal,
              url.getConnNum(),
              url.isConnWarmup());
        }
   ...
   * @param syncCreateNumWhenNotWarmup you can specify this param to ensure at least desired number
   *     of connections available in sync way
   * @throws RemotingException
   */
  private void doCreate(
      ...
      final int syncCreateNumWhenNotWarmup)
      throws RemotingException {
