...
import xyz.vopen.mixmicro.components.remoting.exception.ConnectionClosedException;
...
import xyz.vopen.mixmicro.components.remoting.rpc.exception.InvokeException;
import xyz.vopen.mixmicro.components.remoting.rpc.exception.InvokeSendFailedException;
import xyz.vopen.mixmicro.components.remoting.rpc.exception.InvokeServerBusyException;
import xyz.vopen.mixmicro.components.remoting.rpc.exception.InvokeServerException;
import xyz.vopen.mixmicro.components.remoting.rpc.exception.InvokeTimeoutException;
import xyz.vopen.mixmicro.components.remoting.rpc.protocol.RpcResponseCommand;
...
public class RpcResponseResolver {
  ...
  public static Object resolveResponseObject(ResponseCommand responseCommand, String addr)
    ...
    if (responseCommand.getResponseStatus() == ResponseStatus.SUCCESS) {
      ...
      String msg =
          String.format(
              "Rpc invocation exception: %s, the address is %s, id=%s",
    ...
    if (responseCommand == null) {
      ...
      switch (responseCommand.getResponseStatus()) {
        ...
        case SERVER_THREADPOOL_BUSY:
          msg =
              String.format(
                  "Server thread pool busy! the address is %s, id=%s",
        ...
        case CODEC_EXCEPTION:
          msg =
              String.format(
                  "Codec exception! the address is %s, id=%s", addr, responseCommand.getId());
        ...
        case SERVER_SERIAL_EXCEPTION:
          ...
          e =
              new SerializationException(
                  detailErrMsg(msg, responseCommand), toThrowable(responseCommand), true);
          break;
        ...
        case SERVER_EXCEPTION:
          msg =
              String.format(
                  ...
                  addr, responseCommand.getId());
          e =
              new InvokeServerException(
                  detailErrMsg(msg, responseCommand), toThrowable(responseCommand));
          break;
        default:
          break;
      }
