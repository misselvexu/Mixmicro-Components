...
/**
 * Database table configuration information either supplied by Spring or direct Java wiring or from
 * a {@link DatabaseTable} annotation.
 *
...
public class DatabaseTableConfig<T> {
  ...
  /**
   * Setup a table config associated with the dataClass and field configurations. The table-name
   * will be extracted from the dataClass.
   */
  public DatabaseTableConfig(
      DatabaseType databaseType, Class<T> dataClass, List<DatabaseFieldConfig> fieldConfigs) {
  ...
  public DatabaseTableConfig(
      Class<T> dataClass, String tableName, List<DatabaseFieldConfig> fieldConfigs) {
    this.dataClass = dataClass;
    this.tableName = tableName;
    this.fieldConfigs = fieldConfigs;
  }

  private DatabaseTableConfig(
      DatabaseType databaseType, Class<T> dataClass, String tableName, FieldType[] fieldTypes) {
  ...
  @Deprecated
  public static <T> DatabaseTableConfig<T> fromClass(
  ...
  /**
   * Extract the DatabaseTableConfig for a particular class by looking for class and field
   * annotations. This is used by internal classes to configure a class.
   */
  public static <T> DatabaseTableConfig<T> fromClass(DatabaseType databaseType, Class<T> clazz)
    ...
    return new DatabaseTableConfig<T>(
        databaseType, clazz, tableName, extractFieldTypes(databaseType, clazz, tableName));
  }
  ...
  public static <T> String extractTableName(DatabaseType databaseType, Class<T> clazz) {
    ...
    if (databaseTable != null
        && databaseTable.tableName() != null
        && databaseTable.tableName().length() > 0) {
  ...
  private static <T> FieldType[] extractFieldTypes(
      DatabaseType databaseType, Class<T> clazz, String tableName) throws SQLException {
  ...
  /**
   * Optional setting. This is here so we can control the lowercasing of the table name in the
   * database-type.
   */
  ...
  private FieldType[] convertFieldConfigs(
      DatabaseType databaseType, String tableName, List<DatabaseFieldConfig> fieldConfigs)
      throws SQLException {
    List<FieldType> fieldTypes = new ArrayList<FieldType>();
    for (DatabaseFieldConfig fieldConfig : fieldConfigs) {
      FieldType fieldType = null;
      // walk up the classes until we find the field
      for (Class<?> classWalk = dataClass;
          classWalk != null;
      ...
      if (fieldType == null) {
        throw new SQLException(
            "Could not find declared field with name '"
