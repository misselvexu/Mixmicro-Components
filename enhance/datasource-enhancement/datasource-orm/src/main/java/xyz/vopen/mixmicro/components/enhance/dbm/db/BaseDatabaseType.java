...
import xyz.vopen.mixmicro.components.enhance.dbm.field.*;
...
/**
 * Base class for all of the {@link DatabaseType} classes that provide the per-database type
 * functionality to create tables and build queries.
 *
 * <p>Here's a good page which shows some of the <a href="http://troels.arvin.dk/db/rdbms/"
 * >differences between SQL databases</a>.
 *
...
public abstract class BaseDatabaseType implements DatabaseType {
  ...
  /**
   * Return the name of the driver(s) class associated with this database type. They will be tried
   * in order until one class is found.
   */
  ...
  public boolean loadDriver() {
    for (String className : getDriverClassNames()) {
      ...
      try {
        ...
        logger.warn(
            "Driver class was not found for "
                + getDatabaseName()
  ...
  public void appendColumnArg(
    ...
    // if we have a generated-id then neither the not-null nor the default make sense and cause
    // syntax errors
  ...
  /**
   * Output the SQL type for a Java UUID. This is used to support specific sub-class database types
   * which support the UUID type.
   */
  ...
  protected void appendBigDecimalNumericType(
      StringBuilder sb, FieldType fieldType, int fieldWidth) {
    sb.append("NUMERIC");
  }
  ...
  /**
   * Output the SQL necessary to configure a generated-id column. This may add to the before
   * statements list or additional arguments later.
   *
   * <p>NOTE: Only one of configureGeneratedIdSequence, configureGeneratedId, or configureId will be
   * called.
   */
  protected void configureGeneratedIdSequence(
    ...
    throw new SQLException(
        "GeneratedIdSequence is not supported by database "
            + getDatabaseName()
            + " for field "
            + fieldType);
  }
  ...
  protected void configureGeneratedId(
    ...
    throw new IllegalStateException(
        "GeneratedId is not supported by database "
            + getDatabaseName()
            + " for field "
            + fieldType);
  }
  ...
  /**
   * Output the SQL necessary to configure an id column. This may add to the before statements list
   * or additional arguments later.
   *
   * <p>NOTE: Only one of configureGeneratedIdSequence, configureGeneratedId, or configureId will be
   * called.
   */
  ...
  /**
   * Return true if we should add generated-id SQL in the {@link #addPrimaryKeySql} method at the
   * end. If false then it needs to be done by hand inline.
   */
  ...
  /**
   * If the field can be nullable, do we need to add some sort of NULL SQL for the create table. By
   * default it is a noop. This is necessary because MySQL has a auto default value for the
   * TIMESTAMP type that required a default value otherwise it would stick in the current date
   * automagically.
   */
  ...
  /**
   * Conversion to/from the Boolean Java field as a number because some databases like the
   * true/false.
   */
