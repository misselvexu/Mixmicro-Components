...
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.mapped.MappedQueryForFieldEq;
...
import xyz.vopen.mixmicro.components.enhance.dbm.repository.*;
...
/**
 * Per field information configured from the {@link DatabaseField} annotation and the associated
 * {@link Field} in the class. Use the {@link #createFieldType} static method to instantiate the
 * class.
 *
...
public class FieldType {
  ...
  /**
   * ThreadLocal counters to detect initialization loops. Notice that there is _not_ an initValue()
   * method on purpose. We don't want to create these if we don't have to.
   */
  private static final ThreadLocal<LevelCounters> threadLevelCounters =
      new ThreadLocal<LevelCounters>();
  ...
  private Repository<?, ?> foreignRepository;
  private MappedQueryForFieldEq<?, ?> mappedQueryForForeignField;

  /**
   * You should use {@link FieldType#createFieldType} to instantiate one of these field if you have
   * a {@link Field}.
   */
  public FieldType(
      ...
      if (persisterClass == null || persisterClass == VoidType.class) {
        ...
        try {
          ...
          throw SqlExceptionUtil.create(
              "Could not find getSingleton static method on class " + persisterClass, e);
        }
        Object result;
        try {
          ...
          throw SqlExceptionUtil.create(
              "Could not run getSingleton method on class " + persisterClass,
              e.getTargetException());
        } catch (Exception e) {
          throw SqlExceptionUtil.create(
              "Could not run getSingleton method on class " + persisterClass, e);
        }
        if (result == null) {
    ...
    if (fieldConfig.isForeign()
      ...
      if (clazz != Collection.class && !ForeignCollection.class.isAssignableFrom(clazz)) {
        throw new SQLException(
            "Field class for '"
                + field.getName()
                + "' must be of class "
                + ForeignCollection.class.getSimpleName()
                + " or Collection.");
      }
      Type type = field.getGenericType();
      if (!(type instanceof ParameterizedType)) {
        throw new SQLException(
            "Field class for '" + field.getName() + "' must be a parameterized Collection.");
      }
      ...
      if (byte[].class.isAssignableFrom(clazz)) {
        throw new SQLException(
            "DBM ORM does not know how to store "
                + clazz
                + " for field '"
                + field.getName()
        ...
        throw new IllegalArgumentException(
            "DBM ORM does not know how to store "
                ...
                + " for field "
                + field.getName()
                + ". Use another class or a custom persister.");
      }
    }
    ...
    if (this.isId && (fieldConfig.isForeign() || fieldConfig.isForeignAutoRefresh())) {
      throw new IllegalArgumentException(
          "Id field " + field.getName() + " cannot also be a foreign object");
    }
    if (fieldConfig.isUseGetSet()) {
      ...
      if (!field.isAccessible()) {
        try {
          ...
          throw new IllegalArgumentException(
              "Could not open access to field "
                  + field.getName()
  ...
  public static FieldType createFieldType(
      ConnectionSource connectionSource, String tableName, Field field, Class<?> parentClass)
      throws SQLException {
    return createFieldType(connectionSource.getDatabaseType(), tableName, field, parentClass);
  }

  /**
   * Return An instantiated {@link FieldType} or null if the field does not have a {@link
   * DatabaseField} annotation.
   */
  public static FieldType createFieldType(
      DatabaseType databaseType, String tableName, Field field, Class<?> parentClass)
      throws SQLException {
  ...
  /**
   * Because we go recursive in a lot of situations if we construct Repositorys inside of the FieldType
   * constructor, we have to do this 2nd pass initialization so we can better use the Repository caches.
   *
   * @see BaseRepositoryImpl#initialize()
   */
  public <FT, FID> void configRepositoryInformation(
      ConnectionSource connectionSource, Class<?> parentClass) throws SQLException {
    Class<?> fieldClass = field.getType();
    DatabaseType databaseType = connectionSource.getDatabaseType();
    final FieldType foreignIdField;
    final FieldType foreignRefField;
    final FieldType foreignFieldType;
    final Repository<FT, FID> foreignRepository;
    ...
    if (fieldConfig.isForeignAutoRefresh() || foreignColumnName != null) {
      ...
      if (tableConfig == null) {
        ...
        Repository<FT, FID> castRepository = (Repository<FT, FID>) RepositoryManager.createRepository(connectionSource, fieldClass);
        foreignRepository = castRepository;
        foreignTableInfo = foreignRepository.getTableInfo();
      } else {
        tableConfig.extractFieldTypes(databaseType);
        // NOTE: the cast is necessary for maven
        @SuppressWarnings("unchecked")
        Repository<FT, FID> castRepository = (Repository<FT, FID>) RepositoryManager.createRepository(connectionSource, tableConfig);
        foreignRepository = castRepository;
        foreignTableInfo = castRepository.getTableInfo();
      }
      foreignIdField = foreignTableInfo.getIdField();
      if (foreignIdField == null) {
        throw new IllegalArgumentException(
            "Foreign field " + fieldClass + " does not have id field");
      }
      if (foreignColumnName == null) {
      ...
      MappedQueryForFieldEq<FT, FID> castMappedQueryForForeignField =
          MappedQueryForFieldEq.build(foreignRepository, foreignTableInfo, foreignRefField);
      ...
      if (tableConfig != null) {
        ...
        /*
         * Initially we were only doing this just for BaseRepositoryEnabled.class and isForeignAutoCreate(). But we
         * need it also for foreign fields because the alternative was to use reflection. Chances are if it is
         * foreign we're going to need the Repository in the future anyway so we might as well create it. This also
      ...
      TableInfo<FT, FID> foreignTableInfo = foreignRepository.getTableInfo();
      foreignIdField = foreignTableInfo.getIdField();
      if (foreignIdField == null) {
        throw new IllegalArgumentException(
            "Foreign field " + fieldClass + " does not have id field");
      ...
      if (isForeignAutoCreate() && !foreignIdField.isGeneratedId()) {
        throw new IllegalArgumentException(
            "Field "
                + field.getName()
                + ", if foreignAutoCreate = true then class "
      ...
      @SuppressWarnings("unchecked")
      DatabaseTableConfig<FT> tableConfig =
          (DatabaseTableConfig<FT>) fieldConfig.getForeignTableConfig();
      Repository<FT, FID> foundRepository;
      if (tableConfig == null) {
        Repository<FT, FID> castRepository = RepositoryManager.createRepository(connectionSource, collectionClazz);
        foundRepository = castRepository;
      } else {
        Repository<FT, FID> castRepository = RepositoryManager.createRepository(connectionSource, tableConfig);
        foundRepository = castRepository;
      }
      foreignRepository = foundRepository;
      foreignFieldType = findForeignFieldType(collectionClazz, parentClass, foundRepository);
      foreignIdField = null;
    ...
    // we have to do this because if we have a foreign field then our id type might have gone to an
    // _id primitive
  ...
  /**
   * Return the default value as parsed from the {@link DatabaseFieldConfig#getDefaultValue()} or
  ...
  /**
   * Return whether the field is an id field. It is an id if {@link DatabaseField#id}, {@link
   * DatabaseField#generatedId}, OR {@link DatabaseField#generatedIdSequence} are enabled.
   */
  public boolean isId() {
    return isId;
  }
  ...
  /**
   * Return whether the field is a generated-id field. This is true if {@link
   * DatabaseField#generatedId} OR {@link DatabaseField#generatedIdSequence} are enabled.
   */
  ...
  /**
   * Return whether the field is a generated-id-sequence field. This is true if {@link
   * DatabaseField#generatedIdSequence} is specified OR if {@link DatabaseField#generatedId} is
   * enabled and the {@link DatabaseType#isIdSequenceNeeded} is enabled. If the latter is true then
   * the sequence name will be auto-generated.
   */
  ...
  /**
   * Return the generated-id-sequence associated with the field or null if {@link
  ...
  public void assignField(
    ...
    if (foreignRefField != null && val != null) {
      ...
      // awhitlock: raised as DBMORM issue: bug #122
      Object cachedVal;
      ObjectCache foreignCache = foreignRepository.getObjectCache();
    ...
    if (fieldSetMethod == null) {
      try {
        ...
        if (val == null) {
          throw SqlExceptionUtil.create(
              "Could not assign object '" + val + "' to field " + this, e);
        } else {
          throw SqlExceptionUtil.create(
      ...
      try {
        ...
        throw SqlExceptionUtil.create(
            "Could not call " + fieldSetMethod + " on object with '" + val + "' for " + this, e);
      }
    }
  }

  /** Assign an ID value to this field. */
  public Object assignIdValue(
      ConnectionSource connectionSource, Object data, Number val, ObjectCache objectCache)
      throws SQLException {
  ...
  /**
   * Return the value from the field in the object that is defined by this FieldType. If the field
   * is a foreign object then the ID of the field is returned instead.
   */
  ...
  /**
   * Extract a field from an object and convert to something suitable to be passed to SQL as an
   * argument.
   */
  ...
  public boolean isComparable() throws SQLException {
    ...
    if (dataPersister == null) {
      throw new SQLException(
          "Internal error.  Data-persister is not configured for field.  "
              + "Please post _full_ exception with associated data objects to mailing list: "
              + this);
    } else {
   ...
   * This can return null in certain circumstances.
   *
   * @param parent The parent object that we will set on each item in the collection.
   * @param id The id of the foreign object we will look for. This can be null if we are creating an
   *     empty collection.
   */
  public <FT, FID> BaseForeignCollection<FT, FID> buildForeignCollection(Object parent, FID id)
    ...
    @SuppressWarnings("unchecked")
    Repository<FT, FID> castRepository = (Repository<FT, FID>) foreignRepository;
    if (!fieldConfig.isForeignCollectionEager()) {
      // we know this won't go recursive so no need for the counters
      return new LazyForeignCollection<FT, FID>(
          castRepository,
          parent,
          id,
          foreignFieldType,
    ...
    try {
      return new EagerForeignCollection<FT, FID>(
          castRepository,
          parent,
          id,
          foreignFieldType,
  ...
  public <T> T resultToJava(DatabaseResults results, Map<String, Integer> columnPositions)
    ...
    if (fieldConfig.isForeign()) {
      ...
      if (fieldConfig.isThrowIfNull() && results.wasNull(dbColumnPos)) {
        throw new SQLException(
            "Results value for primitive field '"
                + field.getName()
  ...
  /**
   * Call through to {@link DatabaseFieldConfig#getColumnDefinition()} and {@link
   * DatabaseFieldConfig#getFullColumnDefinition()}.
   */
  ...
  /**
   * Return the value of field in the data argument if it is not the default value for the class. If
   * it is the default then null is returned.
   */
  ...
  /** Pass the foreign data argument to the foreign {@link Repository#create(Object)} method. */
  public <T> int createWithForeignRepository(T foreignData) throws SQLException {
    @SuppressWarnings("unchecked")
    Repository<T, ?> castRepository = (Repository<T, ?>) foreignRepository;
    return castRepository.create(foreignData);
  }
  ...
  public boolean equals(Object arg) {
    ...
    return field.equals(other.field)
        && (parentClass == null
            ? other.parentClass == null
            : parentClass.equals(other.parentClass));
  }
  ...
  private Object createForeignObject(
      ConnectionSource connectionSource, Object val, ObjectCache objectCache) throws SQLException {
    ...
    if (levelCounters.autoRefreshLevel == 0) {
      // if we aren't in an auto-refresh loop then don't _start_ an new loop if auto-refresh is not
      // configured
      if (!fieldConfig.isForeignAutoRefresh()) {
        return createForeignShell(connectionSource, val, objectCache);
      }
    ...
    if (mappedQueryForForeignField == null) {
      @SuppressWarnings("unchecked")
      Repository<Object, Object> castRepository = (Repository<Object, Object>) foreignRepository;
      MappedQueryForFieldEq<Object, Object> castMappedQueryForId =
          MappedQueryForFieldEq.build(castRepository, castRepository.getTableInfo(), foreignIdField);
  ...
  private <FT, FID> FT createForeignShell(
      ConnectionSource connectionSource, Object val, ObjectCache objectCache) throws SQLException {
    @SuppressWarnings("unchecked")
    Repository<FT, FID> castRepository = (Repository<FT, FID>) foreignRepository;
    FT foreignObject = castRepository.createObjectInstance();
  ...
  /**
   * If we have a class Foo with a collection of Bar's then we go through Bar's Repository looking for a
   * Foo field. We need this field to build the query that is able to find all Bar's that have
   * foo_id that matches our id.
   */
  private FieldType findForeignFieldType(
      Class<?> clazz, Class<?> foreignClass, Repository<?, ?> foreignRepository) throws SQLException {
    String foreignColumnName = fieldConfig.getForeignCollectionForeignFieldName();
    for (FieldType fieldType : foreignRepository.getTableInfo().getFieldTypes()) {
      if (fieldType.getType() == foreignClass
          && (foreignColumnName == null
              || fieldType.getField().getName().equals(foreignColumnName))) {
    ...
    sb.append(" for field '")
        ...
        .append("' column-name does not contain a foreign field");
    if (foreignColumnName != null) {
  ...
  /**
   * Configure our data persister and any dependent fields. We have to do this here because both the
   * constructor and {@link #configRepositoryInformation} method can set the data-type.
   */
  private void assignDataType(DatabaseType databaseType, DataPersister dataPersister)
    ...
    if (fieldConfig.isThrowIfNull() && !dataPersister.isPrimitive()) {
      throw new SQLException(
          "Field " + field.getName() + " must be a primitive if set with throwIfNull");
    }
    ...
    if (defaultStr == null) {
      ...
      throw new SQLException(
          "Field '"
              + field.getName()
              + "' cannot be a generatedId and have a default value '"
              + defaultStr
              + "'");
    } else {
