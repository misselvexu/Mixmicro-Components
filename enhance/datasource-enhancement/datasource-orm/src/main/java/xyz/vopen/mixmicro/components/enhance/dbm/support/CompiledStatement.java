...
/**
 * An internal reduction of the SQL PreparedStatement so we can implement its functionality outside
 * of JDBC.
 *
...
public interface CompiledStatement extends Closeable {
  ...
  /**
   * Cancel a currently running query associated with this statement. Support for this is highly
   * architecture and database dependent.
   */
  ...
  /**
   * Set the query timeout in milliseconds. This may or may not be supported by all database types.
   * Although this is in milliseconds, the underlying timeout resolution may be in seconds.
   *
   * <p><b> WARNING: </b> This will stop the query connection but it will _not_ terminate the query
   * if it is already be running by the database.
   */
