...
/**
 * Information about a database table including the associated tableName, class, constructor, and
 * the included fields.
 *
...
public class TableInfo<T, ID> {
  ...
  @Deprecated
  public TableInfo(
      ConnectionSource connectionSource, BaseRepositoryImpl<T, ID> baseRepositoryImpl, Class<T> dataClass)
      throws SQLException {
  ...
  @Deprecated
  public TableInfo(
      DatabaseType databaseType, BaseRepositoryImpl<T, ID> baseRepositoryImpl, DatabaseTableConfig<T> tableConfig)
      throws SQLException {
      ...
      if (fieldType.isId() || fieldType.isGeneratedId() || fieldType.isGeneratedIdSequence()) {
        if (findIdFieldType != null) {
          throw new SQLException(
              "More than 1 idField configured for class "
                  + dataClass
                  + " ("
                  + findIdFieldType
                  + ","
    ...
    for (FieldType fieldType2 : fieldTypes) {
      if (fieldType2.getFieldName().equals(columnName)) {
        throw new IllegalArgumentException(
            "You should use columnName '"
                + fieldType2.getColumnName()
                + "' for table "
                + tableName
                + " instead of fieldName '"
                + fieldType2.getFieldName()
                + "'");
      }
    }
    throw new IllegalArgumentException(
        "Unknown column name '" + columnName + "' in table " + tableName);
  }
  ...
  /**
   * Return true if this table information has a field with this columnName as set by {@link
