...
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.Clause;
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.SetExpression;
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.SetValue;
...
public class UpdateBuilder<T, ID> extends StatementBuilder<T, ID> {
  ...
  public UpdateBuilder(DatabaseType databaseType, TableInfo<T, ID> tableInfo, Repository<T, ID> repository) {
    super(databaseType, tableInfo, repository, StatementType.UPDATE);
  }

  /**
   * Build and return a prepared update that can be used by {@link Repository#update(PreparedUpdate)}
  ...
  /**
   * Add a column to be set to a value for UPDATE statements. This will generate something like
   * columnName = 'value' with the value escaped if necessary.
   */
  public UpdateBuilder<T, ID> updateColumnValue(String columnName, Object value)
      throws SQLException {
  ...
  /**
   * Add a column to be set to a value for UPDATE statements. This will generate something like
   * 'columnName = expression' where the expression is built by the caller.
   *
   * <p>The expression should have any strings escaped using the {@link #escapeValue(String)} or
   * {@link #escapeValue(StringBuilder, String)} methods and should have any column names escaped
   * using the {@link #escapeColumnName(String)} or {@link #escapeColumnName(StringBuilder, String)}
   * methods.
   */
  public UpdateBuilder<T, ID> updateColumnExpression(String columnName, String expression)
      throws SQLException {
    FieldType fieldType = verifyColumnName(columnName);
    if (fieldType.isForeignCollection()) {
      throw new SQLException("Can't update foreign colletion field: " + columnName);
  ...
  /**
   * When you are building the expression for {@link #updateColumnExpression(String, String)}, you
   * may need to escape column names since they may be reserved words to the database. This will
   * help you by adding escape characters around the word.
   */
  ...
  /**
   * Same as {@link #escapeColumnName(StringBuilder, String)} but it will return the escaped string.
   * The StringBuilder method is more efficient since this method creates a StringBuilder
   * internally.
   */
  ...
  /**
   * When you are building the expression for {@link #updateColumnExpression(String, String)}, you
   * may need to escape values since they may be reserved words to the database. Numbers should not
   * be escaped. This will help you by adding escape characters around the word.
   */
  ...
  /**
   * Same as {@link #escapeValue(StringBuilder, String)} but it will return the escaped string.
   * Numbers should not be escaped. The StringBuilder method is more efficient since this method
  ...
  /** A short cut to {@link Repository#update(PreparedUpdate)}. */
  public int update() throws SQLException {
    return repository.update(prepare());
  }
