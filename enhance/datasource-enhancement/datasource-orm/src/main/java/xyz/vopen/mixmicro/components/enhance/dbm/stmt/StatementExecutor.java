...
import xyz.vopen.mixmicro.components.enhance.dbm.misc.TransactionManager;
...
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.mapped.*;
...
/**
 * Executes SQL statements for a particular table in a particular database. Basically a call through
 * to various mapped statement methods.
 *
...
public class StatementExecutor<T, ID> implements GenericRowMapper<String[]> {
  ...
  private final ThreadLocal<Boolean> localIsInBatchMode =
  ...
  public StatementExecutor(DatabaseType databaseType, TableInfo<T, ID> tableInfo, Repository<T, ID> repository) {
  ...
  /**
   * Return the object associated with the id or null if none. This does a SQL {@code SELECT
   * col1,col2,... FROM ... WHERE ... = id} type query.
   */
  public T queryForId(DatabaseConnection databaseConnection, ID id, ObjectCache objectCache)
      throws SQLException {
    if (mappedQueryForId == null) {
      mappedQueryForId = MappedQueryForFieldEq.build(repository, tableInfo, null);
    }
  ...
  public T queryForFirst(
      DatabaseConnection databaseConnection, PreparedStmt<T> preparedStmt, ObjectCache objectCache)
      throws SQLException {
    CompiledStatement compiledStatement =
        preparedStmt.compile(databaseConnection, StatementBuilder.StatementType.SELECT);
    ...
    try {
      ...
      if (results.first()) {
        logger.debug(
            "query-for-first of '{}' returned at least 1 result", preparedStmt.getStatement());
  ...
  /**
   * Return a list of all of the data in the table. Should be used carefully if the table is large.
   * Consider using the {@link Repository#iterator} if this is the case.
   */
  ...
  public long queryForLong(DatabaseConnection databaseConnection, PreparedStmt<T> preparedStmt)
      throws SQLException {
    CompiledStatement compiledStatement =
        preparedStmt.compile(databaseConnection, StatementBuilder.StatementType.SELECT_LONG);
    DatabaseResults results = null;
    try {
  ...
  /**
   * Return a list of all of the data in the table that matches the {@link PreparedStmt}. Should be
   * used carefully if the table is large. Consider using the {@link Repository#iterator} if this is the
   * case.
   */
  public List<T> query(
      ConnectionSource connectionSource, PreparedStmt<T> preparedStmt, ObjectCache objectCache)
      throws SQLException {
    SelectIterator<T, ID> iterator =
        buildIterator(
            /* no repository specified because no removes */ null,
            connectionSource,
            preparedStmt,
            objectCache,
            DatabaseConnection.DEFAULT_RESULT_FLAGS);
    try {
      ...
      logger.debug(
          "query of '{}' returned {} results", preparedStmt.getStatement(), results.size());
      return results;
    } finally {
  ...
  /**
   * Create and return a SelectIterator for the class using the default mapped query for all
   * statement.
   */
  public SelectIterator<T, ID> buildIterator(
      BaseRepositoryImpl<T, ID> classRepository,
      ConnectionSource connectionSource,
      int resultFlags,
      ObjectCache objectCache)
      throws SQLException {
    prepareQueryForAll();
    return buildIterator(classRepository, connectionSource, preparedQueryForAll, objectCache, resultFlags);
  }
  ...
  /**
   * Return a raw row mapper suitable for use with {@link Repository#queryRaw(String, RawRowMapper,
   * String...)}.
   */
  public RawRowMapper<T> getRawRowMapper() {
    if (rawRowMapper == null) {
      rawRowMapper = new RawRowMapperImpl<T, ID>(repository);
    }
    return rawRowMapper;
  }
  ...
  public SelectIterator<T, ID> buildIterator(
    ...
    try {
      compiledStatement =
          preparedStmt.compile(connection, StatementBuilder.StatementType.SELECT, resultFlags);
      SelectIterator<T, ID> iterator =
          new SelectIterator<T, ID>(
  ...
  public GenericRawResults<String[]> queryRaw(
      ConnectionSource connectionSource, String query, String[] arguments, ObjectCache objectCache)
      throws SQLException {
  ...
  public <UO> GenericRawResults<UO> queryRaw(
    ...
    try {
      ...
      RawResultsImpl<UO> rawResults =
          new RawResultsImpl<UO>(
              connectionSource,
              connection,
              query,
              String[].class,
              compiledStatement,
              new UserRawRowMapper<UO>(rowMapper, this),
              objectCache);
      compiledStatement = null;
      connection = null;
      return rawResults;
    ...
    try {
      ...
      RawResultsImpl<UO> rawResults =
          new RawResultsImpl<UO>(
              ...
              String[].class,
              compiledStatement,
              new UserRawRowObjectMapper<UO>(rowMapper, columnTypes),
              objectCache);
      compiledStatement = null;
      connection = null;
      return rawResults;
  ...
  public GenericRawResults<Object[]> queryRaw(
    ...
    try {
      ...
      RawResultsImpl<Object[]> rawResults =
          new RawResultsImpl<Object[]>(
              connectionSource,
              connection,
              query,
              Object[].class,
              compiledStatement,
              new ObjectArrayRowMapper(columnTypes),
              objectCache);
      compiledStatement = null;
      connection = null;
      return rawResults;
    ...
    try {
      ...
      RawResultsImpl<UO> rawResults =
          new RawResultsImpl<UO>(
              ...
              Object[].class,
              compiledStatement,
              new UserDatabaseResultsMapper<UO>(mapper),
              objectCache);
      compiledStatement = null;
      connection = null;
      return rawResults;
  ...
  public int create(DatabaseConnection databaseConnection, T data, ObjectCache objectCache)
    ...
    if (mappedInsert == null) {
      mappedInsert = MappedCreate.build(repository, tableInfo);
    }
    int result = mappedInsert.insert(databaseType, databaseConnection, data, objectCache);
    if (repository != null && !localIsInBatchMode.get()) {
  ...
  public int update(DatabaseConnection databaseConnection, T data, ObjectCache objectCache)
    ...
    if (mappedUpdate == null) {
      mappedUpdate = MappedUpdate.build(repository, tableInfo);
    }
  ...
  public int updateId(
    ...
    if (mappedUpdateId == null) {
      mappedUpdateId = MappedUpdateId.build(repository, tableInfo);
    }
  ...
  public int update(DatabaseConnection databaseConnection, PreparedUpdate<T> preparedUpdate)
      throws SQLException {
    CompiledStatement compiledStatement =
        preparedUpdate.compile(databaseConnection, StatementBuilder.StatementType.UPDATE);
    try {
  ...
  /**
   * Does a query for the object's Id and copies in each of the field values from the database to
   * refresh the data parameter.
   */
  public int refresh(DatabaseConnection databaseConnection, T data, ObjectCache objectCache)
      throws SQLException {
    if (mappedRefresh == null) {
      mappedRefresh = MappedRefresh.build(repository, tableInfo);
    }
  ...
  public int delete(DatabaseConnection databaseConnection, T data, ObjectCache objectCache)
    ...
    if (mappedDelete == null) {
      mappedDelete = MappedDelete.build(repository, tableInfo);
    }
  ...
  public int deleteObjects(
    ...
    int result =
        MappedDeleteCollection.deleteObjects(
            repository, tableInfo, databaseConnection, datas, objectCache);
    if (repository != null && !localIsInBatchMode.get()) {
      repository.notifyChanges();
    }
    return result;
  ...
  public int deleteIds(
    ...
    // have to build this on the fly because the collection has variable number of args
    int result =
        MappedDeleteCollection.deleteIds(repository, tableInfo, databaseConnection, ids, objectCache);
    if (repository != null && !localIsInBatchMode.get()) {
      repository.notifyChanges();
    }
    return result;
  ...
  public int delete(DatabaseConnection databaseConnection, PreparedDelete<T> preparedDelete)
      throws SQLException {
    CompiledStatement compiledStatement =
        preparedDelete.compile(databaseConnection, StatementBuilder.StatementType.DELETE);
    try {
      int result = compiledStatement.runUpdate();
      if (repository != null && !localIsInBatchMode.get()) {
        repository.notifyChanges();
  ...
  private <CT> CT doCallBatchTasks(ConnectionSource connectionSource, Callable<CT> callable)
    ...
    try {
       ...
       * changes. This disables the repository change notification for every batched call.
       */
      ...
      if (repository != null) {
        // only at the end is the Repository notified of changes
        repository.notifyChanges();
      }
    }
  }

  private <CT> CT doCallBatchTasks(
      DatabaseConnection connection, boolean saved, Callable<CT> callable) throws SQLException {
    ...
    try {
      if (connection.isAutoCommitSupported()) {
        if (connection.isAutoCommit()) {
          ...
          logger.debug(
              "disabled auto-commit on table {} before batch tasks", tableInfo.getTableName());
        }
      }
      try {
      ...
      if (resetAutoCommit) {
         ...
         *
         * <p>NOTE: we do _not_ have to do a commit here. According to {@link
         * Connection#setAutoCommit(boolean)}, this will start a transaction when auto-commit is
         * turned off and it will be committed here.
         */
        connection.setAutoCommit(true);
        logger.debug(
            "re-enabled auto-commit on table {} after batch tasks", tableInfo.getTableName());
      }
    }
  }
  ...
  public boolean ifExists(DatabaseConnection connection, ID id) throws SQLException {
    if (ifExistsQuery == null) {
      QueryBuilder<T, ID> qb = new QueryBuilder<T, ID>(databaseType, tableInfo, repository);
      qb.selectRaw("COUNT(*)");
      /*
    ...
    long count =
        connection.queryForLong(ifExistsQuery, new Object[] {idSqlArg}, ifExistsFieldTypes);
  ...
  private void prepareQueryForAll() throws SQLException {
    if (preparedQueryForAll == null) {
      preparedQueryForAll = new QueryBuilder<T, ID>(databaseType, tableInfo, repository).prepare();
    }
  }
    ...
    public UO mapRow(DatabaseResults results) throws SQLException {
      ...
      for (int colC = 0; colC < columnN; colC++) {
        if (colC >= columnTypes.length) {
          ...
          objectResults[colC] =
              columnTypes[colC].getDataPersister().resultToJava(null, results, colC);
        }
      }
