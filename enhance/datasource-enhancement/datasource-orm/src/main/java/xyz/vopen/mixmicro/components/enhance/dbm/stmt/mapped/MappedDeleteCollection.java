...
public class MappedDeleteCollection<T, ID> extends BaseMappedStatement<T, ID> {
  ...
  private MappedDeleteCollection(
   ...
   * the fly because the datas could be variable sized.
   */
  public static <T, ID> int deleteObjects(
    ...
    MappedDeleteCollection<T, ID> deleteCollection =
        MappedDeleteCollection.build(repository, tableInfo, datas.size());
    ...
    return updateRows(
        databaseConnection, tableInfo.getDataClass(), deleteCollection, fieldObjects, objectCache);
  }

  /**
   * Delete all of the objects in the collection. This builds a {@link MappedDeleteCollection} on
   * the fly because the ids could be variable sized.
   */
  public static <T, ID> int deleteIds(
      Repository<T, ID> repository,
      TableInfo<T, ID> tableInfo,
      DatabaseConnection databaseConnection,
      Collection<ID> ids,
      ...
      throws SQLException {
    MappedDeleteCollection<T, ID> deleteCollection =
        MappedDeleteCollection.build(repository, tableInfo, ids.size());
  ...
  private static <T, ID> MappedDeleteCollection<T, ID> build(
      Repository<T, ID> repository, TableInfo<T, ID> tableInfo, int dataSize) throws SQLException {
    FieldType idField = tableInfo.getIdField();
    if (idField == null) {
    ...
    return new MappedDeleteCollection<T, ID>(repository, tableInfo, sb.toString(), argFieldTypes);
  }

  private static <T, ID> int updateRows(
      DatabaseConnection databaseConnection,
      Class<T> clazz,
      MappedDeleteCollection<T, ID> deleteCollection,
      Object[] args,
    ...
    try {
      int rowC =
          databaseConnection.delete(
              deleteCollection.statement, args, deleteCollection.argFieldTypes);
      ...
      logger.debug(
          "delete-collection with statement '{}' and {} args, changed {} rows",
          deleteCollection.statement,
          args.length,
          rowC);
      if (args.length > 0) {
      ...
      throw SqlExceptionUtil.create(
          "Unable to run delete collection stmt: " + deleteCollection.statement, e);
    }
  }

  private static void appendWhereIds(
