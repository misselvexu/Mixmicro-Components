...
/**
 * Class which logs to java.util.log. This will never be chosen by default because it should always
 * in the classpath, but can be injected if someone really wants it.
 *
...
public class JavaUtilLog implements Log {
  ...
  private java.util.logging.Level levelToJavaLevel(Log.Level level) {
    switch (level) {
      case TRACE:
