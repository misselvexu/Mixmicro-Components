...
/**
 * Like {@link SelectArg} but using a {@link ThreadLocal} internally to improve reentrance so that
 * multiple threads can use the same compiled statement.
 *
...
public class ThreadLocalSelectArg extends BaseArgumentHolder {
  ...
  /**
   * Value wrapper so we can differentiate between a null value and no value. Since we need to do
   * this on a per thread basis, this can't be a volatile field or something.
   */
