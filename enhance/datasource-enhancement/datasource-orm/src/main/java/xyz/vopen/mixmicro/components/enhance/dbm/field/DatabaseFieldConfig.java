...
import xyz.vopen.mixmicro.components.enhance.dbm.misc.JavaxPersistenceConfigurer;
...
/**
 * Database field configuration information either supplied by a {@link DatabaseField} annotation or
 * by direct Java or Spring wiring.
 *
...
public class DatabaseFieldConfig {
  ...
  static {
    try {
      ...
      Class<?> clazz = Class.forName("xyz.vopen.mixmicro.components.enhance.dbm.misc.JavaxPersistenceImpl");
      javaxPersistenceConfigurer =
          (JavaxPersistenceConfigurer) clazz.getConstructor().newInstance();
    } catch (Exception e) {
      // no configurer
  ...
  /**
   * Create and return a config converted from a {@link Field} that may have one of the following
   * annotations: {@link DatabaseField}, {@link ForeignCollectionField}, or javax.persistence...
   */
  public static DatabaseFieldConfig fromField(
      DatabaseType databaseType, String tableName, Field field) throws SQLException {
  ...
  public static Method findGetMethod(
      Field field, DatabaseType databaseType, boolean throwExceptions)
    ...
    Method fieldGetMethod =
        findMethodFromNames(
            ...
            true,
            throwExceptions,
            methodFromField(field, "get", databaseType, true),
            methodFromField(field, "get", databaseType, false),
            methodFromField(field, "is", databaseType, true),
            methodFromField(field, "is", databaseType, false));
  ...
  public static Method findSetMethod(
    ...
    Method fieldSetMethod =
        findMethodFromNames(
            ...
            false,
            throwExceptions,
            methodFromField(field, "set", databaseType, true),
            methodFromField(field, "set", databaseType, false));
      ...
      if (throwExceptions) {
        throw new IllegalArgumentException(
            "Return type of set method "
                + fieldSetMethod.getName()
                + " returns "
                + fieldSetMethod.getReturnType()
                + " instead of void");
      } else {
        return null;
      }
  ...
  public static DatabaseFieldConfig fromDatabaseField(
      DatabaseType databaseType, String tableName, Field field, DatabaseField databaseField) {
  ...
  public static Enum<?> findMatchingEnumVal(Field field, String unknownEnumName) {
    ...
    throw new IllegalArgumentException(
        "Unknwown enum unknown name " + unknownEnumName + " for field " + field);
  }

  private static DatabaseFieldConfig fromForeignCollection(
      DatabaseType databaseType, Field field, ForeignCollectionField foreignCollection) {
    DatabaseFieldConfig config = new DatabaseFieldConfig();
    config.fieldName = field.getName();
    ...
    config.foreignCollectionForeignFieldName =
        valueIfNotBlank(foreignCollection.foreignFieldName());
    return config;
  }
  ...
  private static Method findMethodFromNames(
      Field field, boolean isGetMethod, boolean throwExceptions, String... methodNames) {
  ...
  private static String methodFromField(
      Field field, String prefix, DatabaseType databaseType, boolean forceEnglish) {
  ...
  /**
   * For a foreign class which does not use the {@link DatabaseField} annotations, you need to
  ...
  public void postProcess() {
    ...
    if (foreignAutoRefresh
        && maxForeignAutoRefreshLevel == NO_MAX_FOREIGN_AUTO_REFRESH_LEVEL_SPECIFIED) {
