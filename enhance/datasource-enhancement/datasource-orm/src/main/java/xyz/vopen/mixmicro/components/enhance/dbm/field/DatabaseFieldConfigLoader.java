...
/**
 * Database field configuration loader which reads and writes {@link DatabaseFieldConfig} from a
 * text file/stream.
 *
...
public class DatabaseFieldConfigLoader {
  ...
  private static final String FIELD_NAME_MAX_FOREIGN_AUTO_REFRESH_LEVEL =
  ...
  private static final String FIELD_NAME_MAX_EAGER_FOREIGN_COLLECTION_LEVEL_OLD =
      "maxEagerForeignCollectionLevel";
  private static final String FIELD_NAME_MAX_EAGER_FOREIGN_COLLECTION_LEVEL =
      "foreignCollectionMaxEagerLevel";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_COLUMN_NAME =
      "foreignCollectionColumnName";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_ORDER_COLUMN_NAME_OLD =
      "foreignCollectionOrderColumn";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_ORDER_COLUMN_NAME =
      "foreignCollectionOrderColumnName";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_ORDER_ASCENDING =
      "foreignCollectionOrderAscending";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_FOREIGN_FIELD_NAME_OLD =
      "foreignCollectionForeignColumnName";
  private static final String FIELD_NAME_FOREIGN_COLLECTION_FOREIGN_FIELD_NAME =
      "foreignCollectionForeignFieldName";
  ...
  public static void writeConfig(
      BufferedWriter writer, DatabaseFieldConfig config, String tableName) throws IOException {
    ...
    if (config.getDataPersister() != DEFAULT_DATA_PERSISTER) {
      ...
      if (!found) {
        throw new IllegalArgumentException(
            "Unknown data persister field: " + config.getDataPersister());
      }
    }
    ...
    if (config.isCanBeNull() != DatabaseFieldConfig.DEFAULT_CAN_BE_NULL) {
      writer
          .append(FIELD_NAME_CAN_BE_NULL)
          .append('=')
          .append(Boolean.toString(config.isCanBeNull()));
      writer.newLine();
    }
    if (config.isId()) {
    ...
    if (config.getGeneratedIdSequence() != null) {
      writer
          .append(FIELD_NAME_GENERATED_ID_SEQUENCE)
          .append('=')
          .append(config.getGeneratedIdSequence());
      writer.newLine();
    }
    if (config.isForeign()) {
    ...
    if (config.getUnknownEnumValue() != null) {
      /**
       * NOTE: we need to use getDeclaringClass() below. If we use getClass() then for anonymous
       * enum types, we get an extraneous $1 in the class name. Did not know that. See
       * Enum#getDeclaringClass(). See bug #197.
       */
      writer
          .append(FIELD_NAME_UNKNOWN_ENUM_VALUE)
          .append('=')
    ...
    if (config.getMaxForeignAutoRefreshLevel()
        != DatabaseFieldConfig.NO_MAX_FOREIGN_AUTO_REFRESH_LEVEL_SPECIFIED) {
      writer
          .append(FIELD_NAME_MAX_FOREIGN_AUTO_REFRESH_LEVEL)
          .append('=')
    ...
    if (config.getPersisterClass() != DatabaseFieldConfig.DEFAULT_PERSISTER_CLASS) {
      writer
          .append(FIELD_NAME_PERSISTER_CLASS)
          .append('=')
          .append(config.getPersisterClass().getName());
      writer.newLine();
    }
    ...
    if (config.getFullColumnDefinition() != null) {
      writer
          .append(FIELD_NAME_FULL_COLUMN_DEFINITION)
          .append('=')
          .append(config.getFullColumnDefinition());
      writer.newLine();
    }
    ...
    if (config.getForeignCollectionMaxEagerLevel() != DEFAULT_MAX_EAGER_FOREIGN_COLLECTION_LEVEL) {
      writer
          .append(FIELD_NAME_MAX_EAGER_FOREIGN_COLLECTION_LEVEL)
          .append('=')
    ...
    if (config.getForeignCollectionColumnName() != null) {
      writer
          .append(FIELD_NAME_FOREIGN_COLLECTION_COLUMN_NAME)
          .append('=')
    ...
    if (config.getForeignCollectionOrderColumnName() != null) {
      writer
          .append(FIELD_NAME_FOREIGN_COLLECTION_ORDER_COLUMN_NAME)
          .append('=')
          .append(config.getForeignCollectionOrderColumnName());
      writer.newLine();
    }
    if (config.isForeignCollectionOrderAscending()
        != DatabaseFieldConfig.DEFAULT_FOREIGN_COLLECTION_ORDER_ASCENDING) {
      writer
          .append(FIELD_NAME_FOREIGN_COLLECTION_ORDER_ASCENDING)
          .append('=')
    ...
    if (config.getForeignCollectionForeignFieldName() != null) {
      writer
          .append(FIELD_NAME_FOREIGN_COLLECTION_FOREIGN_FIELD_NAME)
          .append('=')
