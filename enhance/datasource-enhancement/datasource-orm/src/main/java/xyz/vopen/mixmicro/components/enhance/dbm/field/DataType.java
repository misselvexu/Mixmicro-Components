...
import xyz.vopen.mixmicro.components.enhance.dbm.field.types.*;
...
public enum DataType {
  ...
  /**
   * Persists the {@link String} Java class as an array of bytes. By default this will use {@link
   * #STRING} so you will need to specify this using {@link DatabaseField#dataType()}.
   */
  ...
  /**
   * Persists the {@link java.util.Date} Java class as long milliseconds since epoch. By default
   * this will use {@link #DATE} so you will need to specify this using {@link
  ...
  /**
   * Persists the {@link java.util.Date} Java class as int seconds since epoch. By default this will
   * use {@link #DATE} so you will need to specify this using {@link DatabaseField#dataType()}.
   *
   * <p>NOTE: This is <i>not</i> the same as the {@link java.sql.Date} class.
   */
  DATE_INTEGER(DateIntegerType.getSingleton()),
  /**
   * Persists the {@link java.util.Date} Java class as a string of a format. By default this will
   * use {@link #DATE} so you will need to specify this using {@link DatabaseField#dataType()}.
   *
   * <p>NOTE: This is <i>not</i> the same as the {@link java.sql.Date} class.
   *
   * <p><b>WARNING:</b> Because of SimpleDateFormat not being reentrant, this has to do some
   * synchronization with every data in/out unfortunately.
   */
  ...
  /**
   * Persists the byte[] array type. Because of some backwards compatibility issues, you will need
   * to specify this using {@link DatabaseField#dataType()}. It won't be detected automatically.
   */
  ...
  /**
   * Persists an unknown Java Object that is serializable. Because of some backwards and forwards
   * compatibility concerns, you will need to specify this using {@link DatabaseField#dataType()}.
   * It won't be detected automatically.
   */
  SERIALIZABLE(SerializableType.getSingleton()),
  /** See {@link #ENUM_NAME} */
  ENUM_STRING(EnumStringType.getSingleton()),
  /**
   * Persists an Enum Java class as its name produced by call @{link {@link Enum#name()}. You can
   * also specify the {@link #ENUM_INTEGER} or {@link #ENUM_TO_STRING} as the type.
   */
  ENUM_NAME(EnumStringType.getSingleton()),
  /**
   * Persists an Enum Java class as its string value produced by call @{link {@link
   * Enum#toString()}. You can also specify the {@link #ENUM_INTEGER} or {@link #ENUM_STRING} as the
   * type.
   */
  ENUM_TO_STRING(EnumToStringType.getSingleton()),
  /**
   * Persists an Enum Java class as its ordinal integer value. You can also specify the {@link
   * #ENUM_STRING} or {@link #ENUM_TO_STRING} as the type.
   */
  ENUM_INTEGER(EnumIntegerType.getSingleton()),
  /** Persists the {@link java.util.UUID} Java class. */
  UUID(UuidType.getSingleton()),
  /**
   * Persists the {@link java.util.UUID} Java class as a native UUID column which is only supported
   * by a couple of database types.
   */
  ...
  /**
   * Persists the org.joda.time.DateTime type as a long integer. This uses reflection since we don't
   * want to add the dependency. Because of this, you have to specify this using {@link
   * DatabaseField#dataType()}. It won't be detected automatically.
   */
  DATE_TIME(DateTimeType.getSingleton()),
  /**
   * Persists the {@link java.sql.Date} Java class.
   *
   * <p>NOTE: If you want to use the {@link java.util.Date} class then use {@link #DATE} which is
   * recommended instead.
   */
  SQL_DATE(SqlDateType.getSingleton()),
  /**
   * Persists the {@link java.sql.Timestamp} Java class. The {@link #DATE} type is recommended
   * instead.
   */
