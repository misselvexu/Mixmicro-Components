...
import xyz.vopen.mixmicro.components.enhance.dbm.support.GeneratedKeyHolder;
...
public class MappedCreate<T, ID> extends BaseMappedStatement<T, ID> {
  ...
  public static <T, ID> MappedCreate<T, ID> build(Repository<T, ID> repository, TableInfo<T, ID> tableInfo) {
    ...
    return new MappedCreate<T, ID>(
        repository, tableInfo, sb.toString(), argFieldTypes, queryNext, versionFieldTypeIndex);
  }
  ...
  private static boolean isFieldCreatable(DatabaseType databaseType, FieldType fieldType) {
    ...
    if (fieldType.isForeignCollection()) {
      ...
      // we need to query for the next value from the sequence and the idField is inserted
      // afterwards
      return true;
  ...
  public int insert(
    ...
    try {
      // implement {@link DatabaseField#foreignAutoCreate()}, need to do this _before_
      // getFieldObjects() below
      if (tableInfo.isForeignAutoCreate()) {
        for (FieldType fieldType : tableInfo.getFieldTypes()) {
          ...
          if (foreignObj != null
              && fieldType.getForeignIdField().isObjectsFieldValueDefault(foreignObj)) {
            fieldType.createWithForeignRepository(foreignObj);
          }
        }
      }
      ...
      if (versionFieldTypeIndex >= 0 && args[versionFieldTypeIndex] == null) {
        ...
        args[versionFieldTypeIndex] =
            versionFieldType.convertJavaFieldToSqlArgValue(versionDefaultValue);
      }
      ...
      try {
        ...
        logger.debug(
            "insert data with statement '{}' and {} args, threw exception: {}",
            statement,
            args.length,
            e);
        if (args.length > 0) {
      ...
      logger.debug(
          "insert data with statement '{}' and {} args, changed {} rows",
          statement,
          args.length,
          rowC);
      if (args.length > 0) {
      ...
      if (rowC > 0) {
        if (versionDefaultValue != null) {
          argFieldTypes[versionFieldTypeIndex].assignField(
              connectionSource, data, versionDefaultValue, false, null);
        }
        if (keyHolder != null) {
         ...
         */
        if (objectCache != null
            && foreignCollectionsAreAssigned(tableInfo.getForeignCollections(), data)) {
      ...
      throw SqlExceptionUtil.create(
          "Unable to run insert stmt on object " + data + ": " + statement, e);
    }
  }
  ...
  private void assignSequenceId(
      DatabaseConnection databaseConnection, T data, ObjectCache objectCache) throws SQLException {
