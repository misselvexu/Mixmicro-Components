...
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.ColumnNameOrRawSql;
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.OrderBy;
...
/**
 * Assists in building sql query (SELECT) statements for a particular table in a particular
 * database.
 *
 * <p>Here is a <a href="http://www.w3schools.com/Sql/" >good tutorial of SQL commands</a>.
 *
...
public class QueryBuilder<T, ID> extends StatementBuilder<T, ID> {
  ...
  public QueryBuilder(DatabaseType databaseType, TableInfo<T, ID> tableInfo, Repository<T, ID> repository) {
    super(databaseType, tableInfo, repository, StatementType.SELECT);
    this.idField = tableInfo.getIdField();
    this.selectIdColumn = (idField != null);
  }

  /**
   * This is used by the internal call structure to note when a query builder is being used as an
   * inner query. This is necessary because by default, we add in the ID column on every query. When
   * you are returning a data item, its ID field _must_ be set otherwise you can't do a refresh() or
   * update(). But internal queries must have 1 select column set so we can't add the ID.
   */
  ...
  /**
   * Build and return a prepared query that can be used by {@link Repository#query(PreparedQuery)} or
   * {@link Repository#iterator(PreparedQuery)} methods. If you change the where or make other calls you
   * will need to re-call this method to re-prepare the statement for execution.
   */
  ...
  /**
   * Add columns to be returned by the SELECT query. If no columns are selected then all columns are
   * returned by default. For classes with id columns, the id column is added to the select list
   * automagically. This can be called multiple times to add more columns to select.
   *
   * <p><b>WARNING:</b> If you specify any columns to return, then any foreign-collection fields
   * will be returned as null <i>unless</i> their {@link ForeignCollectionField#columnName()} is
   * also in the list.
   */
  ...
  /**
   * Same as {@link #selectColumns(String...)} except the columns are specified as an iterable --
   * probably will be a {@link Collection}. This can be called multiple times to add more columns to
   * select.
   */
  ...
  /**
   * Add raw columns or aggregate functions (COUNT, MAX, ...) to the query. This will turn the query
   * into something only suitable for the {@link Repository#queryRaw(String, String...)} type of statement.
  ...
  /**
   * Add "GROUP BY" clause to the SQL query statement. This can be called multiple times to add
   * additional "GROUP BY" clauses.
   *
   * <p>NOTE: Use of this means that the resulting objects may not have a valid ID column value so
  ...
  /**
   * Add a raw SQL "GROUP BY" clause to the SQL query statement. This should not include the "GROUP
   * BY".
   */
  ...
  /**
   * Add "ORDER BY" clause to the SQL query statement. This can be called multiple times to add
   * additional "ORDER BY" clauses. Ones earlier are applied first.
   */
   ...
   *
   * @param rawSql The raw SQL order by clause. This should not include the "ORDER BY".
   * @param args Optional arguments that correspond to any ? specified in the rawSql. Each of the
   *     arguments must have the sql-type set.
   */
  ...
  /**
   * Start the output at this row number. Set to null for no offset (the default). If you are paging
   * through a table, you should consider using the {@link Repository#iterator()} method instead which
   * handles paging with a database cursor. Otherwise, if you are paging you probably want to
   * specify a {@link #orderBy(String, boolean)}.
   *
   * <p><b>NOTE:</b> This is not supported for all databases. Also, for some databases, the limit
   * _must_ also be specified since the offset is an argument of the limit.
   */
  ...
  /**
   * Set whether or not we should only return the count of the results. This query can then be used
   * by {@link Repository#countOf(PreparedQuery)}.
   *
   * <p>To get the count-of directly, use {@link #countOf()}.
   */
  ...
  /**
   * Set the field that we are counting from the database. For example, you can {@code
   * qb.setCountOf("DISTINCT(fieldname)")}. This query can then be used by {@link
   * Repository#countOf(PreparedQuery)}.
   *
   * <p>To get the count-of directly, use {@link #countOf(String)}.
   */
  ...
  /**
   * Add raw SQL "HAVING" clause to the SQL query statement. This should not include the "HAVING"
   * string.
   */
  ...
  /**
   * Join with another query builder. This will add into the SQL something close to " INNER JOIN
   * other-table ...". Either the object associated with the current QueryBuilder or the argument
   * QueryBuilder must have a foreign field of the other one. An exception will be thrown otherwise.
   *
   * <p><b>NOTE:</b> This will do combine the WHERE statement of the two query builders with a SQL
   * "AND". See {@link #joinOr(QueryBuilder)}.
   */
  ...
  /**
   * Like {@link #join(QueryBuilder)} but allows you to specify the join type and the operation used
   * to combine the WHERE statements.
   */
  public QueryBuilder<T, ID> join(
      QueryBuilder<?, ?> joinedQueryBuilder, JoinType type, JoinWhereOperation operation)
      throws SQLException {
  ...
  /**
   * Like {@link #join(QueryBuilder)} but this combines the WHERE statements of two query builders
   * with a SQL "OR".
   */
   ...
   *
   * <p>See: <a href="http://www.w3schools.com/sql/sql_join_left.asp" >LEFT JOIN SQL docs</a>
   *
   * <p><b>NOTE:</b> RIGHT and FULL JOIN SQL commands are not supported because we are only
   * returning objects from the "left" table.
   *
   * <p><b>NOTE:</b> This will do combine the WHERE statement of the two query builders with a SQL
   * "AND". See {@link #leftJoinOr(QueryBuilder)}.
   */
  ...
  /**
   * Like {@link #leftJoin(QueryBuilder)} but this combines the WHERE statements of two query
   * builders with a SQL "OR".
   */
  ...
  /**
   * Similar to {@link #join(QueryBuilder)} but this allows you to link two tables that share a
   * field of the same type. So even if there is _not_ a foreign-object relationship between the
   * tables, you can JOIN them. This will add into the SQL something close to " INNER JOIN
   * other-table ...".
   */
  public QueryBuilder<T, ID> join(
      String localColumnName, String joinedColumnName, QueryBuilder<?, ?> joinedQueryBuilder)
      throws SQLException {
    addJoinInfo(
        JoinType.INNER,
        localColumnName,
  ...
  /**
   * Similar to {@link #join(QueryBuilder, JoinType, JoinWhereOperation)} but this allows you to
   * link two tables that share a field of the same type.
   */
  public QueryBuilder<T, ID> join(
  ...
  /** A short cut to {@link Repository#query(PreparedQuery)}. */
  public List<T> query() throws SQLException {
    return repository.query(prepare());
  }

  /** A short cut to {@link Repository#queryRaw(String, String...)}. */
  public GenericRawResults<String[]> queryRaw() throws SQLException {
    return repository.queryRaw(prepareStatementString());
  }

  /** A short cut to {@link Repository#queryForFirst(PreparedQuery)}. */
  public T queryForFirst() throws SQLException {
    return repository.queryForFirst(prepare());
  }

  /**
   * A short cut to {@link Repository#queryRaw(String, String...)} and {@link
   * GenericRawResults#getFirstResult()}.
   */
  public String[] queryRawFirst() throws SQLException {
    return repository.queryRaw(prepareStatementString()).getFirstResult();
  }

  /** A short cut to {@link Repository#iterator(PreparedQuery)}. */
  public CloseableIterator<T> iterator() throws SQLException {
    return repository.iterator(prepare());
  }

  /**
   * Returns the count of the number of rows in the table. This uses {@link #setCountOf(boolean)} to
   * true and then calls {@link Repository#countOf(PreparedQuery)}. It restores the previous count-of value
   * before returning.
   */
  public long countOf() throws SQLException {
    ...
    try {
      setCountOf(true);
      return repository.countOf(prepare());
    } finally {
  ...
  /**
   * Returns the count of the number of rows that match a field so you can do {@code
   * qb.countOf("DISTINCT(fieldname)")}. This uses {@link #setCountOf(String)} and then calls {@link
   * Repository#countOf(PreparedQuery)}. It restores the previous count-of value before returning.
   */
  ...
  /**
   * Add an alias for this table. In FROM clause, the table will use AS to define the alias, and
   * qualified fields will be qualified using the alias instead of table name.
   */
  ...
  @Override
  protected boolean appendWhereStatement(
      StringBuilder sb, List<ArgumentHolder> argList, WhereOperation operation)
      throws SQLException {
  ...
  /**
   * Match up our joined fields so we can throw a nice exception immediately if you can't join with
   * this type.
   */
  private void matchJoinedFieldsByName(
      JoinInfo joinInfo,
      String localColumnName,
      String joinedColumnName,
      QueryBuilder<?, ?> joinedQueryBuilder)
    ...
    if (joinInfo.remoteField == null) {
      throw new SQLException(
          "Could not find field in "
              + joinedQueryBuilder.tableInfo.getDataClass()
              + " that has column-name '"
              + joinedColumnName
              + "'");
    }
  ...
  private void matchJoinedFields(JoinInfo joinInfo, QueryBuilder<?, ?> joinedQueryBuilder)
    ...
    for (FieldType fieldType : tableInfo.getFieldTypes()) {
      ...
      if (fieldType.isForeign()
          && foreignRefField.equals(joinedQueryBuilder.tableInfo.getIdField())) {
  ...
  private void appendFieldColumnName(
      StringBuilder sb, FieldType fieldType, List<FieldType> fieldTypeList) {
  ...
  private void appendOffset(StringBuilder sb) throws SQLException {
    ...
    if (databaseType.isOffsetLimitArgument()) {
      if (limit == null) {
        throw new SQLException(
            "If the offset is specified, limit must also be specified with this database");
      }
    } else {
  ...
  private boolean maybeAppendGroupBys(StringBuilder sb, boolean first) {
    ...
    // NOTE: this doesn't seem to work with some database types but we will check for sub-queries
    // anyway
    if (joinList != null) {
      for (JoinInfo joinInfo : joinList) {
  ...
  private boolean maybeAppendOrderBys(
      StringBuilder sb, List<ArgumentHolder> argList, boolean first) {
  ...
  public enum JoinType {
    /**
     * The most common type of join. "An SQL INNER JOIN return all rows from multiple tables where
     * the join condition is met."
     *
     * <p>See <a href="http://www.w3schools.com/sql/sql_join.asp" >SQL JOIN</a>
     */
    INNER("INNER"),
    /**
  ...
  /**
   * When we are combining WHERE statements from the two joined query-builders, this determines the
   * operator to use to do so.
   */
  ...
  /**
   * Internal class used to expose methods to internal classes but through a wrapper instead of a
   * builder.
   */
