 ...
 *
 * <p><b>NOTE:</b> For transactions to work, the database being used must support the functionality.
 *
 ...
 *
 * <p>You can call this as an instance with a new TransactionManager(dataSource); or you can call it
 * as a static like the below example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>For Spring wiring of a Transaction Manager bean, we would do something like the following:
 *
 * <pre>
 * &lt;bean id="transactionManager" class="TransactionManager" init-method="initialize"&gt;
 * 	&lt;property name="dataSource" ref="dataSource" /&gt;
 * &lt;/bean&gt;
 * </pre>
 *
 * <p>WARNING: For most of the methods in this class, it is up to you to properly synchronize them
 * if multiple threads are using a single database connection -- this includes a connection-source
 * which works gives out a single-connection. The reason why this is necessary is that multiple
 * operations are performed on the connection and race-conditions will exist with multiple threads
 * working on the same connection.
 *
...
public class TransactionManager {
  ...
  private static final String SAVE_POINT_PREFIX = "DBMORM";
  ...
  /**
   * Same as {@link #callInTransaction(Callable)} except as a static method with a connection
   * source.
   *
   * <p>WARNING: it is up to you to properly synchronize around this method if multiple threads are
   * using a connection-source which works gives out a single-connection. The reason why this is
   * necessary is that multiple operations are performed on the connection and race-conditions will
   * exist with multiple threads working on the same connection.
   */
  public static <T> T callInTransaction(
      final ConnectionSource connectionSource, final Callable<T> callable) throws SQLException {
   ...
   */
  public static <T> T callInTransaction(
      String tableName, final ConnectionSource connectionSource, final Callable<T> callable)
      throws SQLException {
  ...
  /**
   * Same as {@link #callInTransaction(Callable)} except as a static method on a connection with
   * database-type.
   *
   * <p>WARNING: it is up to you to properly synchronize around this method if multiple threads are
   * using the same database connection or if your connection-source is single-connection. The
   * reason why this is necessary is that multiple operations are performed on the connection and
   * race-conditions will exist with multiple threads working on the same connection.
   */
  public static <T> T callInTransaction(
      ...
      try {
        ...
        if (hasSavePoint) {
          try {
            ...
            logger.error(
                e, "after commit exception, rolling back to save-point also threw exception");
  ...
  private static void release(DatabaseConnection connection, Savepoint savePoint)
      throws SQLException {
    String name = (savePoint == null ? null : savePoint.getSavepointName());
  ...
  /**
   * Execute the {@link Callable} class inside of a transaction. If the callable returns then the
   * transaction is committed. If the callable throws an exception then the transaction is rolled
   * back and a {@link SQLException} is thrown by this method.
   *
   * <p><b> NOTE: </b> If your callable block really doesn't have a return object then use the Void
   * class and return null from the call method.
   *
   * <p>WARNING: it is up to you to properly synchronize around this method if multiple threads are
   * using a connection-source which works gives out a single-connection. The reason why this is
   * necessary is that multiple operations are performed on the connection and race-conditions will
   ...
   * @throws SQLException If the callable threw an exception then the transaction is rolled back and
   *     a SQLException wraps the callable exception and is thrown by this method.
   */
