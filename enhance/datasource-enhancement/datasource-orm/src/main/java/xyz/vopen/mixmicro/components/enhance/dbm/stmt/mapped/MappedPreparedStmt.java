...
public class MappedPreparedStmt<T, ID> extends BaseMappedQuery<T, ID>
  ...
  public MappedPreparedStmt(
      ...
      StatementType type,
      boolean cacheStore) {
    super(repository, tableInfo, statement, argFieldTypes, resultFieldTypes);
  ...
  @Override
  public CompiledStatement compile(DatabaseConnection databaseConnection, StatementType type)
      throws SQLException {
  ...
  public CompiledStatement compile(
      DatabaseConnection databaseConnection, StatementType type, int resultFlags)
      throws SQLException {
    if (this.type != type) {
      throw new SQLException(
          "Could not compile this "
              + this.type
              + " statement since the caller is expecting a "
              + type
              + " statement.  Check your QueryBuilder methods.");
    }
    CompiledStatement stmt =
        databaseConnection.compileStatement(
            statement, type, argFieldTypes, resultFlags, cacheStore);
  ...
  private CompiledStatement assignStatementArguments(CompiledStatement stmt) throws SQLException {
    ...
    try {
      ...
      if (logger.isLevelEnabled(Log.Level.TRACE) && argHolders.length > 0) {
