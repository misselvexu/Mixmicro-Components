...
public class TableUtils {
  ...
  public static <T> int createTable(ConnectionSource connectionSource, Class<T> dataClass)
      throws SQLException {
    Repository<T, ?> repository = RepositoryManager.createRepository(connectionSource, dataClass);
    return doCreateTable(repository, false);
  }

  /**
   * Issue the database statements to create the table associated with a table configuration.
   *
   * @param repository Associated repository.
   * @return The number of statements executed to do so.
   */
  public static int createTable(Repository<?, ?> repository) throws SQLException {
    return doCreateTable(repository, false);
  }
  ...
  public static <T> int createTableIfNotExists(
      ConnectionSource connectionSource, Class<T> dataClass) throws SQLException {
    Repository<T, ?> repository = RepositoryManager.createRepository(connectionSource, dataClass);
    return doCreateTable(repository, true);
  }
   ...
   * @param tableConfig Hand or spring wired table configuration. If null then the class must have
   *     {@link DatabaseField} annotations.
   * @return The number of statements executed to do so.
   */
  public static <T> int createTable(
      ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig) throws SQLException {
    Repository<T, ?> repository = RepositoryManager.createRepository(connectionSource, tableConfig);
    return doCreateTable(repository, false);
  }
  ...
  /**
   * Return an list of SQL statements that need to be run to create a table. To do the work of
   * creating, you should call {@link #createTable}.
   *
   * @param connectionSource Our connect source which is used to get the database type, not to apply
   *     the creates.
   * @param tableConfig Hand or spring wired table configuration. If null then the class must have
   *     {@link DatabaseField} annotations.
  ...
  public static <T, ID> List<String> getCreateTableStatements(
      ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig) throws SQLException {
    Repository<T, ID> repository = RepositoryManager.createRepository(connectionSource, tableConfig);
    DatabaseType databaseType = connectionSource.getDatabaseType();
    if (repository instanceof BaseRepositoryImpl<?, ?>) {
      return addCreateTableStatements(
          databaseType, ((BaseRepositoryImpl<?, ?>) repository).getTableInfo(), false, false);
    } else {
   ...
   * @param dataClass Hand or spring wired table configuration. If null then the class must have
   *     {@link DatabaseField} annotations.
   * @return A list of table create statements.
   */
  public static <T> List<String> getCreateTableStatements(
      DatabaseType databaseType, Class<T> dataClass) throws SQLException {
   ...
   *
   * <p><b>WARNING:</b> This is [obviously] very destructive and is unrecoverable.
   *
   * @param connectionSource Associated connection source.
   * @param dataClass The class for which a table will be dropped.
   * @param ignoreErrors If set to true then try each statement regardless of {@link SQLException}
   *     thrown previously.
   * @return The number of statements executed to do so.
   */
  public static <T, ID> int dropTable(
      ConnectionSource connectionSource, Class<T> dataClass, boolean ignoreErrors)
      throws SQLException {
    Repository<T, ID> repository = RepositoryManager.createRepository(connectionSource, dataClass);
    return dropTable(repository, ignoreErrors);
  }

  /**
   * Issue the database statements to drop the table associated with a repository.
   *
   * @param repository Associated repository.
   * @return The number of statements executed to do so.
   */
  public static <T, ID> int dropTable(Repository<T, ID> repository, boolean ignoreErrors) throws SQLException {
    ConnectionSource connectionSource = repository.getConnectionSource();
    Class<T> dataClass = repository.getDataClass();
    DatabaseType databaseType = connectionSource.getDatabaseType();
    if (repository instanceof BaseRepositoryImpl<?, ?>) {
      return doDropTable(
          databaseType, connectionSource, ((BaseRepositoryImpl<?, ?>) repository).getTableInfo(), ignoreErrors);
    } else {
   ...
   */
  public static <T, ID> int dropTable(
      ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig, boolean ignoreErrors)
      throws SQLException {
    DatabaseType databaseType = connectionSource.getDatabaseType();
    Repository<T, ID> repository = RepositoryManager.createRepository(connectionSource, tableConfig);
    if (repository instanceof BaseRepositoryImpl<?, ?>) {
  ...
  /**
   * Clear all data out of the table. For certain database types and with large sized tables, which
   * may take a long time. In some configurations, it may be faster to drop and re-create the table.
   *
   * <p><b>WARNING:</b> This is [obviously] very destructive and is unrecoverable.
   */
  ...
  private static <T, ID> int doCreateTable(Repository<T, ID> repository, boolean ifNotExists)
      throws SQLException {
    ConnectionSource connectionSource = repository.getConnectionSource();
    DatabaseType databaseType = connectionSource.getDatabaseType();
    if (repository instanceof BaseRepositoryImpl<?, ?>) {
      return doCreateTable(connectionSource, ((BaseRepositoryImpl<?, ?>) repository).getTableInfo(), ifNotExists);
    } else {
      TableInfo<T, ID> tableInfo = new TableInfo<T, ID>(databaseType, repository.getDataClass());
      return doCreateTable(connectionSource, tableInfo, ifNotExists);
    }
  }

  private static <T, ID> int doCreateTable(
      ConnectionSource connectionSource, TableInfo<T, ID> tableInfo, boolean ifNotExists)
      throws SQLException {
    DatabaseType databaseType = connectionSource.getDatabaseType();
    List<String> statements = new ArrayList<String>();
  ...
  private static int doCreateTestQueries(
      DatabaseConnection connection, DatabaseType databaseType, List<String> queriesAfter)
      throws SQLException {
    int stmtC = 0;
  ...
  private static <T, ID> List<String> addCreateTableStatements(
    ...
    addCreateTableStatements(
        databaseType, tableInfo, statements, queriesAfter, ifNotExists, logDetails);
    return statements;
  }

  /**
   * Generate and return the list of statements to create a database table and any associated
   * features.
   */
  private static <T, ID> void addCreateTableStatements(
    ...
    databaseType.addPrimaryKeySql(
        tableInfo.getFieldTypes(), additionalArgs, statementsBefore, statementsAfter, queriesAfter);
    // add any sql that sets any unique fields
    databaseType.addUniqueComboSql(
        tableInfo.getFieldTypes(), additionalArgs, statementsBefore, statementsAfter, queriesAfter);
  ...
  private static <T, ID> void addCreateIndexStatements(
      DatabaseType databaseType,
      TableInfo<T, ID> tableInfo,
      List<String> statements,
      boolean ifNotExists,
    ...
    for (Map.Entry<String, List<String>> indexEntry : indexMap.entrySet()) {
      if (logDetails) {
        logger.info(
            "creating index '{}' for table '{}", indexEntry.getKey(), tableInfo.getTableName());
      }
      sb.append("CREATE ");
      if (unique) {
        sb.append("UNIQUE ");
