...
import xyz.vopen.mixmicro.components.enhance.dbm.stmt.query.*;
...
/**
 * Manages the various clauses that make up the WHERE part of a SQL statement. You get one of these
 * when you call {@link StatementBuilder#where} or you can set the where clause by calling {@link
 * StatementBuilder#setWhere}.
 *
 * <p>Here's a page with a <a href="http://www.w3schools.com/Sql/" >good tutorial of SQL
 * commands</a>.
 *
 * <p>To create a query which looks up an account by name and password you would do the following:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>In this example, the SQL query that will be generated will be approximately:
 *
 * <pre>
 * SELECT * FROM account WHERE (name = 'foo' AND passwd = '_secret')
 * </pre>
 *
 * <p>If you'd rather chain the methods onto one line (like StringBuilder), this can also be written
 * as:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>If you'd rather use parens and the like then you can call:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>All three of the above call formats produce the same SQL. For complex queries that mix ANDs
 * and ORs, the last format will be necessary to get the grouping correct. For example, here's a
 * complex query:
 *
 * <pre>
 * Where where = queryBuilder.where();
...
public class Where<T, ID> {
   ...
   *
   * <p><b>NOTE:</b> There is no guarantee of the order of the clauses that are generated in the
   * final query.
   *
   * <p><b>NOTE:</b> I can't remove the generics code warning that can be associated with this
   * method. You can instead use the {@link #and(int)} method.
   */
  ...
  /**
   * This method needs to be used carefully. This will absorb a number of clauses that were
   * registered previously with calls to {@link Where#eq(String, Object)} or other methods and will
   * string them together with AND's. There is no way to verify the number of previous clauses so
   * the programmer has to count precisely.
   *
   * <p><b>NOTE:</b> There is no guarantee of the order of the clauses that are generated in the
   * final query.
   *
   * <p><b>NOTE:</b> This will throw an exception if numClauses is 0 but will work with 1 or more.
   */
  ...
  /**
   * Add a IN clause which makes sure the column is in one of the columns returned from a sub-query
   * inside of parenthesis. The QueryBuilder must return 1 and only one column which can be set with
   * the {@link QueryBuilder#selectColumns(String...)} method calls. That 1 argument must match the
   * SQL type of the column-name passed to this method.
   *
   * <p><b>NOTE:</b> The sub-query will be prepared at the same time that the outside query is.
   */
  ...
  public Where<T, ID> like(String columnName, Object value) throws SQLException {
    addClause(
        new SimpleComparison(
            columnName, findColumnFieldType(columnName), value, SimpleComparison.LIKE_OPERATION));
    return this;
  }
   ...
   *
   * <p><b>NOTE:</b> I can't remove the generics code warning that can be associated with this
   * method. You can instead use the {@link #or(int)} method.
   */
   ...
   * This method needs to be used carefully. This will absorb a number of clauses that were
   * registered previously with calls to {@link Where#eq(String, Object)} or other methods and will
   * string them together with OR's. There is no way to verify the number of previous clauses so the
   * programmer has to count precisely.
   *
   * <p><b>NOTE:</b> There is no guarantee of the order of the clauses that are generated in the
   * final query.
   *
  ...
  public Where<T, ID> idEq(ID id) throws SQLException {
    ...
    addClause(
        new SimpleComparison(idColumnName, idFieldType, id, SimpleComparison.EQUAL_TO_OPERATION));
    return this;
  }

  /** Add a clause where the ID is from an existing object. */
  public <OD> Where<T, ID> idEq(Repository<OD, ?> dataRepository, OD data) throws SQLException {
    if (idColumnName == null) {
      throw new SQLException("Object has no id column specified");
    }
    addClause(
        new SimpleComparison(
            idColumnName,
            idFieldType,
            dataRepository.extractId(data),
            SimpleComparison.EQUAL_TO_OPERATION));
    return this;
  }

  /**
   * Add a raw statement as part of the where that can be anything that the database supports. Using
   * more structured methods is recommended but this gives more control over the query and allows
   * you to utilize database specific features.
   *
   * @param rawStatement The statement that we should insert into the WHERE.
   * @param args Optional arguments that correspond to any ? specified in the rawStatement. Each of
   *     the arguments must have either the corresponding columnName or the sql-type set.
   *     <b>WARNING,</b> you cannot use the {@code SelectArg("columnName")} constructor since that
   *     sets the _value_, not the name. Use {@code new SelectArg("column-name", null);}.
   */
  ...
  /**
   * Make a comparison where the operator is specified by the caller. It is up to the caller to
   * specify an appropriate operator for the database and that it be formatted correctly.
   */
  public Where<T, ID> rawComparison(String columnName, String rawOperator, Object value)
      throws SQLException {
    addClause(
        new SimpleComparison(columnName, findColumnFieldType(columnName), value, rawOperator));
    return this;
  }
  ...
  private Where<T, ID> in(boolean in, String columnName, QueryBuilder<?, ?> subQueryBuilder)
    ...
    if (subQueryBuilder.getSelectColumnCount() != 1) {
      if (subQueryBuilder.getSelectColumnCount() == 0) {
        ...
        throw new SQLException(
            "Inner query must have only 1 select column specified instead of "
                + subQueryBuilder.getSelectColumnCount()
                + ": "
                + subQueryBuilder.getSelectColumnsAsString());
      }
    }
    // we do this to turn off the automatic addition of the ID column in the select column list
    subQueryBuilder.enableInnerQuery();
    addClause(
        new InSubQuery(
            columnName,
            findColumnFieldType(columnName),
            new QueryBuilder.InternalQueryBuilderWrapper(subQueryBuilder),
            in));
    return this;
  }
