...
/**
 * Cache for DBM ORM which stores objects with a {@link WeakReference} or {@link SoftReference} to
 * them. Java Garbage Collection can then free these objects if no one has a "strong" reference to
 * the object (weak) or if it runs out of memory (soft).
 *
...
public class ReferenceObjectCache implements ObjectCache {
  ...
  /**
   * @param useWeak Set to true if you want the cache to use {@link WeakReference}. If false then
   *     the cache will use {@link SoftReference}.
   */
