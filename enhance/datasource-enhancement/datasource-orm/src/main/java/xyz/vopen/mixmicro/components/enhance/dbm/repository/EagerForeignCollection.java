...
public class EagerForeignCollection<T, ID> extends BaseForeignCollection<T, ID>
  ...
  public EagerForeignCollection(
    ...
    if (parentId == null) {
      ...
      results = repository.query(getPreparedQuery());
    }
  }
      ...
      public void remove() {
        ...
        if (offset >= results.size()) {
          throw new IllegalStateException(
              "current results position (" + offset + ") is out of bounds");
        }
  ...
  public boolean remove(Object data) {
    if (!results.remove(data) || repository == null) {
      return false;
    }
    ...
    T castData = (T) data;
    try {
      return (repository.delete(castData) == 1);
    } catch (SQLException e) {
      throw new IllegalStateException("Could not delete data element from repository", e);
    }
  }
  ...
  public boolean retainAll(Collection<?> collection) {
    // delete from the iterate removes from the eager list and repository
  ...
  public int updateAll() throws SQLException {
    int updatedC = 0;
    for (T data : results) {
      updatedC += repository.update(data);
    }
    return updatedC;
  }
  ...
  public int refreshAll() throws SQLException {
    int updatedC = 0;
    for (T data : results) {
      updatedC += repository.refresh(data);
    }
    return updatedC;
  }
