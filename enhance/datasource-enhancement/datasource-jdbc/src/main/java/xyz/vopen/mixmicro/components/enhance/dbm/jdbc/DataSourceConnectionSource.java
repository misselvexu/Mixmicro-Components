...
/**
 * Wrapper around a {@link DataSource} that supports our ConnectionSource interface. This allows you
 * to wrap other multi-threaded, high-performance data sources, see Apache DBCP, CP30, or BoneCP.
 *
...
public class DataSourceConnectionSource extends BaseConnectionSource implements ConnectionSource {
  ...
  /**
   * Create a data source wrapper for a DataSource. The databaseType is usually determined from the
   * databaseUrl so most users should call {@link #DataSourceConnectionSource(DataSource, String)}
   * instead. If, however, you need to force the class to use a specific DatabaseType then this
    ...
    if (databaseType == null) {
      if (databaseUrl == null) {
        throw new IllegalStateException(
            "either the databaseUri or the databaseType must be set on "
   ...
   * {@link DataSource} (grrrr), this close method does _nothing_. You must close the underlying
  ...
  public void close() throws IOException {
    ...
    // unfortunately, you will need to close the DataSource directly since there is no close on the
    // interface
  }
  ...
  /**
   * Unfortunately we cannot tell if the related data source has been closed so this just returns
   * true.
   */
  @Override
  ...
  /**
   * Return true if there is only one connection to the database. If true then some thread locks may
   * be enabled when using batch tasks and auto-commit.
   *
   * <p>NOTE: to test the data-source to see if it gives out multiple connections, we request two
   * connections to see if they are different. I guess that's the best that we can do.
   */
  @Override
