...
import xyz.vopen.mixmicro.components.enhance.dbm.misc.VersionUtils;
...
public class JdbcDatabaseConnection implements DatabaseConnection {
  ...
  public long queryForLong(String statement, Object[] args, FieldType[] argFieldTypes)
    ...
    Object result =
        queryForOne(statement, args, argFieldTypes, longWrapper, null, "query for long");
    if (result == null) {
  ...
  private int update(String statement, Object[] args, FieldType[] argFieldTypes, String label)
    ...
    try {
      ...
      logger.trace(
          "{} statement is prepared and executed returning {}: {}", label, rowCount, statement);
      return rowCount;
    } finally {
      stmt.close();
    }
  ...
  private Number getIdColumnData(ResultSet resultSet, ResultSetMetaData metaData, int columnIndex)
    ...
    switch (typeVal) {
      ...
      default:
        ...
        throw new SQLException(
            "Unexpected ID column type "
                + TypeValMapper.getSqlTypeForTypeVal(typeVal)
                + " (typeVal "
                + typeVal
                + ") in column "
                + columnName
