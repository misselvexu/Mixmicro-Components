...
import xyz.vopen.mixmicro.components.enhance.dbm.db.DatabaseTypeUtils;
...
import xyz.vopen.mixmicro.components.enhance.dbm.support.BaseConnectionSource;
...
import xyz.vopen.mixmicro.components.enhance.dbm.support.DatabaseConnectionProxyFactory;
...
/**
 * Implementation of the ConnectionSource interface that supports what is needed by DBM ORM. This is
 * not thread-safe nor synchronized and under the covers uses a single database connection. For
...
public class JdbcConnectionSource extends BaseConnectionSource implements ConnectionSource {
  ...
  /**
   * Constructor for Spring type wiring if you are using the set methods. If you are using Spring
   * then your should use: init-method="initialize"
   */
   ...
   * @throws SQLException If the driver associated with the database driver is not found in the
   *     classpath.
   */
  ...
  /**
   * Create a data source for a particular database URL. The databaseType is usually determined from
   * the databaseUrl so most users should call {@link #JdbcConnectionSource(String)} instead. If,
   * however, you need to force the class to use a specific DatabaseType then this constructor
   * should be used.
   *
   * @param url The database URL which should start jdbc:...
  ...
  /**
   * Create a data source for a particular database URL with username and password permissions. The
   * databaseType is usually determined from the databaseUrl so most users should call {@link
   * #JdbcConnectionSource(String, String, String)} instead. If, however, you need to force the
   ...
   */
  public JdbcConnectionSource(
      String url, String username, String password, DatabaseType databaseType) throws SQLException {
    this.url = url;
  ...
  public static void setDatabaseConnectionProxyFactory(
      DatabaseConnectionProxyFactory connectionProxyFactory) {
    JdbcConnectionSource.connectionProxyFactory = connectionProxyFactory;
  }

  /**
   * Initialize the class after the setters have been called. If you are using the no-arg
   * constructor and Spring type wiring, this should be called after all of the set methods.
   *
   * @throws SQLException If the driver associated with the database URL is not found in the
   *     classpath.
   */
   ...
   *
   * @param logger This is here so we can use the right logger associated with the sub-class.
   */
  ...
  protected DatabaseConnection makeConnection(Logger logger) throws SQLException {
    ...
    DatabaseConnection connection =
        new JdbcDatabaseConnection(DriverManager.getConnection(url, properties));
