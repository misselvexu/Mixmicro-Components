...
/**
 * A connection sounds that uses an existing open database connection. This is not thread-safe nor
...
public class JdbcSingleConnectionSource extends JdbcConnectionSource implements ConnectionSource {
   ...
   * @param sqlConnection Already open database connection that we will use.
   ...
   * the databaseUrl so most users should call {@link #JdbcSingleConnectionSource(String,
   * Connection)} instead. If, however, you need to force the class to use a specific DatabaseType
   * then this constructor should be used.
   *
   * @param url The database URL which should start jdbc:...
