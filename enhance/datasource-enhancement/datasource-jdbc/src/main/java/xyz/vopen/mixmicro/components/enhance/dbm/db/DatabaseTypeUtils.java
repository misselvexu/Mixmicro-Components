...
public class DatabaseTypeUtils {
   ...
   *
   * @throws IllegalArgumentException if the url format is not recognized, the database type is
   *     unknown, or the class could not be constructed.
   */
  public static DatabaseType createDatabaseType(String databaseUrl) {
    ...
    throw new IllegalArgumentException(
        "Unknown database-type url part '" + dbTypePart + "' in: " + databaseUrl);
  }

  private static String extractDbType(String databaseUrl) {
    if (!databaseUrl.startsWith("jdbc:")) {
      throw new IllegalArgumentException(
          "Database URL was expected to start with jdbc: but was " + databaseUrl);
    }
