...
/**
 * Implementation of the ConnectionSource interface that supports basic pooled connections. New
 * connections are created on demand only if there are no dormant connections otherwise released
 * connections will be reused. This class is reentrant and can handle requests from multiple
 * threads.
 *
 ...
 *
 * <p><b> NOTE: </b> This class spawns a thread to test the pooled connections that are in the
 * free-list as a keep-alive mechanism. It will test any dormant connections every so often to see
 * if they are still valid. If this is not the behavior that you want then call {@link
 * #setCheckConnectionsEveryMillis(long)} with 0 to disable the thread. You can also call {@link
 * #setTestBeforeGet(boolean)} and set it to true to test the connection before it is handed back to
 * you.
 *
...
public class JdbcPooledConnectionSource extends JdbcConnectionSource implements ConnectionSource {
  ...
  private static final int DEFAULT_MAX_CONNECTION_AGE_MILLIS = 60 * 60 * 1000;
  private static final int CHECK_CONNECTIONS_EVERY_MILLIS = 30 * 1000;
  ...
  public JdbcPooledConnectionSource(String url, String username, String password)
      throws SQLException {
  ...
  /**
   * Set the number of milliseconds that a connection can stay open before being closed. Set to
   * Long.MAX_VALUE to have the connections never expire.
   */
   ...
   * mechanism. This set the number of milliseconds it sleeps between checks -- default is 30000. To
   * disable the checking thread, set this to 0 before you start using the connection source.
   */
