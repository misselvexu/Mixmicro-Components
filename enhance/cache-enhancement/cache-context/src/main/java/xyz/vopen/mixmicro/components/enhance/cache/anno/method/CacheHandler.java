...
public class CacheHandler implements InvocationHandler {
    ...
    private static void doInvalidate(CacheInvokeContext context, CacheInvalidateAnnoConfig annoConfig) {
        ...
        if (annoConfig.isMulti()) {
            ...
            if (it == null) {
                logger.error("mixcache @CacheInvalidate key is not instance of Iterable or array: " + annoConfig.getDefineMethod());
                return;
            }
    ...
    private static void doUpdate(CacheInvokeContext context, CacheUpdateAnnoConfig updateAnnoConfig) {
        ...
        if (updateAnnoConfig.isMulti()) {
            ...
            if (keyIt == null) {
                logger.error("mixcache @CacheUpdate key is not instance of Iterable or array: " + updateAnnoConfig.getDefineMethod());
                return;
            }
            if (valueIt == null) {
                logger.error("mixcache @CacheUpdate value is not instance of Iterable or array: " + updateAnnoConfig.getDefineMethod());
                return;
            }
            ...
            if (keyList.size() != valueList.size()) {
                logger.error("mixcache @CacheUpdate key size not equals with value size: " + updateAnnoConfig.getDefineMethod());
                return;
            } else {
                Map m = new HashMap();
