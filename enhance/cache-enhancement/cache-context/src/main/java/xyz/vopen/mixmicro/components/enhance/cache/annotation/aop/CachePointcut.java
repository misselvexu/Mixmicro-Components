package xyz.vopen.mixmicro.components.enhance.cache.annotation.aop;
...
import xyz.vopen.mixmicro.components.enhance.cache.annotation.method.ClassUtil;
...
public class CachePointcut extends StaticMethodMatcherPointcut implements ClassFilter {
  ...
  public boolean matches(Method method, Class targetClass) {
    ...
    if (b) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "check method match true: method={}, declaringClass={}, targetClass={}",
            method.getName(),
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
            "check method match false: method={}, declaringClass={}, targetClass={}",
            method.getName(),
            ClassUtil.getShortClassName(method.getDeclaringClass().getName()),
            targetClass == null ? null : ClassUtil.getShortClassName(targetClass.getName()));
      }
  ...
  private boolean matchesImpl(Method method, Class targetClass) {
    ...
    if (cac == CacheInvokeConfig.getNoCacheInvokeConfigInstance()) {
      ...
      if (!cac.isEnableCacheContext()
          && cac.getCachedAnnoConfig() == null
          && cac.getInvalidateAnnoConfigs() == null
          && cac.getUpdateAnnoConfig() == null) {
  ...
  private void parseByTargetClass(
      CacheInvokeConfig cac, Class<?> clazz, String name, Class<?>[] paramTypes) {
