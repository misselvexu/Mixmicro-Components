...
import xyz.vopen.mixmicro.components.enhance.cache.MultiLevelCacheBuilder;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.CacheConsts;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.CacheType;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.EnableCache;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.method.CacheHandler;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.method.CacheInvokeConfig;
import xyz.vopen.mixmicro.components.enhance.cache.annotation.method.CacheInvokeContext;
...
public class CacheContext {
  ...
  private static ThreadLocal<CacheThreadLocal> cacheThreadLocal =
      new ThreadLocal<CacheThreadLocal>() {
        @Override
          ...
          if (cache == null) {
            if (cacheAnnoConfig instanceof CachedAnnoConfig) {
              cache = createCacheByCachedConfig((CachedAnnoConfig) cacheAnnoConfig, invokeContext);
                ...
                || (cacheAnnoConfig instanceof CacheUpdateAnnoConfig)) {
              CacheInvokeConfig cacheDefineConfig =
                  configMap.getByCacheName(cacheAnnoConfig.getArea(), cacheAnnoConfig.getName());
              if (cacheDefineConfig == null) {
                String message =
                    "can't find @Cached definition with area="
                        + cacheAnnoConfig.getArea()
                        + " name="
                        + cacheAnnoConfig.getName()
                        + ", specified in "
                        + cacheAnnoConfig.getDefineMethod();
              ...
              cache =
                  createCacheByCachedConfig(cacheDefineConfig.getCachedAnnoConfig(), invokeContext);
            }
  ...
  private Cache createCacheByCachedConfig(CachedAnnoConfig ac, CacheInvokeContext invokeContext) {
    ...
    if (CacheConsts.isUndefined(cacheName)) {
      ...
      cacheName =
          configProvider
              .createCacheNameGenerator(invokeContext.getHiddenPackages())
  ...
  public Cache __createOrGetCache(
      CachedAnnoConfig cachedAnnoConfig, String area, String cacheName) {
      ...
      synchronized (this) {
        ...
        if (cache == null) {
          if (globalCacheConfig.isAreaInCacheName()) {
            // for compatible reason, if we use default configuration, the prefix should same to
            // that version <=2.4.3
    ...
    if (cachedAnnoConfig.getCacheType() == CacheType.LOCAL) {
      ...
      cache =
          MultiLevelCacheBuilder.createMultiLevelCacheBuilder()
              .expireAfterWrite(
                  remote.config().getExpireAfterWriteInMillis(), TimeUnit.MILLISECONDS)
  ...
  protected Cache buildRemote(CachedAnnoConfig cachedAnnoConfig, String area, String cacheName) {
    ExternalCacheBuilder cacheBuilder =
        (ExternalCacheBuilder) globalCacheConfig.getRemoteCacheBuilders().get(area);
    ...
    if (!CacheConsts.isUndefined(cachedAnnoConfig.getKeyConvertor())) {
      cacheBuilder.setKeyConvertor(
          configProvider.parseKeyConvertor(cachedAnnoConfig.getKeyConvertor()));
    }
    if (!CacheConsts.isUndefined(cachedAnnoConfig.getSerialPolicy())) {
      cacheBuilder.setValueEncoder(
          configProvider.parseValueEncoder(cachedAnnoConfig.getSerialPolicy()));
      cacheBuilder.setValueDecoder(
          configProvider.parseValueDecoder(cachedAnnoConfig.getSerialPolicy()));
    }
  ...
  protected Cache buildLocal(CachedAnnoConfig cachedAnnoConfig, String area) {
    EmbeddedCacheBuilder cacheBuilder =
        (EmbeddedCacheBuilder) globalCacheConfig.getLocalCacheBuilders().get(area);
    if (cacheBuilder == null) {
    ...
    if (cachedAnnoConfig.getCacheType() == CacheType.BOTH
      ...
      cacheBuilder.expireAfterWrite(
          cachedAnnoConfig.getLocalExpire(), cachedAnnoConfig.getTimeUnit());
