...
import xyz.vopen.mixmicro.components.enhance.cache.external.AbstractExternalCache;
...
public class RedisLettuceCache<K, V> extends AbstractExternalCache<K, V> {
  ...
  public RedisLettuceCache(RedisLettuceCacheConfig<K, V> config) {
    ...
    stringCommands =
        (RedisStringCommands<byte[], byte[]>) lettuceConnectionManager.commands(client);
    stringAsyncCommands =
        (RedisStringAsyncCommands<byte[], byte[]>) lettuceConnectionManager.asyncCommands(client);
  ...
  protected CacheResult do_PUT(K key, V value, long expireAfterWrite, TimeUnit timeUnit) {
    try {
      ...
      RedisFuture<String> future =
          stringAsyncCommands.psetex(
              newKey, timeUnit.toMillis(expireAfterWrite), valueEncoder.apply(holder));
      CacheResult result =
          new CacheResult(
              future.handle(
                  (rt, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor().execute(() -> logError("PUT", key, ex));
  ...
  @Override
  protected CacheResult do_PUT_ALL(
    ...
    try {
      ...
      for (Map.Entry<? extends K, ? extends V> en : map.entrySet()) {
        CacheValueHolder<V> holder =
            new CacheValueHolder(en.getValue(), timeUnit.toMillis(expireAfterWrite));
        RedisFuture<String> resp =
            stringAsyncCommands.psetex(
                buildKey(en.getKey()),
        ...
        future =
            future.thenCombine(
                resp, (failCount, respStr) -> "OK".equals(respStr) ? failCount : failCount + 1);
      }
      CacheResult result =
          new CacheResult(
              future.handle(
                  (failCount, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor()
                          .execute(() -> logError("PUT_ALL", "map(" + map.size() + ")", ex));
                      return new ResultData(ex);
                    } else {
                      if (failCount == 0) {
                        return new ResultData(CacheResultCode.SUCCESS, null, null);
      ...
      CacheGetResult result =
          new CacheGetResult(
              future.handle(
                  (valueBytes, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor().execute(() -> logError("GET", key, ex));
                      return new ResultData(ex);
                    } else {
                      if (valueBytes != null) {
                        CacheValueHolder<V> holder =
                            (CacheValueHolder<V>) valueDecoder.apply(valueBytes);
  ...
  protected MultiGetResult<K, V> do_GET_ALL(Set<? extends K> keys) {
    try {
      ...
      MultiGetResult result =
          new MultiGetResult<K, V>(
              mgetResults.handle(
                  (list, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor()
                          .execute(() -> logError("GET_ALL", "keys(" + keys.size() + ")", ex));
                      return new ResultData(ex);
                    } else {
                      for (int i = 0; i < list.size(); i++) {
                        ...
                        if (kv != null && kv.hasValue()) {
                          ...
                          if (System.currentTimeMillis() >= holder.getExpireTime()) {
                            ...
                            CacheGetResult<V> r =
                                new CacheGetResult<V>(CacheResultCode.SUCCESS, null, holder);
                            resultMap.put(key, r);
                          }
                        } else {
      ...
      CacheResult result =
          new CacheResult(
              future.handle(
                  (rt, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor().execute(() -> logError("REMOVE", key, ex));
                      return new ResultData(ex);
                    } else {
                      if (rt == null) {
                        return new ResultData(CacheResultCode.FAIL, null, null);
  ...
  protected CacheResult do_REMOVE_ALL(Set<? extends K> keys) {
    try {
      byte[][] newKeys =
          keys.stream().map((k) -> buildKey(k)).toArray((len) -> new byte[keys.size()][]);
          ...
          new CacheResult(
              future.handle(
                  (rt, ex) -> {
                    if (ex != null) {
                      MixCacheExecutor.defaultExecutor()
                          .execute(() -> logError("PUT_IF_ABSENT", key, ex));
                      return new ResultData(ex);
                    } else {
                      if ("OK".equals(rt)) {
                        return new ResultData(CacheResultCode.SUCCESS, null, null);
