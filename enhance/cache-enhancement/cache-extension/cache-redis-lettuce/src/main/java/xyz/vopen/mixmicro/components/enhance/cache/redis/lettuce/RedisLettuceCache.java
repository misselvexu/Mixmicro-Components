...
public class RedisLettuceCache<K, V> extends AbstractExternalCache<K, V> {
    ...
    protected CacheResult do_PUT_ALL(Map<? extends K, ? extends V> map, long expireAfterWrite, TimeUnit timeUnit) {
        try {
            ...
            CacheResult result = new CacheResult(future.handle((failCount, ex) -> {
                if (ex != null) {
                    MixCacheExecutor.defaultExecutor().execute(() -> logError("PUT_ALL", "map(" + map.size() + ")", ex));
                    return new ResultData(ex);
                } else {
                    if (failCount == 0) {
                        return new ResultData(CacheResultCode.SUCCESS, null, null);
    ...
    protected MultiGetResult<K, V> do_GET_ALL(Set<? extends K> keys) {
        try {
            ...
            MultiGetResult result = new MultiGetResult<K, V>(mgetResults.handle((list, ex) -> {
                if (ex != null) {
                    MixCacheExecutor.defaultExecutor().execute(() -> logError("GET_ALL", "keys(" + keys.size() + ")", ex));
                    return new ResultData(ex);
                } else {
    ...
    protected CacheResult do_REMOVE_ALL(Set<? extends K> keys) {
        try {
            ...
            CacheResult result = new CacheResult(future.handle((v, ex) -> {
                if (ex != null) {
                    MixCacheExecutor.defaultExecutor().execute(() -> logError("REMOVE_ALL", "keys(" + keys.size() + ")", ex));
                    return new ResultData(ex);
                } else {
                    return new ResultData(CacheResultCode.SUCCESS, null, null);
                }
    ...
    protected CacheResult do_PUT_IF_ABSENT(K key, V value, long expireAfterWrite, TimeUnit timeUnit) {
        try {
            ...
            CacheResult result = new CacheResult(future.handle((rt, ex) -> {
                if (ex != null) {
                    MixCacheExecutor.defaultExecutor().execute(() -> logError("PUT_IF_ABSENT", key, ex));
                    return new ResultData(ex);
                } else {
                    if ("OK".equals(rt)) {
                        return new ResultData(CacheResultCode.SUCCESS, null, null);
