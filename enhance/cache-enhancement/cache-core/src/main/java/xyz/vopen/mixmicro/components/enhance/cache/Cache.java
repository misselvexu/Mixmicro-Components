...
public interface Cache<K, V> extends Closeable {
   ...
   *
   * <p>If the cache's builder has specified a {@link CacheLoader} and there is no association in
   * the cache , it will attempt to load the entry.
   *
   * <p>If error occurs during cache access, the method return null instead of throwing an
   * exception.
   *
   * @param key the key whose associated value is to be returned
   * @return the associated value. null may indicates:
   *     <ul>
   *       <li>the entry does not exist or expired
   *       <li>the entry value is null
   *       <li>error occurs during cache access(no exception throws)
   *     </ul>
   *
  ...
  /**
   * Gets a collection of entries from the Cache, returning them as Map of the values associated
   * with the set of keys requested.
   *
   * <p>If the cache's builder has specified a {@link CacheLoader} and there is no association in
   * the cache , it will attempt to load the entry.
   *
   * <p>If error occurs during cache access, the method will not throw an exception.
   *
   * @param keys The keys whose associated values are to be returned.
   * @return A map of entries that were found for the given keys. Keys not found in the cache are
   ...
   * <p>If error occurs during cache access, the method will not throw an exception.
   *
   * <p>if the implementation supports asynchronous operation, the cache operation of this method is
   * asynchronous.
   *
  ...
  /**
   * Atomically associates the specified key with the given value if it is not already associated
   * with a value.
   *
   * <p>If error occurs during cache access, the method will not throw an exception.
   *
   * <p>{@link MultiLevelCache} does not support this method.
   *
   * @param key key with which the specified value is to be associated
   * @param value value to be associated with the specified key
   * @return true if a value was set, false if the KV association does not exists in the cache, or
   *     error occurs during cache access.
   * @see #PUT_IF_ABSENT(Object, Object, long, TimeUnit)
   */
  default boolean putIfAbsent(K key, V value) {
    CacheResult result =
        PUT_IF_ABSENT(key, value, config().getExpireAfterWriteInMillis(), TimeUnit.MILLISECONDS);
   ...
   * @return true if the key is removed successfully, false if the KV association does not exists in
   *     the cache, or error occurs during cache access.
   * @see #REMOVE(Object)
   */
  ...
  /**
   * Provides a standard way to access the underlying concrete cache entry implementation in order
   * to provide access to further, proprietary features.
   *
   * <p>If the implementation does not support the specified class, the {@link
   * IllegalArgumentException} is thrown.
   *
   * @param clazz the proprietary class or interface of the underlying concrete cache. It is this
  ...
  /**
   * Use this cache attempt to acquire a exclusive lock specified by the key, this method will not
   * block. examples:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>{@link MultiLevelCache} will use the last level cache to support this operation.
   *
   * @param key lockKey
   * @param expire lock expire time
   * @param timeUnit lock expire time unit
   * @return an AutoReleaseLock(implements java.lang.AutoCloseable) instance if success. or null if
   *     the attempt fails, which indicates there is an another thread/process/server has the lock,
  ...
  default AutoReleaseLock tryLock(K key, long expire, TimeUnit timeUnit) {
          ...
          while (unlockCount++ < config.getTryLockUnlockCount()) {
            if (System.currentTimeMillis() < expireTimestamp) {
              ...
              if (unlockResult.getResultCode() == CacheResultCode.FAIL
                ...
                logger.info(
                    "[tryLock] [{} of {}] [{}] unlock failed. Key={}, msg = {}",
                    unlockCount,
                    config.getTryLockUnlockCount(),
                    uuid,
                    key,
                ...
                logger.debug(
                    "[tryLock] [{} of {}] [{}] successfully release the lock. Key={}",
                    unlockCount,
                    config.getTryLockUnlockCount(),
                    uuid,
                    key);
                ...
                logger.warn(
                    "[tryLock] [{} of {}] [{}] unexpected unlock result: Key={}, result={}",
                    unlockCount,
                    config.getTryLockUnlockCount(),
                    uuid,
                    key,
                    unlockResult.getResultCode());
                return;
              }
            } else {
              logger.info(
                  "[tryLock] [{} of {}] [{}] lock already expired: Key={}",
                  unlockCount,
                  config.getTryLockUnlockCount(),
                  uuid,
                  key);
    ...
    while (lockCount++ < config.getTryLockLockCount()) {
      ...
      if (lockResult.isSuccess()) {
        logger.debug(
            ...
            lockCount,
            config.getTryLockLockCount(),
            uuid,
            key);
        return lock;
      } else if (lockResult.getResultCode() == CacheResultCode.FAIL
          || lockResult.getResultCode() == CacheResultCode.PART_SUCCESS) {
        logger.info(
            "[tryLock] [{} of {}] [{}] cache access failed during get lock, will inquiry {} times. Key={}, msg={}",
            lockCount,
            config.getTryLockLockCount(),
            uuid,
            config.getTryLockInquiryCount(),
            key,
    ...
    logger.debug(
        "[tryLock] [{}] return null after {} attempts. Key={}",
        uuid,
        config.getTryLockLockCount(),
        key);
    return null;
   ...
   *
   * <p>{@link MultiLevelCache} will use the last level cache to support this operation. examples:
   *
   * <pre>
   ...
   * Gets an entry from the cache.
   *
   * <p>if the implementation supports asynchronous operation, the cache access may not completed
   * after this method return. The invoke of getResultCode()/isSuccess()/getMessage()/getValue() on
   * the result will block until cache operation is completed. Call future() method on the result
   * will get a CompletionStage instance for asynchronous programming.
   *
   * @param key the key
   * @return the result
   */
  ...
  /**
   * If there is a value associated with the key, return the value; otherwise use the loader load
   * the value and return, and then update the cache.
   *
   * @param key the key
   ...
   * @param key the key
   * @param loader the value loader
   * @param cacheNullWhenLoaderReturnNull true if null value returned by loader should put into
   *     cache use the key
   * @return the value
   */
   ...
   *
   * <p>if the implementation supports asynchronous operation, the cache access may not completed
   * after this method return. The invoke of getResultCode()/isSuccess()/getMessage() on the result
   * will block until cache operation is completed. Call future() method on the result will get a
   * CompletionStage instance for asynchronous programming.
   *
   * @param key key with which the specified value is to be associated
   * @param value value to be associated with the specified key
   * @return the result
   ...
   * @param expireAfterWrite the TTL(time to live) of the KV association
   * @param timeUnit the time unit of expireAfterWrite
   * @return SUCCESS if the specified key is not already associated with a value, or EXISTS if the
   *     specified key is already associated with a value, or FAIL if error occurs.
   */
