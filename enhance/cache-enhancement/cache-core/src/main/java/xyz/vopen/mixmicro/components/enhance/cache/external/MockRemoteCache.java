...
public class MockRemoteCache<K, V> extends AbstractExternalCache<K, V> {
  ...
  protected MultiGetResult<K, V> do_GET_ALL(Set<? extends K> keys) {
    ...
    if (resultMap != null) {
      ...
      result =
          new MultiGetResult<ByteBuffer, byte[]>(result.getResultCode(), null, (Map) returnMap);
    }
  ...
  protected CacheResult do_PUT(K key, V value, long expireAfterWrite, TimeUnit timeUnit) {
    return cache.PUT(
        genKey(key), config.getValueEncoder().apply(value), expireAfterWrite, timeUnit);
  }
  ...
  protected CacheResult do_PUT_ALL(
    ...
    map.entrySet()
        .forEach(
            (e) -> newMap.put(genKey(e.getKey()), config.getValueEncoder().apply(e.getValue())));
