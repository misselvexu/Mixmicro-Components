...
public class MultiLevelCache<K, V> extends AbstractCache<K, V> {
  ...
  protected MultiGetResult<K, V> do_GET_ALL(Set<? extends K> keys) {
    ...
    for (int i = 0; i < caches.length; i++) {
      ...
      if (allResult.isSuccess() && allResult.getValues() != null) {
        for (Map.Entry<K, CacheGetResult<CacheValueHolder<V>>> en :
            allResult.getValues().entrySet()) {
          K key = en.getKey();
  ...
  private CompletableFuture<ResultData> combine(
      CompletableFuture<ResultData> future, CacheResult result) {
