...
public class StatInfoLogger implements Consumer<StatInfo> {
  ...
  private StringBuilder logStatSummary(StatInfo statInfo) {
    ...
    List<CacheStat> stats = statInfo.getStats();
    OptionalInt maxCacheNameLength =
        stats.stream().mapToInt((s) -> getName(s.getCacheName()).length()).max();
    int len = Math.max(5, maxCacheNameLength.orElse(0));

    String title =
        String.format(
            "%-" + len + "s|%10s|%7s|%14s|%14s|%14s|%14s|%11s|%11s",
            "cache",
            "qps",
            "rate",
            "get",
  ...
  private StringBuilder logVerbose(StatInfo statInfo) {
    ...
    for (CacheStat s : stats) {
      String title =
          String.format(
              "%-10s|%10s|%14s|%14s|%14s|%14s|%14s|%9s|%7s|%7s",
              "oper",
              "qps/tps",
              "count",
              "success/hit",
      ...
      sb.append(String.format("%,7d", s.getMinGetTime() == Long.MAX_VALUE ? 0 : s.getMinGetTime()))
          .append('|');
      ...
      sb.append(String.format("%,7d", s.getMinPutTime() == Long.MAX_VALUE ? 0 : s.getMinPutTime()))
          .append('|');
      ...
      sb.append(
              String.format(
                  "%,7d", s.getMinRemoveTime() == Long.MAX_VALUE ? 0 : s.getMinRemoveTime()))
          .append('|');
      ...
      sb.append(
              String.format("%,7d", s.getMinLoadTime() == Long.MAX_VALUE ? 0 : s.getMinLoadTime()))
          .append('|');
