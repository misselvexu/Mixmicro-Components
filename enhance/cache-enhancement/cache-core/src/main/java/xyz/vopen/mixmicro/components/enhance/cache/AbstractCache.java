...
import xyz.vopen.mixmicro.components.enhance.cache.embedded.AbstractEmbeddedCache;
import xyz.vopen.mixmicro.components.enhance.cache.event.*;
...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
  ...
  static <K, V> V synchronizedLoad(
      CacheConfig config,
      AbstractCache<K, V> abstractCache,
      K key,
      Function<K, V> newLoader,
      Consumer<V> cacheUpdater) {
  ...
  public final CacheResult PUT(K key, V value, long expireAfterWrite, TimeUnit timeUnit) {
    ...
    result
        ...
        .thenRun(
            () -> {
              CachePutEvent event =
                  new CachePutEvent(this, System.currentTimeMillis() - t, key, value, result);
              notify(event);
            });
    return result;
  }
  ...
  @Override
  public final CacheResult PUT_ALL(
    ...
    result
        ...
        .thenRun(
            () -> {
              CachePutAllEvent event =
                  new CachePutAllEvent(this, System.currentTimeMillis() - t, map, result);
              notify(event);
            });
    return result;
  }
  ...
  public final CacheResult REMOVE(K key) {
    ...
    result
        ...
        .thenRun(
            () -> {
              CacheRemoveEvent event =
                  new CacheRemoveEvent(this, System.currentTimeMillis() - t, key, result);
              notify(event);
            });
    return result;
  }
  ...
  public final CacheResult REMOVE_ALL(Set<? extends K> keys) {
    ...
    result
        ...
        .thenRun(
            () -> {
              CacheRemoveAllEvent event =
                  new CacheRemoveAllEvent(this, System.currentTimeMillis() - t, keys, result);
              notify(event);
            });
    return result;
  }
  ...
  protected abstract CacheResult do_PUT_IF_ABSENT(
