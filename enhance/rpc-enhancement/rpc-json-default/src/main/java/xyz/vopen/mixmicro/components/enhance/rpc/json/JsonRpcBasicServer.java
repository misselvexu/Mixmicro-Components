...
public class JsonRpcBasicServer {
	...
	public int handleRequest(final InputStream input, final OutputStream output) throws IOException {
		...
		try {
			...
			return writeAndFlushValueError(output, createResponseError(VERSION, NULL, ErrorResolver.JsonError.PARSE_ERROR)).code;
		}
	}
	...
	protected ErrorResolver.JsonError handleJsonNodeRequest(final JsonNode node, final OutputStream output) throws IOException {
		...
		return this.writeAndFlushValueError(output, this.createResponseError(VERSION, NULL, ErrorResolver.JsonError.INVALID_REQUEST));
	}
	 ...
	 * @throws IOException on error
	 */
	private ErrorResolver.JsonError handleArray(ArrayNode node, OutputStream output) throws IOException {
		logger.debug("Handling {} requests", node.size());
		ErrorResolver.JsonError result = ErrorResolver.JsonError.OK;
		output.write('[');
		int errorCount = 0;
		for (int i = 0; i < node.size(); i++) {
			ErrorResolver.JsonError nodeResult = handleJsonNodeRequest(node.get(i), output);
	...
	private boolean isError(ErrorResolver.JsonError result) {
		return result.code != ErrorResolver.JsonError.OK.code;
	}
	 ...
	 * @throws IOException on error
	 */
	private ErrorResolver.JsonError handleObject(final ObjectNode node, final OutputStream output) throws IOException {
		logger.debug("Request: {}", node);
		
		if (!isValidRequest(node)) {
			return writeAndFlushValueError(output, createResponseError(VERSION, NULL, ErrorResolver.JsonError.INVALID_REQUEST));
		}
		Object id = parseId(node.get(ID));
		
		String jsonRpc = Util.hasNonNullData(node, JSONRPC) ? node.get(JSONRPC).asText() : VERSION;
		if (!Util.hasNonNullData(node, METHOD)) {
			return writeAndFlushValueError(output, createResponseError(jsonRpc, id, ErrorResolver.JsonError.METHOD_NOT_FOUND));
		}
		...
		Set<Method> methods = ReflectionUtil.findCandidateMethods(getHandlerInterfaces(serviceName), partialMethodName);
		if (methods.isEmpty()) {
			return writeAndFlushValueError(output, createResponseError(jsonRpc, id, ErrorResolver.JsonError.METHOD_NOT_FOUND));
		}
		AMethodWithItsArgs methodArgs = findBestMethodByParamsNode(methods, node.get(PARAMS));
		if (methodArgs == null) {
			return writeAndFlushValueError(output, createResponseError(jsonRpc, id, ErrorResolver.JsonError.METHOD_PARAMS_INVALID));
		}
		try (InvokeListenerHandler handler = new InvokeListenerHandler(methodArgs, invocationListener)) {
			try {
				...
				return ErrorResolver.JsonError.OK;
			} catch (Throwable e) {
				handler.error = e;
	...
	private ErrorResolver.JsonError handleError(OutputStream output, Object id, String jsonRpc, AMethodWithItsArgs methodArgs, Throwable e) throws IOException {
	 ...
	 */
	private ErrorObjectWithJsonError createResponseError(String jsonRpc, Object id, ErrorResolver.JsonError errorObject) {
