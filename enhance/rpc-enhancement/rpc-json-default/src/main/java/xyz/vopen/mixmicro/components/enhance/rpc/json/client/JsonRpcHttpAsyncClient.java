...
public class JsonRpcHttpAsyncClient {
	...
	private void writeRequest(String methodName, Object arguments, HttpRequest httpRequest) throws IOException {
		...
		request.put(JsonRpcBasicServer.ID, nextId.getAndIncrement());
		request.put(JsonRpcBasicServer.JSONRPC, JsonRpcBasicServer.VERSION);
		request.put(JsonRpcBasicServer.METHOD, methodName);
		
		if (arguments != null && arguments.getClass().isArray()) {
			Object[] args = Object[].class.cast(arguments);
			if (args.length > 0) {
				request.set(JsonRpcBasicServer.PARAMS, mapper.valueToTree(Object[].class.cast(arguments)));
			}
	...
	private <T> T readResponse(Type returnType, InputStream ips) throws Throwable {
		...
		if (jsonObject.has(JsonRpcBasicServer.ERROR) && jsonObject.get(JsonRpcBasicServer.ERROR) != null && !jsonObject.get(JsonRpcBasicServer.ERROR).isNull()) {
			throw exceptionResolver.resolveException(jsonObject);
		}
		if (jsonObject.has(JsonRpcBasicServer.RESULT) && !jsonObject.get(JsonRpcBasicServer.RESULT).isNull() && jsonObject.get(JsonRpcBasicServer.RESULT) != null) {
			
			JsonParser returnJsonParser = mapper.treeAsTokens(jsonObject.get(JsonRpcBasicServer.RESULT));
