...
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.ERROR;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.ID;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.JSONRPC;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.METHOD;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.PARAMS;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.RESULT;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.server.JsonRpcBasicServer.VERSION;
import static xyz.vopen.mixmicro.components.enhance.rpc.json.utils.Util.hasNonNullData;

/**
 * A Mixmicro RPC client.
 *
...
public class JsonRpcClient {
	...
	public Object invokeAndReadResponse(String methodName, Object argument, Type returnType, OutputStream output, InputStream input) throws Throwable {
		return invokeAndReadResponse(methodName, argument, returnType, output, input, this.requestIDGenerator.nextId());
	}
	 ...
	 * @param output     the {@link OutputStream} to write to
	 * @param input      the {@link InputStream} to read from
	 * @param id         id to send with the Mixmicro RPC request
	 * @return the returned Object
	...
	/**
	 * Reads a Mixmicro RPC response from the server.  This blocks until
	...
	/**
	 * Writes a Mixmicro RPC request to the given {@link OutputStream}.
	 * If the value passed for argument is null then the {@code params}
	 * property is omitted from the Mixmicro RPC request.  If the argument
	...
	public void invoke(String methodName, Object argument, OutputStream output) throws IOException {
		invoke(methodName, argument, output, this.requestIDGenerator.nextId());
	}
	...
	protected ObjectNode createRequest(String methodName, Object argument) {
		return internalCreateRequest(methodName, argument, this.requestIDGenerator.nextId());
	}
	...
	/**
	 * Writes a Mixmicro RPC notification to the given
	 * {@link OutputStream}.
	 *
	 * @param methodName the method to invoke
	 * @param argument   the method argument
	 ...
	 * that represent the Mixmicro RPC requests and responses.
	 */
