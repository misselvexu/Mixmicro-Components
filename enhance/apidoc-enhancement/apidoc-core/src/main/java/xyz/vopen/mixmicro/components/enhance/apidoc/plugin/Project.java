...
class Project {
  ...
  private static void setResultMapToAction(
      Map<String, Object> resultMap, Set<Parameter> parameterSet) {
    for (Map.Entry<String, Object> entry : resultMap.entrySet()) {
      Parameter parameter = Parameter.newParameter();
      parameter.setIdentifier(entry.getKey());
      if (CommonUtils.isValueType(entry.getValue())) {
  ...
  private static void setResponseToAction(ClassNode responseNode, Set<Parameter> parameterSet) {
    for (FieldNode fieldNode : responseNode.getChildNodes()) {
      ...
      // cover
      if (mockNode != null) {
        if (StringUtils.isNotEmpty(mockNode.getLimit())) {
          parameter.setIdentifier(String.format("%s|%s", fieldNode.getName(), mockNode.getLimit()));
        }
        if (StringUtils.isNotEmpty(mockNode.getValue())) {
