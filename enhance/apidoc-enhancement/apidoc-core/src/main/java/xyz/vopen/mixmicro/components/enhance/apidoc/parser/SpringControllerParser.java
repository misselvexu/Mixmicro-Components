        ...
        .ifPresent(
            a -> {
              if (a instanceof SingleMemberAnnotationExpr) {
                ...
                controllerNode.setBaseUrl(CommonUtils.removeQuotations(baseUrl));
                return;
              }
              if (a instanceof NormalAnnotationExpr) {
                ((NormalAnnotationExpr) a)
                    .getPairs().stream()
                        .filter(v -> isUrlPathKey(v.getNameAsString()))
                        .findFirst()
                        .ifPresent(
                            p -> {
                              controllerNode.setBaseUrl(
        ...
        .forEach(
            an -> {
              ...
              if (Arrays.asList(MAPPING_ANNOTATIONS).contains(name)) {
                String method = CommonUtils.getClassName(name).toUpperCase().replace("MAPPING", "");
                            ...
                            if ("headers".equals(key)) {
                              ...
                              if (methodAttr instanceof ArrayInitializerExpr) {
                                NodeList<Expression> values =
                                    ((ArrayInitializerExpr) methodAttr).getValues();
                                for (Node n : values) {
                              ...
                              if (methodAttr instanceof ArrayInitializerExpr) {
                                ...
                                for (Node n : values) {
                                  requestNode.addMethod(
                                      RequestMethod.valueOf(CommonUtils.getClassName(n.toString()))
                                          .name());
                                }
                              } else {
                                requestNode.addMethod(
                                    RequestMethod.valueOf(
                                            CommonUtils.getClassName(p.getValue().toString()))
                                        .name());
                              }
                            }
                          });
                    ...
                    .forEach(
                        an -> {
                          ...
                          if (an instanceof SingleMemberAnnotationExpr) {
                            paramNode.setName(
                                ((StringLiteralExpr)
                                        ((SingleMemberAnnotationExpr) an).getMemberValue())
                                    .getValue());
                            return;
                          }
  ...
  protected void handleResponseNode(ResponseNode responseNode, Type resultType) {
    if (resultType instanceof ClassOrInterfaceType) {
      ...
      if ("org.springframework.http.ResponseEntity".endsWith(className)) {
        Optional<NodeList<Type>> nodeListOptional =
            ((ClassOrInterfaceType) resultType).getTypeArguments();
