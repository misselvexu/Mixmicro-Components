...
import static xyz.vopen.mixmicro.components.enhancement.http.HttpUtils.SP;
import static xyz.vopen.mixmicro.components.enhancement.http.HttpUtils.getServerAddr;
...
public class HttpClient implements Runnable {
  ...
  /**
   * mixmicro-http-kit think all connections are keep-alived (since some say it is, but
   * actually is not). but, some are not, mixmicro-http-kit pick them out after the fact
   * <ol>
  ...
  private void doRead(SelectionKey key, long now) {
    ...
    if (read == -1) { // read all, remote closed it cleanly
      ...
      try {
        ...
        if (req.decoder.decode(buffer) == State.ALL_READ) {
          req.finish();
