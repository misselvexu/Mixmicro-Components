...
import xyz.vopen.mixmicro.components.enhancement.http.DynamicBytes;
...
import static xyz.vopen.mixmicro.components.enhancement.http.HttpUtils.*;
...
public class AsyncChannel {
  ...
  private void writeChunk(Object body, boolean close) throws IOException {
    if (body instanceof Map) { // only get body if a map
      body = ((Map<Keyword, Object>) body).get(ClojureRing.BODY);
    }
  ...
  public boolean serverClose(int status) {
    ...
    if (isWebSocket()) {
      server.tryWrite(key, WsEncode(WSDecoder.OPCODE_CLOSE, ByteBuffer.allocate(2)
  ...
  public boolean send(Object data, boolean close) throws IOException {
    ...
    if (isWebSocket()) {
      if (data instanceof Map) { // only get the :body if map
        Object tmp = ((Map<Keyword, Object>) data).get(ClojureRing.BODY);
      ...
      if (data instanceof String) { // null is not allowed
        server.tryWrite(key, WsEncode(WSDecoder.OPCODE_TEXT, ((String) data).getBytes(UTF_8)));
      } else if (data instanceof byte[]) {
        server.tryWrite(key, WsEncode(WSDecoder.OPCODE_BINARY, (byte[]) data));
      } else if (data instanceof InputStream) {
        DynamicBytes bytes = readAll((InputStream) data);
        server.tryWrite(key, WsEncode(WSDecoder.OPCODE_BINARY, bytes.get(), bytes.length()));
