...
public class PathTarget {
  ...
  public <T> PathTarget(
      final Mapper mapper, final Class<T> clazz, final String field, final boolean validateNames) {
  ...
  /**
   * Returns the translated path for this context. If validation is disabled, that path could be the
  ...
  public String translatedPath() {
    ...
    return MongoUtils.join(segments, '.');
  }
  ...
  private void failValidation() {
    throw new ValidationException(
        String.format(
            ...
            MongoUtils.join(segments, '.'), root.getClazz().getName()));
  }
  ...
  private MappedField resolveField(final String segment) {
    ...
    if (mf != null) {
      context =
          mapper.getMappedClass(mf.getSubClass() != null ? mf.getSubClass() : mf.getConcreteType());
    }
    return mf;
  }
