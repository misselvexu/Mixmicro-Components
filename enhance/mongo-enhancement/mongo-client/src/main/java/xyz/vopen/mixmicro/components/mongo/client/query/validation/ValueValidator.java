...
public abstract class ValueValidator implements Validator {
   ...
   * validation is not applied and this method returns false. If the value is to be validated, then
   * the validate method is called to see if the value and type are compatible. Errors are appended
   * to the validationFailures list.
   *
  ...
  public boolean apply(
      final Class<?> type, final Object value, final List<ValidationFailure> validationFailures) {
  ...
  protected abstract void validate(
      Class<?> type, Object value, List<ValidationFailure> validationFailures);
}
