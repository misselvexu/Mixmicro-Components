...
public class MapReduceOptions<T> {
  ...
  /**
   * Set the JavaScript function that associates or "maps" a value with a key and emits the key and
   * value pair.
   *
   ...
   *
   * @param maxTimeMS the maximum execution time in milliseconds. A non-zero value requires a server
   *     version &gt;= 2.6
   * @return this
  ...
  /**
   * Sets the input collection for the job should that collection differ from the mapped collection
   * used in the query
   *
  ...
  /**
   * Sets the read preference for this command. See the documentation for {@link ReadPreference} for
   * more information.
   *
  ...
  /**
   * Sets the JavaScript function that "reduces" to a single object all the values associated with a
   * particular key.
   *
  ...
  MapReduceCommand toCommand(final Mapper mapper) {
    ...
    final MapReduceCommand command =
        new MapReduceCommand(dbColl, map, reduce, target, outputType, query.getQueryObject());
