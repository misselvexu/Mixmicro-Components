...
import xyz.vopen.mixmicro.components.mongo.client.annotations.EntityListeners;
...
import xyz.vopen.mixmicro.components.mongo.client.mapping.validation.MappingValidator;
...
public class MappedClass {
  ...
  private static final List<Class<? extends Annotation>> LIFECYCLE_ANNOTATIONS =
      asList(PrePersist.class, PreSave.class, PreLoad.class, PostPersist.class, PostLoad.class);
      ...
      if (methodPairs != null) {
        ...
        for (final ClassMethodPair cm : methodPairs) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                format(
                    "Calling lifecycle method(@%s %s) on %s", event.getSimpleName(), method, inst));
          }
