...
import xyz.vopen.mixmicro.components.mongo.client.annotations.Indexed;
import xyz.vopen.mixmicro.components.mongo.client.annotations.Indexes;
...
public interface MongoRepository {
   ...
   *
   * <p>mongodb.driver.manual core/document-validation/
   * @since 1.3
   */
   ...
   * @deprecated This method uses the legacy approach for defining indexes. Switch to using
   *     annotations on entity classes or the methods in the Java driver itself.
   */
  @Deprecated
   ...
   * @param operations the updates to apply to the matched documents
   * @param oldVersion indicated the old version of the Entity should be returned
   * @param createIfMissing if the query returns no results, then a new object will be created (sets
   *     upsert=true)
  ...
  @Deprecated
  <T> T findAndModify(
      Query<T> query, UpdateOperations<T> operations, boolean oldVersion, boolean createIfMissing);
  ...
  /**
   * Work as if you did an update with each field in the entity doing a $set; Only at the top level
   * of the entity.
   *
   ...
   *
   * <p><i>The return type will change in 2.0</i>
   *
   ...
   * @param query the query used to match the documents to update
   * @param operations the update operations to perform
   * @param createIfMissing if true, a document will be created if none can be found that match the
   *     query
   * @param <T> the type of the entity
   * @return the results of the updates
   * @deprecated use {@link #update(Query, UpdateOperations, UpdateOptions)} with upsert set to the
   *     value of createIfMissing
   */
  @Deprecated
  ...
  <T> UpdateResults update(
      Query<T> query, UpdateOperations<T> operations, boolean createIfMissing, WriteConcern wc);
  ...
  /**
   * Updates the first entity found with the operations, if nothing is found insert the update as an
   * entity if "createIfMissing" is true.
   *
   * @param query the query used to match the documents to update
   * @param operations the update operations to perform
   * @param createIfMissing if true, a document will be created if none can be found that match the
