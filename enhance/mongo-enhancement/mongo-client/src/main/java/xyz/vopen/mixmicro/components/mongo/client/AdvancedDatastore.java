...
public interface AdvancedDatastore extends Datastore {
  ...
  /**
   * Creates a reference to the entity (using the current DB -can be null-, the collectionName, and
   * id)
   *
  ...
  /**
   * Deletes an entity of the given type T, with the given {@code id}, from the collection with the
   * name in the {@code kind} param. Validates the {@code id}, checking it's the correct type for an
   * ID for entities of type {@code T}. The entity type {@code clazz} is used only for validation,
   * not for filtering, therefore if you have entities of different types in the same collection
   * ({@code kind}), this method will delete any entity with the given {@code id}, regardless of its
   * type.
   *
   ...
   * @param collection the collection to update
   * @param clazz the class from which to get the index definitions
   * @param background if true, the index will be built in the background. If false, the method will
   *     block until the index is created.
   * @param <T> the type to index
   * @deprecated use {@link #ensureIndexes(String, Class)} instead and specify background true or
   *     false in the annotations
   */
  @Deprecated
   ...
   * @param readPreference Uses the supplied ReadPreference for the check. If readPreference is null
   *     the preference is taken from the annotation or uses the default preference.
   ...
   * @return the query
   *
   * @deprecated use {@link #find(String, Class)} and apply the options to the {@link
