...
import xyz.vopen.mixmicro.components.mongo.client.annotations.PrePersist;
...
import xyz.vopen.mixmicro.components.mongo.client.converters.CustomConverters;
...
import xyz.vopen.mixmicro.components.mongo.client.query.QueryImpl;
import xyz.vopen.mixmicro.components.mongo.client.query.ValidationException;
...
import static xyz.vopen.mixmicro.components.mongo.client.utils.ReflectionUtils.getParameterizedClass;
import static xyz.vopen.mixmicro.components.mongo.client.utils.ReflectionUtils.implementsInterface;
import static xyz.vopen.mixmicro.components.mongo.client.utils.ReflectionUtils.isPropertyType;
...
public class Mapper {
   ...
   */
  @Deprecated public static final String ID_KEY = "_id";
   ...
   * @deprecated
   */
  @Deprecated public static final String CLASS_NAME_FIELDNAME = "className";
  ...
  // EntityInterceptors; these are called after EntityListeners and lifecycle methods on an Entity,
  // for all Entities
  ...
  private final xyz.vopen.mixmicro.components.mongo.client.converters.Converters converters;
  ...
  public <T> T fromDb(
    ...
    if (dbObject.containsField("_id")
      ...
      final Key<T> key =
          new Key(entity.getClass(), getCollectionName(entity.getClass()), dbObject.get("_id"));
    ...
    if (entity instanceof Map) {
      ...
      if (updated.containsField("_id") && getMappedClass(entity).getIdField() != null) {
        final Key key =
            new Key(entity.getClass(), getCollectionName(entity.getClass()), updated.get("_id"));
  ...
  public Class<?> getClassFromCollection(final String collection) {
    ...
    if (mcs.size() > 1) {
      if (LOG.isWarnEnabled()) {
        ...
        LOG.warn(
            format("Found more than one class mapped to collection '%s': %s", collection, classes));
      }
    }
  ...
  public xyz.vopen.mixmicro.components.mongo.client.converters.Converters getConverters() {
    return converters;
  }
   ...
   *
   * @param entity the entity to process
   * @param collection the collection to use in the Key rather than the mapped collection as defined
  ...
  /**
   * Gets the {@link MappedClass} for the object (type). If it isn't mapped, create a new class and
   * cache it (without validating).
   *
   ...
   * Very similar to {@link Mapper#toDBObject}. Used (mainly) by query/update operations.
   *
  ...
  public Object toMongoObject(final MappedField mf, final MappedClass mc, final Object value) {
    ...
    if (value instanceof Query) {
      ...
      // convert the value to Key (DBRef) if the field is @Reference or type is Key/DBRef, or if the
      // destination class is an @Entity
      try {
        if (value instanceof Iterable) {
          ...
          if (mf != null) {
            ...
            if (!mf.getType().equals(Key.class) && isMapped(mf.getType())) {
              final MappedField idField = getMappedClass(mf.getType()).getMappedIdField();
      ...
      try {
        mappedValue =
            Serializer.serialize(value, !mf.getAnnotation(Serialized.class).disableCompression());
      } catch (IOException e) {
      ...
      mappedValue =
          toMongoObject(value, EmbeddedMapper.shouldSaveClassName(value, mappedValue, mf));
  ...
  <T> Key<T> manualRefToKey(final String collection, final Object id) {
    return id == null
        ? null
        : new Key<T>((Class<? extends T>) getClassFromCollection(collection), collection, id);
  }
  ...
  /**
   * Converts a java object to a mongo-compatible object (possibly a DBObject for complex mappings).
   * Very similar to {@link Mapper#toDBObject}
   *
  ...
  DBObject toDBObject(
      final Object entity, final Map<Object, DBObject> involvedObjects, final boolean lifecycle) {
