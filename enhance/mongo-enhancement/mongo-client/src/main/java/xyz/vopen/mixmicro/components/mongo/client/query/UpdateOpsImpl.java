...
import static xyz.vopen.mixmicro.components.mongo.client.utils.ReflectionUtils.iterToList;
...
public class UpdateOpsImpl<T> implements UpdateOperations<T> {
  ...
  public UpdateOperations<T> push(final String field, final Object value) {
    return push(
        field, value instanceof List ? (List<?>) value : singletonList(value), new PushOptions());
  }
  ...
  public UpdateOperations<T> push(
    ...
    BasicDBObject dbObject =
        new BasicDBObject(
            UpdateOperator.EACH.val(), mapper.toMongoObject(pathTarget.getTarget(), null, values));
