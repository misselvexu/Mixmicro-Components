...
import xyz.vopen.mixmicro.components.mongo.client.annotations.Collation;
import xyz.vopen.mixmicro.components.mongo.client.annotations.Field;
import xyz.vopen.mixmicro.components.mongo.client.annotations.Index;
import xyz.vopen.mixmicro.components.mongo.client.annotations.IndexOptions;
...
import xyz.vopen.mixmicro.components.mongo.client.internal.MongoUtils;
...
final class IndexHelper {
  ...
  Index convert(final Indexed indexed, final String nameToStore) {
    ...
    List<Field> fields =
        Collections.<Field>singletonList(
            new FieldBuilder().value(nameToStore).type(IndexType.fromValue(indexed.value().toIndexValue())));
  ...
  private MappingException pathFail(final MappedClass mc, final List<String> path) {
    return new MappingException(
        String.format(
            "Could not resolve path '%s' against '%s'.", MongoUtils.join(path, '.'), mc.getClazz().getName()));
  }
