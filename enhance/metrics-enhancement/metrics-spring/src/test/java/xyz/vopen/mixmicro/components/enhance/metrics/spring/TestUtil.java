...
class TestUtil {
	...
	@Deprecated
	static String forLegacyCachedGauge(Class<?> klass, Member member, xyz.vopen.mixmicro.components.enhance.metrics.annotation.CachedGauge annotation) {
		return Util.forCachedGauge(klass, member, annotation);
	}

	@Deprecated
	static String forLegacyCountedMethod(Class<?> klass, Member member, xyz.vopen.mixmicro.components.enhance.metrics.annotation.Counted annotation) {
		return Util.forCountedMethod(klass, member, annotation);
	}

	@Deprecated
	static String forLegacyMetricField(Class<?> klass, Member member, xyz.vopen.mixmicro.components.enhance.metrics.annotation.Metric annotation) {
		return Util.forMetricField(klass, member, annotation);
	}
	...
	static CachedGauge<?> forLegacyCachedGaugeMethod(MetricRegistry metricRegistry, Class<?> clazz, String methodName) {
		Method method = findMethod(clazz, methodName);
		String metricName = forLegacyCachedGauge(clazz, method, method.getAnnotation(xyz.vopen.mixmicro.components.enhance.metrics.annotation.CachedGauge.class));
		log.info("Looking up cached gauge method named '{}'", metricName);
		return (CachedGauge<?>) metricRegistry.getGauges().get(metricName);
	}
	...
	static Counter forLegacyCountedMethod(MetricRegistry metricRegistry, Class<?> clazz, String methodName) {
		Method method = findMethod(clazz, methodName);
		String metricName = forLegacyCountedMethod(clazz, method, method.getAnnotation(xyz.vopen.mixmicro.components.enhance.metrics.annotation.Counted.class));
		log.info("Looking up counted method named '{}'", metricName);
		return metricRegistry.getCounters().get(metricName);
	}
	...
	static com.codahale.metrics.Metric forLegacyMetricField(MetricRegistry metricRegistry, Class<?> clazz, String fieldName) {
		Field field = findField(clazz, fieldName);
		String metricName = forLegacyMetricField(clazz, field, field.getAnnotation(xyz.vopen.mixmicro.components.enhance.metrics.annotation.Metric.class));
		return getMetric(metricRegistry, field.getType(), metricName);
	}
