...
import static xyz.vopen.mixmicro.components.enhance.metrics.spring.TestUtil.forLegacyMetricField;
...
import xyz.vopen.mixmicro.components.enhance.metrics.annotation.Metric;
...
public class LegacyMetricAnnotationTest {
	...
	public void targetIsNotNull() throws Exception {
		...
		Meter meter = (Meter) TestUtil.forLegacyMetricField(metricRegistry, LegacyMetricAnnotationTest.Target.class, "theNameForTheMeter");
		...
		Timer timer = (Timer) TestUtil.forLegacyMetricField(metricRegistry, LegacyMetricAnnotationTest.Target.class, "timer");
		assertSame(target.timer, timer);
		assertSame(target2.timer, timer);

		assertNotNull(target.counter);
		assertNotNull(target2.counter);
		Counter ctr = (Counter) TestUtil.forLegacyMetricField(metricRegistry, LegacyMetricAnnotationTest.Target.class, "counter");
		assertSame(target.counter, ctr);
		assertSame(target2.counter, ctr);

		assertNotNull(target.histogram);
		assertNotNull(target2.histogram);
		Histogram hist = (Histogram) TestUtil.forLegacyMetricField(metricRegistry, LegacyMetricAnnotationTest.Target.class, "histogram");
		...
		Histogram uniHist = (Histogram) TestUtil.forLegacyMetricField(metricRegistry, LegacyMetricAnnotationTest.Target.class, "uniformHistogram");
