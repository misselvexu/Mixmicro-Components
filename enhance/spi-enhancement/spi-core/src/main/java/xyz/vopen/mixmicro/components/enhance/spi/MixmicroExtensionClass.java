 ...
 */
public class MixmicroExtensionClass<T> {
  ...
  public MixmicroExtensionClass(Class<? extends T> clazz, String alias) {
    this.clazz = clazz;
    this.alias = alias;
  }
      ...
      try {
        if (singleton) { // 如果是单例
          if (instance == null) {
            synchronized (this) {
              if (instance == null) {
                instance = MixmicroClassKit.newInstanceWithArgs(clazz, argTypes, args);
              }
            }
          }
          return instance; // 保留单例
        } else {
          return MixmicroClassKit.newInstanceWithArgs(clazz, argTypes, args);
        }
      } catch (Exception e) {
        throw new MixmicroSpiException("create " + clazz.getCanonicalName() + " instance error", e);
      }
    }
    throw new MixmicroSpiException("Class of ExtensionClass is null");
  }
