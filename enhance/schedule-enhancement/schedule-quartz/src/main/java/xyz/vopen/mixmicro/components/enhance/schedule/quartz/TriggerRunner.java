...
import xyz.vopen.mixmicro.components.enhance.schedule.quartz.cluster.TriggerRecoverer;
import xyz.vopen.mixmicro.components.enhance.schedule.quartz.repository.CalendarRepository;
...
public class TriggerRunner {
  ...
  private List<OperableTrigger> acquireNextTriggers(Date noLaterThanDate, int maxCount)
    ...
    try {
      for (Document triggerDoc : triggerRepository.findEligibleToRun(noLaterThanDate)) {
        ...
        if (trigger.getJobKey() == null) {
          log.error(
              ...
              trigger.getKey());
          triggerRepository.transferState(
              trigger.getKey(), Constants.STATE_WAITING, Constants.STATE_ERROR);
          continue;
        }
  ...
  private TriggerFiredBundle createTriggerFiredBundle(OperableTrigger trigger)
      throws JobPersistenceException {
    Calendar cal = calendarRepository.retrieveCalendar(trigger.getCalendarName());
  ...
  private JobDetail retrieveJob(OperableTrigger trigger) throws JobPersistenceException {
    try {
      return jobRepository.retrieveJob(trigger.getJobKey());
