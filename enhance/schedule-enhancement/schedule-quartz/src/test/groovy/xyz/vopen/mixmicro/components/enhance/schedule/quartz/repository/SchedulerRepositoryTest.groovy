...
import xyz.vopen.mixmicro.components.enhance.schedule.quartz.Clocks
...
class SchedulerRepositoryTest extends Specification {
    ...
    def addEntry(String id, long checkinTime) {
        MongoHelper.addScheduler([
                (SchedulerRepository.SCHEDULER_NAME_FIELD)   : schedulerName,
                (SchedulerRepository.INSTANCE_ID_FIELD)      : id,
                (SchedulerRepository.CHECKIN_INTERVAL_FIELD) : 100l,
                (SchedulerRepository.LAST_CHECKIN_TIME_FIELD): checkinTime])
    }
    ...
    def createDao(String id, Clock clock) {
        def dao = new SchedulerRepository(MongoHelper.getSchedulersColl(),
