...
import xyz.vopen.mixmicro.components.enhance.security.encryption.pbe.config.PBECleanablePasswordConfig;
...
import xyz.vopen.mixmicro.components.enhance.security.normalization.Normalizer;
import xyz.vopen.mixmicro.components.enhance.security.salt.FixedSaltGenerator;
...
/**
 * Standard implementation of the {@link PBEByteEncryptor} interface. This class lets the user
 * specify the algorithm, provider and the initialization vector (IV) generator to be used for
 * encryption, the password to use, the number of hashing iterations and the salt generator that
 * will be applied for obtaining the encryption key.
 *
 ...
 *
 * <p>The algorithm, provider, IV generator, password, key-obtention iterations and salt generator
 ...
 *       <tt>setProviderName(...)</tt>, <tt>setIvGenerator(...)</tt>, <tt>setPassword(...)</tt>,
 ...
 * brute force attack on each separate piece of encrypted data. The same applies when a random IV
 * generator is used.
 *
...
public final class StandardPBEByteEncryptor implements PBEByteCleanablePasswordEncryptor {
  ...
  /**
   * The default number of hashing iterations applied for obtaining the encryption key from the
   * specified password, set to 1000.
   */
  public static final int DEFAULT_KEY_OBTENTION_ITERATIONS = 1000;

  /**
   * The default salt size, only used if the chosen encryption algorithm is not a block algorithm
   * and thus block size cannot be used as salt size.
   */
  public static final int DEFAULT_SALT_SIZE_BYTES = 8;

  /**
   * The default IV size, only used if the chosen encryption algorithm is not a block algorithm and
   * thus block size cannot be used as IV size.
   */
  ...
  public synchronized void initialize() {
    ...
    if (!this.initialized) {
      ...
      if (this.config != null) {
        ...
        final Integer configKeyObtentionIterations = this.config.getKeyObtentionIterations();
        if (configKeyObtentionIterations != null) {
          CommonUtils.validateIsTrue(
              configKeyObtentionIterations.intValue() > 0,
              "Number of iterations for key obtention must be " + "greater than zero");
        }
        ...
        this.keyObtentionIterations =
            ((this.iterationsSet) || (configKeyObtentionIterations == null))
                ? this.keyObtentionIterations
      ...
      try {
        ...
        if (this.provider != null) {
          ...
          final SecretKeyFactory factory =
              SecretKeyFactory.getInstance(this.algorithm, this.provider);
          ...
          final SecretKeyFactory factory =
              SecretKeyFactory.getInstance(this.algorithm, this.providerName);
      ...
      this.optimizingDueFixedSalt =
          (this.saltGenerator instanceof FixedSaltGenerator)
   ...
   * Password-Based Cryptography Standard</a>.
   *
   * <p>This encryptor uses a salt and IV for each encryption operation. Sizes of the salt and IV
   * depends on the algorithm being used. The salt and the IV, if generated by a random generator,
   * they are also appended unencrypted at the beginning of the results so that a decryption
   ...
   * perform a brute force attack on each separate piece of encrypted data. The same is applied if a
   * random IV generator is used.
   *
   ...
   *
   * <p>If a random IV generator is used, this decryption operation will expect to find an
   * unencrypted IV at the beginning of the encrypted input, so that the decryption operation can be
  ...
  public byte[] decrypt(final byte[] encryptedMessage)
    ...
    try {
      ...
      if (this.saltGenerator.includePlainSaltInEncryptionResults()) {
        ...
        final int saltSize =
            (this.saltSizeBytes < encryptedMessage.length
                ? this.saltSizeBytes
        ...
        final int encMesKernelSize =
            (this.saltSizeBytes < encryptedMessage.length
                ? (encryptedMessage.length - this.saltSizeBytes)
                : 0);
        ...
        System.arraycopy(
            encryptedMessage, encMesKernelStart, encryptedMessageKernel, 0, encMesKernelSize);
      ...
      if (this.ivGenerator.includePlainIvInEncryptionResults()) {
        ...
        final int ivStart = 0;
        final int ivSize =
            (this.ivSizeBytes < encryptedMessageKernel.length
                ? this.ivSizeBytes
        ...
        final int encMesKernelSize =
            (this.ivSizeBytes < encryptedMessageKernel.length
                ? (encryptedMessageKernel.length - this.ivSizeBytes)
                : 0);
  ...
  private PBEParameterSpec buildPBEParameterSpec(final byte[] salt, final byte[] iv) {
    ...
    try {
      ...
      Constructor<PBEParameterSpec> java8Constructor =
          PBEParameterSpec.class.getConstructor(parameters);
