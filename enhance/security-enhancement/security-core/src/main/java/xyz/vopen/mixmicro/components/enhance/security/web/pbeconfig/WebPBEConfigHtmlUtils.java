...
/**
 * HTML creation class for internal use of {@link WebPBEConfigServlet} and {@link
 * WebPBEConfigFilter}.
 *
 * @since 1.3
...
final class WebPBEConfigHtmlUtils {
  ...
  public static final String PASSWORD_SETTING_FLAG = "mixSecurityPwSetting";
  public static final String VALIDATION_PREFIX = "mixSecurityVa";
  public static final String PASSWORD_PREFIX = "mixSecurityPw";
  public static final String PASSWORD_RETYPED_PREFIX = "mixSecurityRPw";
  ...
  public static String createInputFormHtml(
    ...
    while (configsIter.hasNext()) {
      ...
      strBuff.append(
          "      <label for=\""
              + VALIDATION_PREFIX
              + i
              + "\">Validation word</label>: <input type=\"password\" name=\""
              + VALIDATION_PREFIX
              + i
              + "\" />\n");
      ...
      strBuff.append(
          "      <label for=\""
              + PASSWORD_PREFIX
              + i
              + "\">Password</label>: <input type=\"password\" name=\""
              + PASSWORD_PREFIX
              + i
              + "\" />\n");
      strBuff.append("      <br /><br />\n");
      strBuff.append(
          "      <label for=\""
              + PASSWORD_RETYPED_PREFIX
              + i
              + "\">Retype password</label>: <input type=\"password\" name=\""
              + PASSWORD_RETYPED_PREFIX
              + i
              + "\" />\n");
      strBuff.append("     </fieldset>\n");
    ...
    strBuff.append(
        "     <input type=\"hidden\" name=\"" + PASSWORD_SETTING_FLAG + "\" value=\"true\" />\n");
  ...
  private static void addHeader(final StringBuffer strBuff) {
    ...
    strBuff.append(
        "   html { background-color: #ccc; text-align: center; margin: 0px; padding: 0px;} body {text-align:center;} #page { width: 700px; background-color: white; margin-top: 10px; margin-left: auto; margin-right: auto; padding: 10px; border: 1px solid #000; text-align: left;} h1 { text-weight: bold;} #button { text-align: center; margin-top: 20px; } fieldset { margin-bottom: 20px; } label { font-style: italic; } legend { font-weight: bold; } div.warning { border: 1px dotted #000; margin: 15px; padding: 5px; background-color: eee; font-weight: bold; }");
