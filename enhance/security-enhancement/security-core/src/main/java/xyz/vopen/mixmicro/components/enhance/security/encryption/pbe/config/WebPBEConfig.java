...
import xyz.vopen.mixmicro.components.enhance.security.web.pbeconfig.WebPBEConfigRegistry;

/**
 * Implementation for {@link PBEConfig} which can be used from the Web PBE Config infrastructure
 * (Filter + Servlet) to set the password for an encryptor from the web at initialization time.
 *
...
public class WebPBEConfig extends SimplePBEConfig {
  ...
  /**
   * Returns the name by which this WebPBEConfig object will be identified from the web. This name
   * must be unique for each WebPBEConfig object.
   *
  ...
  /**
   * Sets the name by which this WebPBEConfig object will be identified from the web. This name must
   * be unique for each WebPBEConfig object.
   *
  ...
  /**
   * Returns the validation word which will be asked from the web to the person setting the password
   * for the encryptor this config object belongs to. This validation word will make sure that only
   * an authorized person (for example, the application deployer) sets the value for the encryption
   * password.
   *
  ...
  /**
   * Sets the validation word which will be asked from the web to the person setting the password
   * for the encryptor this config object belongs to. This validation word will make sure that only
   * an authorized person (for example, the application deployer) sets the value for the encryption
   * password.
   *
  ...
  /**
   * Returns whether this config object is complete or not (both name and validation word have been
   * set). <b>Intended for internal use only</b>.
   *
