...
/**
 * String based implementation of {@link IvGenerator}, that will always return the same
 * initialization vector (IV). This IV is returned as bytes using the specified charset for
 * conversion (UTF-8 by default).
 *
 * <p>If the requested IV has a size in bytes smaller than the specified IV, the first n bytes are
 * returned. If it is larger, an exception is thrown.
 *
...
public class StringFixedIvGenerator implements FixedIvGenerator {
  ...
  /**
   * Creates a new instance of <tt>StringFixedIvGenerator</tt> using the default charset.
   *
  ...
  /**
   * As this IV generator provides a fixed IV, its inclusion unencrypted in encryption results is
   * not necessary, and in fact not desirable (so that it remains hidden).
   *
   * @return false
   */
