...
/**
 * Utils for processing numbers in encryptors. Intended only for internal use within security.
 *
 * @since 1.2
...
final class NumberUtils {
  ...
  static byte[] processBigIntegerEncryptedByteArray(final byte[] byteArray, final int signum) {
    ...
    if (byteArray.length > 4) {
      ...
      final int expectedSize = NumberUtils.intFromByteArray(encryptedMessageExpectedSizeBytes);
      ...
      if (processedByteArray.length != expectedSize) {
        ...
        System.arraycopy(
            processedByteArray,
            0,
            paddedProcessedByteArray,
            sizeDifference,
            processedByteArray.length);
