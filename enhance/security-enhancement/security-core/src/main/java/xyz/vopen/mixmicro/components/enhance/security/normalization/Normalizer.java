 ...
 * =============================================================================
 */
package xyz.vopen.mixmicro.components.enhance.security.normalization;
 ...
 *
 * <p>This class tries to use the <tt>java.text.Normalizer</tt> class in JDK 1.6 first and, if it
 * the class is not found (Java version < 6), then it will use the ICU4J
 * <tt>com.ibm.icu.text.Normalizer</tt> class (in this case, a <tt>ClassNotFoundException</tt> will
 * be thrown if ICU4J is not present).
 *
 * @since 1.5
...
public final class Normalizer {
   ...
   *
   * <p>This algorithm will first try to normalize the input's UNICODE using icu4j's
   * <tt>com.ibm.icu.text.Normalizer</tt> and, if it is not present at the classpath, will try to
   * use <tt>java.text.Normalizer</tt>. If this is not present either (this class appeared in JavaSE
   * 6), it will raise an exception.
   *
      ...
      try {
        ...
        try {
          ...
          throw new EncryptionInitializationException(
              "Cannot find a valid UNICODE normalizer: neither "
                  + JDK_NORMALIZER_CLASS_NAME
                  + " nor "
                  + ICU_NORMALIZER_CLASS_NAME
                  + " have been found at the classpath. If you are using "
  ...
  static void initializeJavaTextNormalizer()
    ...
    final Field javaTextNormalizerFormNFCConstantField =
        javaTextNormalizerFormClass.getField("NFC");
  ...
  static char[] normalizeWithJavaNormalizer(final char[] message) {
    ...
    if (javaTextNormalizerMethod == null || javaTextNormalizerFormNFCConstant == null) {
      throw new EncryptionInitializationException(
          "Cannot use: "
              + JDK_NORMALIZER_FORM_CLASS_NAME
              + ", as JDK-based normalization has "
    ...
    try {
      result =
          (String)
              javaTextNormalizerMethod.invoke(
    ...
    while (true) {
      ...
      normalizationResultSize =
          com.ibm.icu.text.Normalizer.normalize(
              message, normalizationResult, com.ibm.icu.text.Normalizer.NFC, 0);
