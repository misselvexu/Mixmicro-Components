...
import xyz.vopen.mixmicro.components.enhance.security.digest.config.StringDigesterConfig;
...
/**
 * Standard implementation of the {@link StringDigester} interface. This class lets the user specify
 ...
 * different default charsets, and returns digests in the form of BASE64-encoded or HEXADECIMAL
 * ASCII Strings.
 *
 ...
 * </ul>
 *
 * Once a digester has been initialized, trying to change its configuration will result in an
 ...
 *   <li>The result of the concatenation is encoded in BASE64 or HEXADECIMAL and returned as an
 *       ASCII String.
 * </ol>
 *
...
public final class StandardStringDigester implements StringDigester {
  ...
  /**
   * Charset to be used to obtain "digestable" byte arrays from input Strings. Set to <b>UTF-8</b>.
   *
   ...
   *
   * <p>Setting this value to UTF-8 does not mean that Strings that originally come for, for
   * example, an ISO-8859-1 input, will not be correcly digested. It simply provides a way of
   * "fixing" the way a String will be converted into bytes for digesting.
   */
  public static final String MESSAGE_CHARSET = "UTF-8";

  /**
   * Charset to be used for encoding the resulting digests. Set to <b>US-ASCII</b>.
   *
   * <p>The result of digesting some bytes can be any other bytes, and so the result of digesting,
   * for example, some LATIN-1 valid String bytes, can be bytes that may not conform a "valid"
   * LATIN-1 String.
   *
   * <p>Because of this, digests are always encoded in <i>BASE64</i> or HEXADECIMAL after being
   * created, and this ensures that the digests will make perfectly representable, safe ASCII
   * Strings. Because of this, the charset used to convert the digest bytes to the returned String
   * is set to <b>US-ASCII</b>.
   */
  public static final String DIGEST_CHARSET = "US-ASCII";

  /**
   * Whether the Unicode normalization step should be ignored because of legacy-compatibility
   * issues. Defaults to <b>FALSE</b> (the normalization step WILL be performed).
   */
  ...
  /**
   * Sets a <tt>{@link DigesterConfig}</tt> or {@link StringDigesterConfig}
   * object for the digester. If this config object is set, it will be asked values for:
   *
   * <ul>
   *   <li>Algorithm
   ...
   *   <li>Salt generator
   *   <li>Use of Unicode normalization mechanisms (only <tt>StringDigesterConfig</tt>)
   *   <li>Output type (base64, hexadecimal) (only <tt>StringDigesterConfig</tt>)
   * </ul>
   *
  ...
  public synchronized void setInvertPositionOfSaltInMessageBeforeDigesting(
    ...
    this.byteDigester.setInvertPositionOfSaltInMessageBeforeDigesting(
  ...
  public synchronized void setInvertPositionOfPlainSaltInEncryptionResults(
    ...
    this.byteDigester.setInvertPositionOfPlainSaltInEncryptionResults(
   ...
   *     ignored or not.
   */
  public synchronized void setUnicodeNormalizationIgnored(
      final boolean unicodeNormalizationIgnored) {
    if (isInitialized()) {
   ...
   *
   * <p>By default, no prefix will be added to encryption results.
   *
   * @since 1.7
   ...
   * EncryptionOperationNotPossibleException} if not).
   *
   * <p>By default, no suffix will be added to encryption results.
   *
   * @since 1.7
  ...
  public synchronized void initialize() {
    ...
    if (!this.isInitialized()) {
      ...
      if (this.stringDigesterConfig != null) {
        ...
        final String configStringOutputType = this.stringDigesterConfig.getStringOutputType();
        final String configPrefix = this.stringDigesterConfig.getPrefix();
        final String configSuffix = this.stringDigesterConfig.getSuffix();

        this.unicodeNormalizationIgnored =
            ((this.unicodeNormalizationIgnoredSet) || (configUnicodeNormalizationIgnored == null))
                ? this.unicodeNormalizationIgnored
                : configUnicodeNormalizationIgnored.booleanValue();
   ...
   *       <i>undigested</i> salt and the final result of the hash function are concatenated and
   *       returned as a result.
   *   <li>The result of the concatenation is encoded in BASE64 (default) or HEXADECIMAL and
  ...
  public boolean matches(final String message, final String digest) {
    ...
    if (processedDigest != null) {
      ...
      if (this.suffix != null) {
        ...
        processedDigest =
            processedDigest.substring(0, processedDigest.length() - this.suffix.length());
      }
    }
