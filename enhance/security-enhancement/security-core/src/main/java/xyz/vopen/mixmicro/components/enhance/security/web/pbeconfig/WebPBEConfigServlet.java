 ...
 *
 * <p>This servlet's URL should be called by the webapp administrator at deploy time, for setting
 * the passwords of all the PBE encryptors which have been previously assigned a {@link
 * WebPBEConfig} configuration object.
 *
 * <p>If web PBE configuration has not been done yet, it will show the user a form containing two
 * inputs for each encryptor: the <i>validation word</i> and the <i>password</i> (retyped).
 *
 * <ul>
 *   <li>The <i>validation word</i> must be input to the value set on the {@link WebPBEConfig}
 *       object with its <tt>setValidationWord(...)</tt> method. This will ensure that only an
 *       authorized person will set the encryption passwords..
 *   <li>The <i>password</i> (retyped) must be input to the value which is desired to be the
 *       encryption password for each specific encryptor.
 * </ul>
 *
 ...
 * </pre>
 *
 * <p>If this servlet's context is set a logger, it will output messages for both successful and
 * failed attempts to set passwords, including date, time and originating IP address.
 *
 * @since 1.3
  ...
  private void execute(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    try {
      ...
      if (registry.isWebConfigurationDone()) {
        ...
        final String settingFlag = req.getParameter(WebPBEConfigHtmlUtils.PASSWORD_SETTING_FLAG);
        if (CommonUtils.isEmpty(settingFlag)) {
          ...
          while (configsIter.hasNext()) {
            ...
            final String validation = req.getParameter(WebPBEConfigHtmlUtils.VALIDATION_PREFIX + i);
            final String password = req.getParameter(WebPBEConfigHtmlUtils.PASSWORD_PREFIX + i);
          ...
          if (valid < configs.size()) {
            ...
            while (configsIter.hasNext()) {
              ...
              String password = req.getParameter(WebPBEConfigHtmlUtils.PASSWORD_PREFIX + i);
      ...
      this.getServletContext().log("Exception raised during servlet execution", e);
      throw e;
    } catch (Throwable t) {
      this.getServletContext().log("Exception raised during servlet execution", t);
