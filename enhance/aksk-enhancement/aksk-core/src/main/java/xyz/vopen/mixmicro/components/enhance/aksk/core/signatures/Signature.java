...
public class Signature {
  ...
  /**
   * REQUIRED. The `keyId` field is an opaque string that the server can use to look up the
   * component they need to validate the signature. It could be an SSH key fingerprint, a URL to
   * machine-readable key data, an LDAP DN, etc. Management of keys and assignment of `keyId` is out
   * of scope for this document.
   */
  private final String keyId;

  /**
   * RECOMMENDED. The `signingAlgorithm` parameter is used to specify the digital signature
   * algorithm to use when generating the signature. Valid values for this parameter can be found in
   * the Signature Algorithms registry located at http://www.iana.org/assignments/signature-
   * algorithms and MUST NOT be marked "deprecated".
   *
   * <p>Verifiers MUST determine the signature's Algorithm from the keyId parameter rather than from
   * algorithm. If algorithm is provided and differs from or is incompatible with the algorithm or
   * key material identified by keyId (for example, algorithm has a value of rsa-sha256 but keyId
   * identifies an EdDSA key), then implementations MUST produce an error.
   *
   * <p>https://datatracker.ietf.org/doc/draft-ietf-httpbis-message-signatures/
   */
  private final xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm
      signingAlgorithm;

  /**
   * REQUIRED. The `algorithm` parameter is used to specify the digital signature algorithm to use
   * when generating the signature. Valid values for this parameter can be found in the Signature
   * Algorithms registry located at http://www.iana.org/assignments/signature- algorithms and MUST
   * NOT be marked "deprecated".
   */
  private final Algorithm algorithm;

  /**
   * REQUIRED. The `signature` parameter is a base 64 encoded digital signature, as described in RFC
   * 4648 [RFC4648], Section 4 [4]. The client uses the `algorithm` and `headers` signature
   * parameters to form a canonicalized `signing string`. This `signing string` is then signed with
   * the key associated with `keyId` and the algorithm corresponding to `algorithm`. The `signature`
   * parameter is then set to the base 64 encoding of the signature.
   *
   * <p>Signing: this field is calculated from the input data. Verification: this field is parsed
   * from the signature field in the Authorization header.
   */
  private final String signature;

  /**
   * OPTIONAL. The `headers` parameter is used to specify the list of HTTP headers included when
   * generating the signature for the message. If specified, it should be a lowercased, quoted list
   * of HTTP header fields, separated by a single space character. If not specified, implementations
   * MUST operate as if the field were specified with a single value, the `Date` header, in the list
   * of HTTP headers. Note that the list order is important, and MUST be specified in the order the
   * HTTP header field-value pairs are concatenated together during signing.
   */
  private final List<String> headers;

  /**
   * OPTIONAL. The `parameterSpec` is used to specify the cryptographic parameters. Some
   * cryptographic algorithm such as RSASSA-PSS require parameters.
   */
  private final AlgorithmParameterSpec parameterSpec;

  /**
   * OPTIONAL. The configurable signature's maximum validation duration, in milliseconds. This field
   * is applicable when the signed headers include '(expires)'. In that case, the value of the
   * '(expires)' field is calculated by adding maxSignatureValidityDuration to the timestamp of the
   * signature creation time.
   *
   * <p>This field is used to derive the signature expiration time when a cryptographic signature is
   * generated.
   */
   ...
   *
   * <p>This field is set at the time the cryptographic signature is generated, or when the
   * 'Authorization' header is parsed.
   */
  ...
  /**
   * Regular expression pattern for fields present in the Authorization field. Fields value may be
   * double-quoted strings, e.g. algorithm="hs2019" Some fields may be numerical values without
   * double-quotes, e.g. created=123456
   */
  private static final Pattern RFC_2617_PARAM =
      Pattern.compile("(?<key>\\w+)=((\"(?<stringValue>[^\"]*)\")|(?<numberValue>\\d+\\.?\\d*))");

  /**
   * The maximum time skew between the client and the server. This is used to validate the (created)
   * and (expires) fields in the HTTP signature.
   */
  public static long maxTimeSkewInMilliseconds = 30 * 1000L;

  /**
   * Construct a signature configuration instance with the specified keyId, algorithm and HTTP
   * headers.
   *
   * @param keyId An opaque string that the server can use to look up the component they need to
   *     validate the signature.
   * @param signingAlgorithm An identifier for the HTTP Signature algorithm. This should be "hs2019"
   *     except for legacy applications that use an older version of the draft HTTP signature
   *     specification.
  ...
  private static xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm
      getSigningAlgorithm(final String scheme) {
    if (scheme == null) throw new IllegalArgumentException("Signing scheme cannot be null");
    return xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm.get(scheme);
  }

  @Deprecated
  public Signature(
      final String keyId, final String algorithm, final String signature, final String... headers) {
  ...
  public Signature(
      final String keyId,
      final xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm
          signingAlgorithm,
      final Algorithm algorithm,
      final AlgorithmParameterSpec parameterSpec,
      final String signature,
  ...
  public Signature(
    ...
    if (signingAlgorithm != null
      ...
      throw new IllegalArgumentException(
          "Signing algorithm "
              + signingAlgorithm.getAlgorithmName()
  ...
  /**
   * Returns the identifier for the HTTP Signature Algorithm, as registered in the HTTP Signature
   * Algorithms Registry.
   *
   * @return the identifier for the HTTP Signature Algorithm.
   */
  public xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm
      getSigningAlgorithm() {
   ...
   *
   * <p>When the '(created)' field is present in the HTTP signature, the '(created)' field
   * represents the date when the signature has been created. When the '(expires)' field is present
   * in the HTTP signature, the '(expires)' field represents the date when the signature expires.
   */
  public void verifySignatureValidityDates() {
    if (signatureCreatedTime != null
        && signatureCreatedTime > System.currentTimeMillis() + maxTimeSkewInMilliseconds) {
   ...
   *
   * <p>As stated in the HTTP signature specification, the value of the algorithm parameter in the
   * 'Authorization' header should be set to generic identifier. The detailed algorithm should be
   * derived from the keyId. Hence it is not possible to determine the detailed algorithm by
   * inspecting the signature data.
   *
   * @param authorization The value of the HTTP 'Authorization' header containing the signature
   *     data.
  ...
  public static Signature fromString(String authorization, final Algorithm algorithm) {
    ...
    try {
      ...
      xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm
          parsedSigningAlgorithm = null;
      try {
        parsedSigningAlgorithm =
            xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm.get(
                algorithmField);
      } catch (
          final xyz.vopen.mixmicro.components.enhance.aksk.core.signatures
      ...
      try {
        ...
        if (algorithm != null && parsedAlgorithm.getPortableName() != algorithm.getPortableName()) {
          throw new IllegalArgumentException(
              "The algorithm does not match the value of the 'Authorization' header.");
        }
      } catch (
          final xyz.vopen.mixmicro.components.enhance.aksk.core.signatures
                  .UnsupportedAlgorithmException
      ...
      throw new xyz.vopen.mixmicro.components.enhance.aksk.core.signatures
          .UnparsableSignatureException(authorization, e);
    }
  }
  ...
  public String toString() {
    final Object alg;
    if (xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.SigningAlgorithm.HS2019.equals(
        signingAlgorithm)) {
    ...
    return "Signature "
        ...
        + keyId
        + '\"'
        + (signatureCreatedTime != null
            ? String.format(",created=%d", signatureCreatedTime / 1000L)
            : "")
        + (signatureExpiresTime != null
