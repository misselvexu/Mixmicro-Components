...
/**
 * It is an intentional part of the design that the same Signer instance can be reused on several
 * HTTP Messages in a multi-threaded fashion
 *
 * <p>The supplied Signature instance will be used as the basis for all future signatures created
 * from this Signer.
 *
 * <p>Each call to 'sign' will emit a Signature with the same 'keyId', 'algorithm', 'headers' but a
 * newly calculated 'signature'
 *
...
public class Signer {
  ...
  public Signer(final Key key, final Signature signature, final Provider provider) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Can't initialise the Signer using the provided algorithm and key", e);
    }
  }
   ...
   * @param uri The path and query of the request target of the message. The value must already be
   *     encoded exactly as it will be sent in the request line of the HTTP message. No URL encoding
   *     is performed by this method.
   * @param headers The HTTP headers.
   * @return a Signature object containing the signed message.
   */
  public Signature sign(final String method, final String uri, final Map<String, String> headers)
      throws IOException {
  ...
  public String createSigningString(
    ...
    return Signatures.createSigningString(
        signature.getHeaders(), method, uri, headers, created, expires);
  }
   ...
   */
  public String createSigningString(
      final String method, final String uri, final Map<String, String> headers) throws IOException {
    return Signatures.createSigningString(
        signature.getHeaders(),
        method,
        uri,
  ...
  private class Asymmetric implements Sign {
    ...
    public byte[] sign(final byte[] signingStringBytes) {
      try {
        ...
        final java.security.Signature instance =
            provider == null
        ...
        throw new xyz.vopen.mixmicro.components.enhance.aksk.core.signatures
            .UnsupportedAlgorithmException(algorithm.getJvmName());
