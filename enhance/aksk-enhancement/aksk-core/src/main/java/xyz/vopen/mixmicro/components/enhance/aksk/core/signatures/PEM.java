...
/**
 * A PEM utility that can be used to read keys from PEM. With this PEM utility, private keys in
 * either PKCS#1 or PKCS#8 PEM encoded format can be read without the need to depend on the Bouncy
 * Castle library.
 *
 * <p>Some background information:
 *
 * <ul>
 *   <li>Interestingly, the creation of a CloudFront Key Pair via the AWS console would result in a
 *       private key in PKCS#1 PEM format.
 *   <li>Unfortunately, the JDK doesn't provide a means to load PEM key encoded in PKCS#1 without
 *       adding the Bouncy Castle to the classpath. The JDK can only load PEM key encoded in PKCS#8
 *       encoding.
 *   <li>One the other hand, one can use openssl to convert a PEM file from PKCS#1 to PKCS#8.
 *       Example:
 *       <pre>
...
public enum PEM {
   ...
   * @throws java.security.spec.InvalidKeySpecException if failed to convert the DER bytes into a
   *     private key.
   ...
   */
  public static PrivateKey readPrivateKey(final InputStream is)
    ...
    for (final PEMObject object : objects) {
      switch (object.getPEMObjectType()) {
        case PRIVATE_KEY_PKCS1:
          return xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.RSA.privateKeyFromPKCS1(
              object.getDerBytes());
        case PRIVATE_EC_KEY_PKCS8:
          return EC.privateKeyFromPKCS8(object.getDerBytes());
        case PRIVATE_KEY_PKCS8:
          try {
            return xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.RSA
                .privateKeyFromPKCS8(object.getDerBytes());
    ...
    for (final PEMObject object : readPEMObjects(is)) {
      ...
      switch (object.getPEMObjectType()) {
        case PUBLIC_KEY_X509:
          try {
            return xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.RSA.publicKeyFrom(
                object.getDerBytes());
          } catch (final InvalidKeySpecException e) {
  ...
  /**
   * A lower level API used to returns all PEM objects that can be read off from the input stream of
   * a PEM file.
   *
   * <p>This method can be useful if more than one PEM object of different types are embedded in the
   * same PEM file.
   */
        ...
        if (readingContent) {
          if (line.contains(endMarker)) {
            pemContents.add( // completed reading one PEM object
                new PEMObject(
                    beginMarker,
                    xyz.vopen.mixmicro.components.enhance.aksk.core.signatures.Base64.decodeBase64(
   ...
   *
   * <p>A PEM file can contain one or multiple PEM objects, each with a beginning and ending marker.
   */
