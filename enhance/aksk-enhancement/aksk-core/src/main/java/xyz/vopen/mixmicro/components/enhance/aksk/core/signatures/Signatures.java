...
public enum Signatures {
  ...
  /**
   * Create a canonicalized string representation of the HTTP request. It is used as the input to
   * calculate the signature of the HTTP request.
   *
   ...
   * calculate the signature of the HTTP request.
   *
   * <p>The provided method, path and query values are used to generate the optional
    ...
    for (final String key : required) {
      if ("(request-target)".equals(key)) {
        ...
        if (signatureCreationTime == null) {
          throw new InvalidCreatedFieldException(
              "(created) field requested but signature creation time is not set");
        }
        list.add(
            key + ": " + Long.toString(TimeUnit.MILLISECONDS.toSeconds(signatureCreationTime)));
        ...
        if (signatureExpiryTime == null) {
          throw new InvalidExpiresFieldException(
              "(expires) field requested but signature expiration time is not set");
        }
